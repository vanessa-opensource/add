...
Функция ПолучитьКонтекстЯдраНаСервере()
	...
	Если НЕ МетаданныеЯдра = Неопределено
		...
		ИмяОбработкиКонтекстаЯдра = СтрЗаменить(ПолноеИмяБраузераТестов, "ОбработкаОбъект", "Обработки");
		КонтекстЯдра = Вычислить(ИмяОбработкиКонтекстаЯдра + ".Создать()");	
	Иначе
		КонтекстЯдра = ВнешниеОбработки.Создать("xddTestRunner", Ложь);
	КонецЕсли;
