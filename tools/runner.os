...
Процедура Декомпилировать(Знач Путь, Знач КаталогВыгрузки, Знач КорневойПутьВыходной = "", Знач КорневойПутьВходной = "", Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы = "") Экспорт
	...
	НастройкиИзФайла = ПрочитатьНастройкиФайлJSON(КорневойПутьПроекта, "./build/cache.txt");
		Если НастройкиИзФайла.Количество() = 0 Тогда
			...
			Лог.Отладка("Нет настроек в файле ./build/cache.txt");
		ИначеЕсли НастройкиИзФайла.Получить("copmpileepfrunner") = Неопределено Тогда
			//НастройкиИзФайла = Новый Соответствие();
			НастройкиИзФайла.Вставить("copmpileepfrunner", Новый Соответствие);
			Лог.Отладка("Нет настройки copmpileepfrunner в файле ./build/cache.txt");
		КонецЕсли;
...
Процедура СледитьЗаИзменениямиФайловВРепозиторииИсходников(Значение, КэшПутей, Фильтр = "", ЕстьИзменения = Ложь)
	...
	НайтиВсеФайлыВКаталогеИПодкаталогах(СписокФайлов, КаталогВходящий, ПолучитьМаскуВсеФайлы());
...
Процедура НайтиВсеФайлыВКаталогеИПодкаталогах(Знач МассивФайлов, Знач Каталог, Знач Маска)
	НовыйМассивФайлов = Неопределено;
	Если МассивФайлов = Неопределено Тогда
		МассивФайлов = Новый Массив;
	КонецЕсли;
	Попытка
		Лог.Отладка("Ищу все файлы в каталоге %1 и его подкаталогах. Маска поиска %1", Каталог, Маска);
		НовыйМассивФайлов = НайтиФайлы(Каталог, Маска, Истина);
		Если ЗначениеЗаполнено(НовыйМассивФайлов) Тогда
			Если Не ЗначениеЗаполнено(МассивФайлов) Тогда
				МассивФайлов = НовыйМассивФайлов;
			Иначе
				Для каждого Файл Из НовыйМассивФайлов Цикл
					МассивФайлов.Добавить(Файл);
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	Исключение
		Если СтрНайти(ОписаниеОшибки(), "System.IO.PathTooLongException") <> 0 Тогда
			Лог.Предупреждение("Получил ошибку длинных путей файлов. Выполняю поиск через перебор подкаталогов");
			
			МассивФайловКаталога = НайтиФайлы(Каталог, Маска, Ложь);
			Для каждого Файл Из МассивФайловКаталога Цикл
				Если Файл.ЭтоКаталог() Тогда 
					НайтиВсеФайлыВКаталогеИПодкаталогах(МассивФайлов, Файл.ПолноеИмя, Маска);
				КонецЕсли;
			КонецЦикла;
		Иначе
			ВызватьИсключение;
