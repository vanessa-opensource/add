...
Процедура Декомпилировать(Знач Путь, Знач КаталогВыгрузки, Знач КорневойПутьВыходной = "", Знач КорневойПутьВходной = "", Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы = "") Экспорт
	...
	Если РезультатыОбработкиВнешнихОбработок.Количество() > 0 Тогда 
		Лог.Ошибка("Были ошибки при разборе внешних файлов!");
...
Процедура ЗапуститьРаспаковкуВнешнихОбработок(Знач Файл, Знач ПапкаИсходников, СтрокаПодключения, Пользователь, Пароль, ВерсияПлатформы)
	...
	Лог.Отладка("Запускаем распаковку файла %1", Файл.ПолноеИмя);
	...
	Для Каждого НайденныйФайл Из НайденныеФайлы Цикл
		Лог.Отладка("Нашли кандидата на переименование %1
		|	%2", НайденныйФайл.Имя, НайденныйФайл.ПолноеИмя);
		Лог.Отладка("Проверяемое имя исходного файла %1", Файл.ИмяБезРасширения);

		Если ИменаФайловСовпадают(НайденныйФайл.Имя, Файл.ИмяБезРасширения) Тогда
			Прервать;
		КонецЕсли;
		
		Если НайденныйФайл.ЭтоФайл() Тогда
			Лог.Отладка("Перемещаем файл: %1 в %2
			|	%3", НайденныйФайл.Имя, ОбъединитьПути(НайденныйФайл.Путь, Файл.ИмяБезРасширения) + ".xml", НайденныйФайл.ПолноеИмя);

			ПереместитьФайл(НайденныйФайл.ПолноеИмя, ОбъединитьПути(НайденныйФайл.Путь, Файл.ИмяБезРасширения) + ".xml");
		Иначе
			Лог.Отладка("Перемещаем файл/каталог: %1 в %2
			|	%3", НайденныйФайл.Имя, ОбъединитьПути(НайденныйФайл.Путь, Файл.ИмяБезРасширения), НайденныйФайл.ПолноеИмя);
...
Функция ИменаФайловСовпадают(ИмяФайла1, ИмяФайла2)
	Рез = НРег(ИмяФайла1) = НРег(ИмяФайла2);
	Возврат Рез;
КонецФункции
