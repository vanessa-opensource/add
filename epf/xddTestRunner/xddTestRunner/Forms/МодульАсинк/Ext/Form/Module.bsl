...
Функция ТиповаяСтруктураДиалогаВыбораФайла(Заголовок = "", Каталог = "", МножественныйВыбор = Ложь, ПолноеИмяФайла = "", Фильтр = "")
	...
	Возврат Новый Структура("Заголовок, Каталог, МножественныйВыбор, ПолноеИмяФайла, ПредварительныеПросмотр, ПроверятьСуществованиеФайла, Фильтр",
									?(ПустаяСтрока(Заголовок), ЗаголовокПоУмолчанию(), Заголовок),
...
Процедура смв_ПоместитьФайлы(ОписаниеОповещенияОЗавершении, 
	...
	#Если ВебКлиент Тогда
		...
		Если ПроверитьРасширениеРаботыСФайламиВебКлиент() Тогда
			...
			НачатьПомещениеФайлов(ОписаниеОповещенияОЗавершении, ПомещаемыеФайлы, , Ложь, КлючУникальности.УникальныйИдентификатор);
	...
	#Иначе
		...
		ИначеЕсли ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь, КлючУникальности.УникальныйИдентификатор) Тогда
...
Процедура смв_ПоместитьФайлыЧерезДиалог(ОписаниеОповещенияОЗавершении, 
	...
	#Если ВебКлиент Тогда
		...
		Если ПроверитьРасширениеРаботыСФайламиВебКлиент() Тогда
			...
			НачатьПомещениеФайлов(ОписаниеОповещенияОЗавершении, , ДиалогОткрытияФайла, Истина, КлючУникальности.УникальныйИдентификатор);
	...
	#Иначе
		...
		ИначеЕсли ПоместитьФайлы(, ПомещенныеФайлы, ДиалогОткрытияФайла, Истина, КлючУникальности.УникальныйИдентификатор) Тогда
...
Процедура смв_ПоместитьФайл(ОписаниеОповещенияОЗавершении,
								АдресХранения, ПомещаемыйФайл, Интерактивно = Ложь) Экспорт
								
	#Если ВебКлиент Тогда
		
		Если ПроверитьРасширениеРаботыСФайламиВебКлиент() Тогда
			
			НачатьПомещениеФайла(ОписаниеОповещенияОЗавершении, АдресХранения, ПомещаемыйФайл, Интерактивно, КлючУникальности.УникальныйИдентификатор);
	...
	#Иначе
		...
		Если МодальностьЗапрещена Тогда
			...
			Выполнить("НачатьПомещениеФайла(ОписаниеОповещенияОЗавершении, АдресХранения, ПомещаемыйФайл, Интерактивно, КлючУникальности.УникальныйИдентификатор)");
			
		ИначеЕсли ПоместитьФайлы(АдресХранения, ПомещаемыйФайл, ПомещаемыйФайл, Интерактивно, КлючУникальности.УникальныйИдентификатор) Тогда
				
			Выполнить("ОписаниеОповещения.Модуль." + ОписаниеОповещенияОЗавершении.ИмяПроцедуры + "(Истина, АдресХранилища, ПомещаемыйФайл, ОписаниеОповещения.ДополнительныеПараметры)");
...
Функция НовыйОписаниеПередаваемогоФайла(ИмяФайла, ДанныеДляПередачи) Экспорт
	...
	Адрес = ПоместитьВоВременноеХранилище(ДанныеДляПередачи, КлючУникальности.УникальныйИдентификатор);
...
Функция СтроковыеУтилиты()
	...
	Если СтроковыеУтилиты = Неопределено Тогда
		...
		СтроковыеУтилиты = КлючУникальности.Плагин("СтроковыеУтилиты");
