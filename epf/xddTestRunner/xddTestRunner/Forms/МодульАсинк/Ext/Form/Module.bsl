...
Процедура смв_ПоместитьФайл(ОписаниеОповещенияОЗавершении,
	...
	#Если ВебКлиент Тогда
		...
		Если ПроверитьРасширениеРаботыСФайламиВебКлиент() Тогда
			...
			PTG_НачатьПомещениеФайла(ОписаниеОповещенияОЗавершении, АдресХранения, ПомещаемыйФайл, Интерактивно,
...
&НаКлиенте
Процедура PTG_НачатьПомещениеФайла(ОписаниеОповещенияОЗавершении, АдресХранения, ПомещаемыйФайл, Интерактивно,
						Идентификатор)					
	...
	//Если ТипЗнч(ПомещаемыйФайл) = Тип("Строка") Тогда
	//	МассивИмени = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПомещаемыйФайл,".");
	//	Если МассивИмени.Количество() >0 Тогда
	//		ЭтоОтчет = МассивИмени[МассивИмени.Количество()-1]
	//	Иначе
	//		ЭтоОтчет = Ложь;
	//	КонецЕсли;
	//Иначе
	//	ЭтоОтчет = ПомещаемыйФайл.Расширение;
	//КонецЕсли;
	...
	Если ТипЗнч(ОписаниеОповещенияОЗавершении) = Тип("ОписаниеОповещения") Тогда
		...
		ПараметрыОповещения.Вставить("PTG_ОбработкаЗавершения", ОписаниеОповещенияОЗавершении);
	КонецЕсли;

	//ПараметрыОповещения.Вставить("ФайлОбработки", ПомещаемыйФайл);
	//ПараметрыОповещения.Вставить("ЭтоОтчет", Ложь);
	
	//Если ОписаниеОповещенияОЗавершении <> Неопределено Тогда
	//	ПараметрыОповещения.Вставить("ОбработкаЗавершения", ОписаниеОповещенияОЗавершении);
	//КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("PTG_ЗавершитьПомещениеФайла", ЭтотОбъект, ПараметрыОповещения);
...
&НаКлиенте
Процедура PTG_ЗавершитьПомещениеФайла(ПомещенныйФайл, ДополнительныеПараметры) Экспорт

	ОбработкаЗавершения = Неопределено;
	Если Не ДополнительныеПараметры.Свойство("PTG_ОбработкаЗавершения",ОбработкаЗавершения) Тогда
		Возврат;
	КонецЕсли;
	...
	Результат = НЕ ПомещенныйФайл = Неопределено; 
	Выполнить("ОбработкаЗавершения.Модуль." +  ОбработкаЗавершения.ИмяПроцедуры + 
				"(Результат, ПомещенныйФайл.Хранение, ПомещенныйФайл.Имя, ДополнительныеПараметры)");
