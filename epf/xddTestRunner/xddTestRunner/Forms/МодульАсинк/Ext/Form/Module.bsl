...
Процедура смв_ПоместитьФайлы(ОписаниеОповещенияОЗавершении,
	...
	#Если ВебКлиент Тогда
		...
		Если ПроверитьРасширениеРаботыСФайламиВебКлиент() Тогда
			...
			//	//НачатьПомещениеФайловНаСервер(<ОписаниеОповещенияОЗавершении>, <ОписаниеОповещенияОХодеВыполнения>, <ОписаниеОповещенияПередНачалом>,
			//	//<ОписанияПередаваемыхФайлов>, <УникальныйИдентификаторФормы>)
			//	НачатьПомещениеФайловНаСервер(ОписаниеОповещенияОЗавершении, Неопределено, Неопределено, ПомещаемыеФайлы, ВладелецФормы.УникальныйИдентификатор);
			//Иначе
	...
	#Иначе
		...
		ИначеЕсли МодальностьЗапрещена Тогда
			...
			//	//НачатьПомещениеФайловНаСервер(<ОписаниеОповещенияОЗавершении>, <ОписаниеОповещенияОХодеВыполнения>, <ОписаниеОповещенияПередНачалом>,
			//	//<ОписанияПередаваемыхФайлов>, <УникальныйИдентификаторФормы>)
			//	Выполнить("НачатьПомещениеФайловНаСервер(ОписаниеОповещенияОЗавершении, Неопределено, Неопределено, ПомещаемыеФайлы, ВладелецФормы.УникальныйИдентификатор)");
			//Иначе
			//	//НачатьПомещениеФайлов(<ОписаниеОповещения>, <ПомещаемыеФайлы>, <НаборПомещаемыхФайлов>, <Интерактивно>, <УникальныйИдентификаторФормы>)
...
Процедура смв_ПоместитьФайлыЧерезДиалог(ОписаниеОповещенияОЗавершении,
	...
	//ПараметрыДиалогаПомещенияФайлов = Неопределено;
	//Если Версия8315ИлиВыше Тогда
	//	ПараметрыДиалогаПомещенияФайлов = Новый ПараметрыДиалогаПомещенияФайлов();
	//	ЗаполнитьЗначенияСвойств(ПараметрыДиалогаПомещенияФайлов, ПараметрыДиалога);
	//КонецЕсли;
	...
	#Если ВебКлиент Тогда
		...
		Если ПроверитьРасширениеРаботыСФайламиВебКлиент() Тогда
			...
			//Если Версия8315ИлиВыше Тогда
			//	//НачатьПомещениеФайловНаСервер(<ОписаниеОповещенияОЗавершении>, <ОписаниеОповещенияОХодеВыполнения>, <ОписаниеОповещенияПередНачалом>,
			//	//<ПараметрыДиалога>, <УникальныйИдентификаторФормы>)
			//	НачатьПомещениеФайловНаСервер(ОписаниеОповещенияОЗавершении, Неопределено, Неопределено, ПараметрыДиалогаПомещенияФайлов, ВладелецФормы.УникальныйИдентификатор);
			//Иначе
			//	//НачатьПомещениеФайлов(<ОписаниеОповещения>, <ПомещаемыеФайлы>, <НаборПомещаемыхФайлов>, <Интерактивно>, <УникальныйИдентификаторФормы>)
	...
	#Иначе
		...
		Если МодальностьЗапрещена Тогда
			...
			//	//НачатьПомещениеФайловНаСервер(<ОписаниеОповещенияОЗавершении>, <ОписаниеОповещенияОХодеВыполнения>, <ОписаниеОповещенияПередНачалом>,
			//	//<ПараметрыДиалога>, <УникальныйИдентификаторФормы>)
			//	Выполнить("НачатьПомещениеФайловНаСервер(ОписаниеОповещенияОЗавершении, Неопределено, Неопределено, ПараметрыДиалогаПомещенияФайлов, ВладелецФормы.УникальныйИдентификатор);");
			//Иначе
			//	//НачатьПомещениеФайлов(<ОписаниеОповещения>, <ПомещаемыеФайлы>, <НаборПомещаемыхФайлов>, <Интерактивно>, <УникальныйИдентификаторФормы>)
...
Процедура смв_ПоместитьФайл(ОписаниеОповещенияОЗавершении,
	...
	#Если ВебКлиент Тогда
		...
		Если ПроверитьРасширениеРаботыСФайламиВебКлиент() Тогда
			...
			Если Версия8315ИлиВыше Тогда
				// //НачатьПомещениеФайлаНаСервер(<ОписаниеОповещенияОЗавершении>, <ОписаниеОповещенияОХодеВыполнения>, <ОписаниеОповещенияПередНачалом>,
				// //<Адрес>, <ПутьКФайлу>, <УникальныйИдентификаторФормы>)
				// НачатьПомещениеФайлаНаСервер(ОписаниеОповещенияОЗавершении, Неопределено, Неопределено, АдресХранения, ПомещаемыйФайл,
				// 	ВладелецФормы.УникальныйИдентификатор);
				НачатьПомещениеФайла_8_3_15_и_Выше(ОписаниеОповещенияОЗавершении, АдресХранения, ПомещаемыйФайл, Интерактивно,
					ВладелецФормы.УникальныйИдентификатор);
			Иначе
...
&НаКлиенте
Процедура НачатьПомещениеФайла_8_3_15_и_Выше(ОписаниеОповещенияОЗавершении, АдресХранения, ПомещаемыйФайл, Интерактивно,
						Идентификатор)

	Если ТипЗнч(ПомещаемыйФайл) = Тип("Строка") Тогда
		ИмяФайла = ПомещаемыйФайл;
	ИначеЕсли ТипЗнч(ПомещаемыйФайл) = Тип("СсылкаНаФайл") Тогда
		ИмяФайла = ПомещаемыйФайл.Файл.ПолноеИмя;
	Иначе
		ИмяФайла = ""; //TODO что делать с пустым именем??
	КонецЕсли;

	ПараметрыОповещения = Новый Структура;

	Если ТипЗнч(ОписаниеОповещенияОЗавершении) = Тип("ОписаниеОповещения") Тогда
		ПараметрыОповещения = ОписаниеОповещенияОЗавершении.ДополнительныеПараметры;
		ПараметрыОповещения.Вставить("ОбработкаЗавершения_8_3_15_и_Выше", ОписаниеОповещенияОЗавершении);
	КонецЕсли;

	ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершитьПомещениеФайла_8_3_15_и_Выше", ЭтотОбъект, ПараметрыОповещения);

	// ПараметрыПолучения = ФайловаяСистемаКлиент.ПараметрыЗагрузкиФайла();
	// ПараметрыПолучения.Интерактивно = Интерактивно;
	// ПараметрыПолучения.ИдентификаторФормы = Идентификатор;

	//ФайловаяСистемаКлиент.ЗагрузитьФайл(ОписаниеОповещения, ПараметрыПолучения, ИмяФайла, АдресХранения);
	//
	НачатьПомещениеФайлаНаСервер(ОписаниеОповещения, , , АдресХранения, ИмяФайла, Идентификатор);
КонецПроцедуры

&НаКлиенте
Процедура ЗавершитьПомещениеФайла_8_3_15_и_Выше(ПомещенныйФайл, ДополнительныеПараметры) Экспорт
	...
	Если Не ДополнительныеПараметры.Свойство("ОбработкаЗавершения_8_3_15_и_Выше", ОбработкаЗавершения) Тогда
		Возврат;
	КонецЕсли;
	...
	Результат = НЕ ПомещенныйФайл.ПомещениеФайлаОтменено;
	Выполнить("ОбработкаЗавершения.Модуль." +  ОбработкаЗавершения.ИмяПроцедуры +
				"(Результат, ПомещенныйФайл.Адрес, ПомещенныйФайл.СсылкаНаФайл.Имя, ДополнительныеПараметры)");
