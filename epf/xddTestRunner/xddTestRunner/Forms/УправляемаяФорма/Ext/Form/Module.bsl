...
Функция СоздатьФайлОбработкиНаСервере(Знач ПолныйПутьФайла, Знач ИдентификаторОбработки, ЭтоОтчет = Ложь)
	...
	НастройкиПродукта = ПолучитьИзВременногоХранилища(Объект.АдресВременногоХранилища);
	...
	Если Не КешПлагины.Свойство(ИдентификаторОбработки, Результат) Тогда
		...
		НастройкиПродукта.Вставить("КешПлагины", КешПлагины);
		ПоместитьВоВременноеХранилище(НастройкиПродукта, Объект.АдресВременногоХранилища);
