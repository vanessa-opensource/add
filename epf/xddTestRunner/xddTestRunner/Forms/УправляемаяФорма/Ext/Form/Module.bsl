...
&НаКлиенте
Функция ВыполнитьТесты(Знач Загрузчик, Знач КонтейнерДереваТестов, Знач Фильтр = Неопределено, Знач Подписчик = Неопределено, ИерархияИсполнения = Неопределено) Экспорт
	
	// Временный код.
	// Нужна Универсальная работа с потоковыми генераторами через параметры запуска.
	// TODO: Доделать.
	ПотоковыйВывод = Плагин("ПотоковыйГенераторОтчетаAllureXMLВерсия2");
	
	Родитель = Новый Структура("Родитель, Ключ, Тип, Имя", ИерархияИсполнения);
	ЗаполнитьЗначенияСвойств(Родитель, КонтейнерДереваТестов);
	...
	Для каждого ДочернийУзел Из СтрокиКонтейнера Цикл
		...
		Если ДочернийУзел.Тип = Объект.ТипыУзловДереваТестов.Контейнер Тогда
			ДочернийРезультатТестирования = ВыполнитьТесты(Загрузчик, ДочернийУзел, ФильтрДляДочернихУзлов, Подписчик, Родитель);
		ИначеЕсли ДочернийУзел.Тип = Объект.ТипыУзловДереваТестов.Элемент Тогда
			Если УзелДереваТестовУдовлетворяетФильтру(ДочернийУзел, ФильтрДляДочернихУзлов) Тогда
				...
				// Место вызова обработки потокового вывода.
				// Должны обрабатываться все подключенные плагины потокового вывода
				// TODO: доделать
				ПотоковыйВывод.ОбработатьПолученияРезультатаТеста(ЭтаФорма, ДочернийРезультатТестирования);
