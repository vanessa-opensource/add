...
Функция ВыполнитьТесты(Знач Загрузчик, Знач КонтейнерДереваТестов, Знач Фильтр = Неопределено, Знач Подписчик = Неопределено, ИерархияИсполнения = Неопределено, ПараметрыПотоковыхОтчетов = Неопределено) Экспорт
	...
	Отладка("КонтейнерДереваТестов.ТипL: " + КонтейнерДереваТестов.Тип);
	Отладка("ИерархияИсполнения: " + КлючЗначениеВСтроку(ИерархияИсполнения));
...
//  КлючЗначение - ФиксированноеСоответствие, Соответствие, Структура, ФиксированнаяСтруктура - 
//  Символ - Строка - Символ разделитель
//	Сдвиг - Число - служебный параметр
// Возвращаемое значение:
//   - 
//
&НаКлиенте
Функция КлючЗначениеВСтроку(КлючЗначение, Символ = "
	|", Сдвиг = 0) Экспорт  
	
	РазрешенныеТипы = Новый Массив();
	РазрешенныеТипы.Добавить(Тип("Структура"));
	РазрешенныеТипы.Добавить(Тип("Соответствие"));
	РазрешенныеТипы.Добавить(Тип("Массив"));
	РазрешенныеТипы.Добавить(Тип("ФиксированнаяСтруктура"));
	РазрешенныеТипы.Добавить(Тип("ФиксированнаяСтруктура"));
	РазрешенныеТипы.Добавить(Тип("ФиксированныйМассив"));
	
	// Проверяем типы
	Если РазрешенныеТипы.Найти(ТипЗнч(КлючЗначение)) = Неопределено Тогда
		Возврат "";	
	КонецЕсли;
	
	
	Ответ = Новый Массив();
	Для Каждого кз Из КлючЗначение Цикл 
		Если ТипЗнч(кз) = Тип("КлючИЗначение") Тогда
			Стр = СтрШаблон("Ключ = ""%1"": Значение = ""%2""", кз.Ключ, ОбработкаТипа(кз.Значение, Символ, Сдвиг));
		Иначе
			Стр = СтрШаблон("Значение = ""%1""", ОбработкаТипа(кз, Символ, Сдвиг));
		КонецЕсли;
		
		
		Стр = ДополнитьСтроку(Стр, СтрДлина(Стр) + Сдвиг, Символы.Таб, "Слева");
		Ответ.Добавить(Стр);
	КонецЦикла;
	
	Возврат "[" + Символы.ПС + СтрСоединить(Ответ, Символ) + Символы.ПС + "]";
КонецФункции

&НаКлиенте
Функция ОбработкаТипа(Значение, Символ, Сдвиг) 
	
	ЗначениеСтрокой = "";
	Если ТипЗнч(Значение) = Тип("Структура") Тогда
		ЗначениеСтрокой = СтрШаблон("Структура(
		|%1
		|)", КлючЗначениеВСтроку(Значение, Символ, Сдвиг+1));
	ИначеЕсли ТипЗнч(Значение) = Тип("Массив") Тогда                      
		врем = Новый Массив();
		Для Каждого знч Из Значение Цикл
			Если ТипЗнч(знч) = Тип("Структура") Тогда
				врем.Добавить(СтрШаблон("Структура(%1)", КлючЗначениеВСтроку(знч, ", ", Сдвиг)));
			Иначе
				врем.Добавить(Строка(знч));
			КонецЕсли;
		КонецЦикла;
		
		ЗначениеСтрокой = "[" + СтрСоединить(врем, ", ") + "]";
	Иначе
