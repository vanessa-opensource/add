...
Процедура КомандаПерезагрузитьФорму(Команда)
	...
	Иначе
		...
		АСинк().смв_ПоместитьФайлы(ОписаниеЗавершения, ЭтаФорма.ИспользуемоеИмяФайла );
...
Процедура ОткрытьИнструмент(Знач ИмяИнструмента, Знач ПутьИнструмента = "", Знач ИмяФормы = "Форма")
	...
	Иначе
		...
		АСинк().смв_Файл_Существуе(ФайлИнструмента, ОписаниеОповещения);
...
Функция АСинк() Экспорт
	...
	Если АСинк = Неопределено Тогда
		...
		Если Объект.ЭтоВстроеннаяОбработка Тогда
			...
			АСинк = ПолучитьФорму("Обработка.xddRunner.Форма.МодульАсинк", , ЭтаФорма, ЭтаФорма);
			
		Иначе
			
			АСинк = ПолучитьФорму("ВнешняяОбработка.xddRunner.Форма.МодульАсинк", , ЭтаФорма, ЭтаФорма);
