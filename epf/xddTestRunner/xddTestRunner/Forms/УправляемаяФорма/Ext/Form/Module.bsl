...
&НаКлиенте
Перем ИтераторТестирования; //массив итераторов, во время тестирования может начатся вложенный обход тестирования 8-/
...
Процедура ПриОткрытииЗавершение()
	...
	ЭтоПакетныйЗапуск = ЭтоПакетныйЗапуск();
	Если ЭтоПакетныйЗапуск Тогда
		ВыполнитьПакетныйЗапуск(ПараметрЗапуска);
	Иначе
		ПерезагрузитьПоследниеТестыПоИстории();
...
Процедура ВыполнитьТестыНаКлиенте(Знач Фильтр = Неопределено)
	Если ЗначениеЗаполнено(ЭтаФорма.ДеревоОтЗагрузчика) Тогда
		...
		СброситьПодключенныеВнешниеОбработки();
		РезультатТестирования = ВыполнитьТесты(Плагин(ИдентификаторЗагрузчика), ЭтаФорма.ДеревоОтЗагрузчика, Фильтр, ЭтаФорма);
		
		Если НЕ РезультатТестирования = Неопределено Тогда
			ОтразитьРезультатыТестирования(РезультатТестирования);	
		КонецЕсли; 
...
&НаКлиенте
Функция ВыполнитьТесты(Знач Загрузчик, Знач КонтейнерДереваТестов, Знач Фильтр = Неопределено, Знач Подписчик = Неопределено, ПриЗавершенииПакетныйЗапуск = Неопределено, ПараметрыПотоковыхОтчетов = Неопределено) Экспорт
	//ПриЗавершенииПакетныйЗапуск ранее ИерархияИсполнения. ИерархияИсполнения использовалоь в рекурсивном выполнении.
	
	ЭтотИтераторТестирования = ИтераторДереваТестов_Конструктор(КонтейнерДереваТестов,Фильтр);
	ДобавитьИтератор(ЭтотИтераторТестирования);
	
	ЭтотИтераторТестирования.Загрузчик 					= Загрузчик;
	ЭтотИтераторТестирования.Подписчик 					= Подписчик;
	ЭтотИтераторТестирования.ПараметрыПотоковыхОтчетов 	= ПараметрыПотоковыхОтчетов;

	Если НЕ ЕстьПоддержкаАсинхронныхВызовов ИЛИ ИтераторТестирования.Количество() > 1 Тогда
		//ИтераторТестирования.Количество() > 1 = самотестирование
		//TechDebt: Переделать самотест что бы он отркрывал новую форму браузера, 
		//а не использовал переданный КонтекстЯдра. Заменить переменную модуля со стэком итераторов на
		//просто итератор.
		РезультатТестирования = Неопределено;
		ОбойтиТестыДереваВнутр(РезультатТестирования);
		Возврат РезультатТестирования; 
	КонецЕсли;
	
	
	Если ЭтоПакетныйЗапуск() И ТипЗнч(ПриЗавершенииПакетныйЗапуск) = Тип("ОписаниеОповещения")  Тогда
		//Пакетный запуск в асинхронном режиме
		ЭтотИтераторТестирования.Завершить = ПриЗавершенииПакетныйЗапуск;
	ИначеЕсли НЕ ЭтоПакетныйЗапуск()  Тогда
		//Интерактивный запуск
		ПараметрыКонструктора = Новый массив;		
		ПараметрыКонструктора.Добавить("ОтразитьРезультатыТестирования"); //ИмяПроцедуры
		ПараметрыКонструктора.Добавить(ЭтаФорма); //Модуль
		ЭтотИтераторТестирования.Завершить = Новый("ОписаниеОповещения",ПараметрыКонструктора);	
	Иначе
		ВызватьИсключение "Не коррекный вызов метода, нужен 5ый параметр с описанием следующего шага пакетного запуска!";
	КонецЕсли; 
		
	ПараметрыКонструктора = Новый массив;
	ПараметрыКонструктора.Добавить("ОбойтиТестыДереваВнутр"); //ИмяПроцедуры
	ПараметрыКонструктора.Добавить(ЭтаФорма); //Модуль
	
	ОповещениеНачать = Новый("ОписаниеОповещения",ПараметрыКонструктора);
	Выполнить("ВыполнитьОбработкуОповещения(ОповещениеНачать,Неопределено)");
...
Процедура НачатьВыполнениеТестов(ДеревоТестовОтЗагрузчика, ОбработкаОповещения)
	...
	ЭтаФорма.ВыполнитьТесты(Загрузчик, ДеревоТестовОтЗагрузчика,,,ОбработкаОповещения,Параметры_xddStreamReport);
...
Процедура ДиспетчерЭкспортаОтчетовАсинхронныйПакетныйРежим(Знач РезультатЭкспорта, Знач ПараметрыДиспетчера) Экспорт
	...
	Если ПараметрыДиспетчера.ИндексВидаОтчета < СписокОтчетов.Количество() Тогда
		...
		Если ФормироватьСводныйФайл Тогда		
			...
			//TechDebt: ГенераторОтчета.НачатьЭкспорт(ОбработчикЭкспорта, Отчет, СтрокаЭкспорта.Путь); Chg: 6.4. - Метод не реализован
			ГенераторОтчета.Экспортировать(Отчет, СтрокаЭкспорта.Путь);
...
Процедура ПервичнаяНастройка()
	...
	Объект.ФлагОтладки = ПлагинНастроек.ПолучитьНастройку("Отладка") = Истина
						 ИЛИ (ЭтоПакетныйЗапуск() И Объект.ФлагОтладки); //Chg: в пакетном режиме флаг может быть передан в параметрах
...
Функция КонфигурацияПоддерживаетАсинхронныеВызовы()
	...
	//Даже если установлен флаг ИспользуютсяСинхронныеВызовыРасширенийПлатформыИВнешнихКомпонент
	//уконфигурации работающей на платформе с версией между 8.3.5.1383 и 8.3.13. использование асинхронных возможно.
	//
...
&НаКлиенте
Функция ЭтоПакетныйЗапуск()
	Если ЗначениеЗаполнено(ПараметрЗапуска) Тогда
		//Наличие команд в параметрах запуска не говорит о том,
		//что это пакетный запуск.
		ПарсерКоманднойСтроки = Плагин("ПарсерКоманднойСтроки");
		
		Если НЕ ПарсерКоманднойСтроки = Неопределено Тогда
			//При помощи парсера
			ПараметрыЗапуска = ПарсерКоманднойСтроки.Разобрать(ПараметрЗапуска);
			ВозможныеКлючи = ПарсерКоманднойСтроки.Объект.ВозможныеКлючи;
			Возврат ПараметрыЗапуска.Количество() > 0;
		Иначе
			//при отсутствии парсера.
			Возврат НЕ (ПараметрЗапуска = "РежимОтладки" 
		   	 			ИЛИ ПараметрЗапуска = "/РежимОтладки");
		КонецЕсли; 	
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
#Область ОбходДереваТестов

&НаКлиенте
Процедура ОбойтиТестыДереваВнутр(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	Перем ДочернийУзел, КонтекстВыполнения, Параметр, ПотоковыйВывод, Путь, РезультатТестирования;
	
	ЭтотИтераторТестирования = ТекущийИтератор();
	
	Если НЕ ДополнительныеПараметры = Неопределено Тогда
		//Это продолжение прерванного обхода
		Если НЕ Жду() Тогда Возврат; КонецЕсли; //уже продолжили по таймауту
		Если НЕ ЭтотИтераторТестирования.ОповещениеПродолжить.ДополнительныеПараметры.Ключ = ДополнительныеПараметры.Ключ Тогда Возврат; КонецЕсли; //ожидаем уже чего то другого
		ЭтотИтераторТестирования.ОповещениеПродолжить = Неопределено;
	КонецЕсли; 
	
	Пока ИтераторДереваТестов_Следующий(ЭтотИтераторТестирования) Цикл
		
		ДочернийУзел = ЭтотИтераторТестирования.Узел;
		
		Если ДочернийУзел.Контейнер.Тип = Объект.ТипыУзловДереваТестов.Элемент Тогда
			
			Если УзелДереваТестовУдовлетворяетФильтру(ДочернийУзел.Контейнер, ДочернийУзел.Выше.ФильтрДляДочернихУзлов) Тогда
				
				Если ДочернийУзел.Выше.Прервать Тогда Продолжить; КонецЕсли; 
				
				ВывестиСообщениеВЛогФайл(".    -->> тест " + ДочернийУзел.Контейнер.Представление);
				
				Если НЕ ДочернийУзел.Выше.Свойство("КонтекстВыполненияДляКонтейнера", КонтекстВыполнения) Тогда
					КонтекстВыполнения = Узел_ПолучитьКонтекстВыполнения(ДочернийУзел);						
				КонецЕсли; 

				Объект.ТекущийКонтейнер = ДочернийУзел.Выше.Контейнер;
				ДочернийУзел.РезультатТестирования = ВыполнитьТестовыйМетод(КонтекстВыполнения, ДочернийУзел.Контейнер);
				
				ДочернийУзел.Выше.Контейнер.Контекст = Объект.ТекущийКонтейнер.Контекст;
				
				Если ЭтотИтераторТестирования.Подписчик <> Неопределено Тогда
					ОповеститьОСобытии(ЭтотИтераторТестирования.Подписчик, Объект.ВозможныеСобытия.ВыполненТестовыйМетод, ДочернийУзел.РезультатТестирования);
				КонецЕсли;
				
				ДочернийУзел.РезультатТестирования.Вставить("Родитель", ДочернийУзел.Родитель);
				
				Если ЭтотИтераторТестирования.ПараметрыПотоковыхОтчетов <> Неопределено Тогда
					Для Каждого Параметр Из ЭтотИтераторТестирования.ПараметрыПотоковыхОтчетов Цикл
						ПотоковыйВывод = Плагин(Параметр[0]);
						Путь = Параметр[1];
						ПотоковыйВывод.ЗаписатьРезультатТеста(ЭтаФорма, ДочернийУзел.РезультатТестирования, Путь);
					КонецЦикла;
				КонецЕсли;
				
				Узел_ПриОкончанииОбработкиЭлемента(ДочернийУзел);
	...
	РезультатыТестирования = ЭтотИтераторТестирования.Узел.Ниже.РезультатТестирования;
	ОповещениеЗавершить = ЭтотИтераторТестирования.Завершить;
	ИтераторТестирования.Удалить(ИтераторТестирования.ВГраница());
		
	//мИтераторТестирования.Узел это корень итератора, не путать с корнем контейнеров дерева
	Если ОповещениеЗавершить = Неопределено Тогда
		Результат = РезультатыТестирования;	
	Иначе
		//через "выполнить" для совместимости с ранними платформами
		Если ТипЗнч(ОповещениеЗавершить.ДополнительныеПараметры) = Тип("Структура")
			 И ОповещениеЗавершить.ДополнительныеПараметры.Свойство("РезультатыТестирования") Тогда
			ОповещениеЗавершить.ДополнительныеПараметры.РезультатыТестирования = РезультатыТестирования;
		КонецЕсли; 
		Выполнить "ВыполнитьОбработкуОповещения(ОповещениеЗавершить, РезультатыТестирования)";
	КонецЕсли;
				
КонецПроцедуры	

&НаКлиенте
Функция ТекущийИтератор()
	Если ИтераторТестирования = Неопределено
		 ИЛИ ИтераторТестирования.Количество() = 0 Тогда	
		Возврат Неопределено;
	КонецЕсли; 
	Возврат ИтераторТестирования[ИтераторТестирования.ВГраница()];
КонецФункции //

&НаКлиенте
Процедура ДобавитьИтератор(Знач Итератор)
	Если ИтераторТестирования = Неопределено Тогда	
		ИтераторТестирования = Новый Массив;
	КонецЕсли; 
	ИтераторТестирования.Добавить(Итератор);
КонецПроцедуры //

&НаКлиенте
Процедура ОтразитьРезультатыТестирования(РезультатыТестирования, ДополнительныеПараметры = Неопределено) Экспорт
	
	Перем ГенераторОтчетаMXL, Отчет;
...
#Область ПрограмныйИнтерфейис_ПриостановитьПродолжитьТестирование
// Приостановить тестирование. Браузер будет ожидать таймаут или оповещение.
// для запуска следующего теста. TechDebt - проверить сценарий ли это.
//
// Параметры:
//  Таймаут  - Число - значение в секундах с точностью до 1/10.
//
// Возвращаемое значение:
//   ОписаниеОповещения   - пример: ВыполнитьОбработкуОповещения(Оповещение) приведет
//	 к продолжению тестирования.
//
&НаКлиенте
Функция Ждать(Таймаут) Экспорт
	ЭтотИтераторТестирования = ТекущийИтератор();
	
	Если ЭтотИтераторТестирования = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Тестирование не начато!'");
	КонецЕсли;
	Если НЕ ЕстьПоддержкаАсинхронныхВызовов Тогда
		ВызватьИсключение НСтр("ru = 'Не предполагается использование асинхронных вызовов!'");	
	КонецЕсли;	
	Если НЕ ТипЗнч(Таймаут) = Тип("Число") ИЛИ Таймаут < 0.1 Тогда
		ВызватьИсключение НСтр("ru = 'Передано не корректное значение таймаута ожидания, значений должно быть > 0.1!'");
	КонецЕсли;
 
	ПараметрыКонструктора = Новый массив;
	ПараметрыКонструктора.Добавить("ОбойтиТестыДереваВнутр"); //ИмяПроцедуры
	ПараметрыКонструктора.Добавить(ЭтаФорма); //Модуль
	ПараметрыКонструктора.Добавить(Новый Структура("Таймаут,Ключ",Таймаут,Новый УникальныйИдентификатор));  //ДополнительныеПараметры
	
	Попытка
		//вроде как добавлено начаная с платформы 8.3.3, но не уверен.
		ЭтотИтераторТестирования.ОповещениеПродолжить = Новый("ОписаниеОповещения",ПараметрыКонструктора);
	Исключение
		ВызватьИсключение НСтр("ru = 'Платформа не поддерживает оповещения!'");	
	КонецПопытки;
	 
	ЭтаФорма.ПодключитьОбработчикОжидания("ВнутрЖдатьТаймаут",Таймаут,Истина);
	
	Возврат ЭтотИтераторТестирования.ОповещениеПродолжить;	
КонецФункции

&НаКлиенте
Процедура ВнутрЖдатьТаймаут() Экспорт
	ВывестиСообщениеВЛогФайл(".    -->> Таймаут ожидания продолжения тестирования ");	
	ЭтаФорма.ОтключитьОбработчикОжидания("ПриостановитьЗавершение");
	Если Жду() Тогда
		////через "выполнить" для совместимости с ранними платформами, врод добавлено в 8.3.3
		Выполнить "ВыполнитьОбработкуОповещения(ТекущийИтератор().ОповещениеПродолжить)";	
	КонецЕсли;
КонецПроцедуры
...
Функция Жду() Экспорт
	ЭтотИтераторТестирования = ТекущийИтератор();
	Возврат НЕ ЭтотИтераторТестирования = Неопределено
	        И НЕ ЭтотИтераторТестирования.ОповещениеПродолжить = Неопределено;		

КонецФункции

#КонецОбласти 

#Область Класс_ИтераторДереваТестов
	
#Область КонструкторыДеструкторы

&НаКлиенте
Функция Внутр_ИтераторДереваТестов_Создать()
			
	ИтераторДереваТестов = Новый Структура("_Тип", "ИтераторДереваТестов");//Класс без контекста не имеет смысла	
	
	ИтераторДереваТестов.Вставить("Узел");
	
	ИтераторДереваТестов.Вставить("Загрузчик");
	ИтераторДереваТестов.Вставить("КонтейнерДереваТестов");
	ИтераторДереваТестов.Вставить("Фильтр");
	ИтераторДереваТестов.Вставить("Подписчик");
	ИтераторДереваТестов.Вставить("ИерархияИсполнения"); //наверно не надо
	ИтераторДереваТестов.Вставить("ПараметрыПотоковыхОтчетов");
	ИтераторДереваТестов.Вставить("ОповещениеПродолжить");
	ИтераторДереваТестов.Вставить("Завершить");
	ИтераторДереваТестов.Вставить("Лог");
	
	Возврат ИтераторДереваТестов;
	
КонецФункции

&НаКлиенте
Функция ИтераторДереваТестов_Конструктор(Знач КонтейнерДереваТестов,Знач Фильтр = Неопределено) Экспорт
	
	// Создать объект
	ИтераторДереваТестов = Внутр_ИтераторДереваТестов_Создать();
	
	ИтераторДереваТестов.КонтейнерДереваТестов 	= КонтейнерДереваТестов;
	ИтераторДереваТестов.Фильтр 				= Фильтр;	
		
	// Создать корень итерации
	ИтераторДереваТестов.Узел = Узел_СоздатьКорень(ИтераторДереваТестов, КонтейнерДереваТестов);
		
	Возврат ИтераторДереваТестов;
	
КонецФункции

// КонструкторыДеструкторы 				
#КонецОбласти 

#Область ЭкспортныеМетоды 

&НаКлиенте
Функция ИтераторДереваТестов_Следующий(Знач ИтераторДереваТестов)
	
	Перем УзелНиже,	УзелПравее, УзелВыше;
	
 	УзелНиже = Узел_Ниже(ИтераторДереваТестов.Узел);	
	Если НЕ УзелНиже = Неопределено Тогда
		ИтераторДереваТестов.Узел =  УзелНиже;
		Возврат Истина;
	ИначеЕсли УзелНиже = Неопределено И Узел_ЭтоКорень(ИтераторДереваТестов.Узел) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	УзелПравее = Узел_Правее(ИтераторДереваТестов.Узел);	
	Если НЕ УзелПравее = Неопределено Тогда
		ИтераторДереваТестов.Узел = УзелПравее;
		Возврат Истина;		
	КонецЕсли;
	...
	Пока Истина Цикл
		...
		УзелВыше = Узел_Выше(ИтераторДереваТестов.Узел);		
		Если УзелВыше = Неопределено Тогда Возврат Ложь; КонецЕсли;
		
		ИтераторДереваТестов.Узел = УзелВыше;
				
		УзелПравее = Узел_Правее(ИтераторДереваТестов.Узел);		
		Если НЕ УзелПравее = Неопределено Тогда Прервать;КонецЕсли;
...
&НаКлиенте
Функция Узел_Создать(Знач УзелВыше,Знач Индекс = Неопределено, Знач СтрокиКонтейнера = Неопределено)
	
	Если НЕ СтрокиКонтейнера = Неопределено Тогда
		УзелВыше.Вставить("СтрокиКонтейнера",СтрокиКонтейнера);	
	КонецЕсли;

	рез = Узел_СтруктураУзла();
	рез.Контейнер = УзелВыше.СтрокиКонтейнера[?(Индекс= Неопределено,0,Индекс)];	
	рез.Индекс = Индекс;	
	рез.Выше = УзелВыше;	
	рез.Итератор = УзелВыше.Итератор;
	рез.Прервать = Ложь;
	
	Если рез.Контейнер.Тип = Объект.ТипыУзловДереваТестов.Контейнер Тогда	
		рез.Вставить("Родитель", Новый Структура("Родитель, Ключ, Тип, Имя",УзелВыше.Родитель));	
		ЗаполнитьЗначенияСвойств(рез.Родитель, рез.Контейнер, "Ключ, Тип, Имя");
		
		рез.Вставить("ФильтрДляДочернихУзлов");	
		Если Не УзелДереваТестовУдовлетворяетФильтру(рез.Контейнер, рез.Выше.ФильтрДляДочернихУзлов) Тогда
			рез.ФильтрДляДочернихУзлов = рез.Выше.ФильтрДляДочернихУзлов;
		КонецЕсли;
			
	Иначе
		рез.Вставить("Родитель", УзелВыше.Родитель);
	КонецЕсли;
...
&НаКлиенте
Функция Узел_СоздатьКорень(Знач Итератор, Знач Контейнер)
	
	рез = Узел_СтруктураУзла();	
	рез.Итератор = Итератор;
	рез.Вставить("Ниже");
	рез.Вставить("ФильтрДляДочернихУзлов", Итератор.Фильтр);	
	
	массив = новый массив;
	массив.Добавить(Контейнер);
	рез.Ниже = Узел_Создать(рез,Неопределено,массив);
		
	Итератор.Лог = Новый массив;
...
&НаКлиенте
Функция Узел_ЭтоКорень(Узел)
	Возврат Узел.Выше = Неопределено;	
КонецФункции

&НаКлиенте
Функция Узел_Выше(Узел)
...
&НаКлиенте
Функция Узел_Ниже(Узел)
	
	Если Узел.Свойство("Ниже") Тогда
		//Для корня
		Возврат Узел.Ниже;	
	КонецЕсли; 
	
	Если Узел.Контейнер.Тип = Объект.ТипыУзловДереваТестов.Контейнер Тогда
		
		Узел.РезультатТестирования = СоздатьКонтейнерРезультатовТестирования(Узел.Контейнер);
		
		//Узел.Итератор.Лог.Добавить(СтрШаблон_("Контейнер: %1 (%2) (%3), Ф: %4", Узел.Контейнер.Имя,Узел.Контейнер.Путь,Узел.Контейнер.Ключ,Узел.Итератор.Фильтр[0]));

		ВывестиСообщениеВЛогФайл("");
		ВывестиСообщениеВЛогФайл(".  ==>> набор тестов " + Узел.Контейнер.Имя);

		Если Узел.Контейнер.Путь <> "" Тогда
			Если Узел.Контейнер.Путь <> Узел.Выше.Контейнер.Путь Тогда
				Узел.Вставить("КонтекстВыполненияДляКонтейнера", Узел_ПолучитьКонтекстВыполнения(Узел));
			Иначе
				Узел.Вставить("КонтекстВыполненияДляКонтейнера", Узел.Выше.КонтекстВыполненияДляКонтейнера);
			КонецЕсли;
		КонецЕсли;
		
		Если Узел.Контейнер.СлучайныйПорядокВыполнения Тогда
			СтрокиКонтейнера = ПеремешатьМассив(Узел.Контейнер.Строки);
		Иначе
			СтрокиКонтейнера = Узел.Контейнер.Строки;
		КонецЕсли;
		
		Узел.Вставить("УзлыНиже", Новый массив);
		Для инд = 0 По СтрокиКонтейнера.ВГраница() Цикл
			Узел.УзлыНиже.Добавить(Узел_Создать(Узел,инд,СтрокиКонтейнера));
		КонецЦикла; 
		
		Если Узел.УзлыНиже.Количество() > 0 Тогда
			Возврат Узел.УзлыНиже[0];
		КонецЕсли; 
...
&НаКлиенте
Функция Узел_Правее(Узел)
	
	Если Узел_ЭтоКорень(Узел) Тогда

		Возврат Неопределено;		
	
	ИначеЕсли Узел_ЭтоКорень(Узел.Выше) Тогда
		
		Возврат Неопределено;
		 
	ИначеЕсли Узел.Выше.УзлыНиже.ВГраница() > Узел.Индекс Тогда
	
		УзелПравее = Узел.Выше.УзлыНиже[Узел.Индекс+1];		
		
	Иначе
		
		Внутр_Узел_ПриОкончанииОбработкиГруппы(Узел.Выше);		
...
&НаКлиенте
Функция Узел_СтруктураУзла()
	Возврат новый Структура("Контейнер,Индекс,Выше,Итератор,РезультатТестирования,Родитель,Прервать");
КонецФункции

&НаКлиенте
Процедура Узел_ПриОкончанииОбработкиЭлемента(Знач ОбработанныйУзел)
		
	Внутр_Узел_ЗаполнитьРезультатТестированияРодителя(ОбработанныйУзел);
	
КонецПроцедуры

&НаКлиенте
Функция  Узел_ПолучитьКонтекстВыполнения(Знач Узел)
	
	лКонтекстВыполнения = ПолучитьКонтекстВыполненияПоПути(Узел.Итератор.Загрузчик, Узел.Контейнер.Путь);
	
	// Заполняем служебные поля, если они имеются
	Если ПеременнаяСодержитСвойство(лКонтекстВыполнения, "ИспользуемыйПрефиксПодсистемы") Тогда
		лКонтекстВыполнения.ИспользуемыйПрефиксПодсистемы = Объект.ИспользуемыйПрефиксПодсистемы;
	КонецЕсли;
	
	Возврат лКонтекстВыполнения;

КонецФункции

&НаКлиенте
Процедура Внутр_Узел_ПриОкончанииОбработкиГруппы(Знач Узел)
	
	Если Узел.РезультатТестирования.Строки.Количество() = 0 Тогда
		Узел.РезультатТестирования = Неопределено;
	Иначе
		
		Внутр_Узел_ЗаполнитьРезультатТестированияРодителя(Узел);
		//Если контейнер НЕ ПродолжитьВыполнениеПослеПаденияТеста, обработка тес методов была рервана из-за ошибки, и деструктор не отработал, то запустить деструктор
		Если Не Узел.Контейнер.СлучайныйПорядокВыполнения И Узел.Контейнер.Путь <> "" И Узел.РезультатТестирования.Состояние <> Объект.СостоянияТестов.Пройден И Узел.Прервать Тогда
			
			ЭлементДеструктор = Узел.Контейнер.ЭлементДеструктор;
			
			УзелДеструктор = Неопределено;
			Для каждого врУзел Из Узел.УзлыНиже Цикл
				Если врУзел.Контейнер.Ключ = ЭлементДеструктор.Ключ Тогда
					УзелДеструктор = врУзел;
					ДочернийРезультатТестирования = врУзел.РезультатТестирования;
					Прервать;
				КонецЕсли; 
			КонецЦикла;

			Если УзелДеструктор <> Неопределено И ДочернийРезультатТестирования = Неопределено Тогда
				...
				Попытка
					ВыполнитьНеобязательнуюПроцедуруТестовогоСлучая(Узел.КонтекстВыполненияДляКонтейнера, ЭлементДеструктор.ИмяМетода);
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
					ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
				...
				Узел.РезультатТестирования.Строки.Добавить(НовыйДочернийРезультатТестирования);
				Узел.РезультатТестирования.Состояние = ВыбратьХудшееСостояниеВыполненияТестов(Узел.РезультатТестирования.Состояние, НовыйДочернийРезультатТестирования.Состояние);
				ЗаполнитьДанныеСтатистики(Узел.РезультатТестирования, НовыйДочернийРезультатТестирования);
...
&НаКлиенте
Процедура Внутр_Узел_ЗаполнитьРезультатТестированияРодителя(Знач ОбработанныйУзел)
	
	Перем УзелКонтейнер;
	
	Если ОбработанныйУзел.Выше = Неопределено Тогда Возврат; КонецЕсли;		
	
	//Заполнить результыты тестирования
	УзелКонтейнер = ОбработанныйУзел.Выше; 
	
	Если НЕ Узел_ЭтоКорень(УзелКонтейнер) И ОбработанныйУзел.РезультатТестирования <> Неопределено Тогда
		УзелКонтейнер.РезультатТестирования.Строки.Добавить(ОбработанныйУзел.РезультатТестирования);
		УзелКонтейнер.РезультатТестирования.Состояние = ВыбратьХудшееСостояниеВыполненияТестов(УзелКонтейнер.РезультатТестирования.Состояние, ОбработанныйУзел.РезультатТестирования.Состояние);
		ЗаполнитьДанныеСтатистики(УзелКонтейнер.РезультатТестирования, ОбработанныйУзел.РезультатТестирования);
		Если Не УзелКонтейнер.Контейнер.ПродолжитьВыполнениеПослеПаденияТеста И ОбработанныйУзел.РезультатТестирования.Состояние <> Объект.СостоянияТестов.Пройден Тогда
			УзелКонтейнер.Прервать = Истина;
		КонецЕсли;
	КонецЕсли;
