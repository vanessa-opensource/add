...
Процедура ВыполнитьШагПакетногоЗапуска(Знач РезультатПредыдущегоШага, Знач Состояние) Экспорт
	...
	ОбработкаОповещения = АСинк().смв_ОписаниеОповещения("ВыполнитьШагПакетногоЗапуска", ЭтаФорма, Состояние, "ВыполнитьШагПакетногоЗапускаИсключение", ЭтаФорма);
...
Процедура ДиспетчерЭкспортаОтчетовАсинхронныйПакетныйРежим(Знач РезультатЭкспорта, Знач ПараметрыДиспетчера) Экспорт
	...
	Если ПараметрыДиспетчера.ИндексВидаОтчета < СписокОтчетов.Количество() Тогда
		...
		ОбработчикЭкспорта = АСинк().смв_НовыОписаниеОповещения("ДиспетчерЭкспортаОтчетовАсинхронныйПакетныйРежим", ЭтаФорма, ПараметрыДиспетчера,
...
Процедура ОткрытьИнструмент(Знач ИмяИнструмента, Знач ПутьИнструмента = "", Знач ИмяФормы = "Форма")
	...
	Иначе
		...
		ОписаниеОповещения = АСинк().смв_НовыйОписаниеОповещения("ОткрытьИнструментПроверкаСуществованияКонец", ЭтаФорма,
...
Процедура ОткрытьИнструментПроверкаСуществованияКонец(Существует, ДополнительныеПараметры) Экспорт
	...
	Оповещение = АСинк().смв_НовыйОписаниеОповещения("ОткрытьИнструментЗавершение", ЭтаФорма, Новый Структура("ИмяИнструмента, ИмяФормыИнструмента", ИмяИнструмента, ИмяФормы));
