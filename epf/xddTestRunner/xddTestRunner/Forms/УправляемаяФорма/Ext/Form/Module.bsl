...
Функция ВыполнитьИнициализациюКонтекста(КонтекстВыполнения, РезультатТестирования)
	...
	ПараметрыИнициализации.Добавить(ЭтаФорма);
...
Функция ПутьФайлаНастроекПредставление()
	...
	Если НЕ ПустаяСтрока(ЭтаФорма.Объект.ПутьФайлаНастроек) Тогда
		сПредставление = СтрШаблон("%1: %2","Настройки загружены из файла", ЭтаФорма.Объект.ПутьФайлаНастроек);
	КонецЕсли;
...
Процедура ОбновитьДекорацияНастройкиЗагружены()
	...
	ЭтаФорма.Элементы.ДекорацияНастройкиЗагружены.Заголовок = ПутьФайлаНастроекПредставление();
	Если ЭтаФорма.Элементы.Найти("ДеревоТестов") <> Неопределено Тогда
		ЭтаФорма.ТекущийЭлемент = ЭтаФорма.Элементы.ДеревоТестов;
	КонецЕсли;
