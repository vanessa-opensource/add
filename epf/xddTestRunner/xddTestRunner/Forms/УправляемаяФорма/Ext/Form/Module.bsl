...
Процедура ЗагрузитьПлагины()
	...
	Иначе
		...
		ОбработчикПоискаКлиентскихПлагинов = АСинк().смв_НовыйОписаниеОповещения(
			"ОбработчикПоискаКлиентскихПлагинов", ЭтаФорма, ДополнительныеПараметры);

		АСинк().смв_НайтиФайлы(ОбработчикПоискаКлиентскихПлагинов,
										ПолучитьПутьКПлагинам(), "*.epf", Ложь);
...
Процедура ОбработчикПоискаКлиентскихПлагинов(Знач НайденныеФайлы, Знач ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("НайденныеФайлыПлагинов", НайденныеФайлы);
	
	ОбработчикПоиска = АСинк().смв_НовыйОписаниеОповещения(
		"ОбработчикПоискаКлиентскихПлагиновИСоздателей", ЭтаФорма, ДополнительныеПараметры);

	АСинк().смв_НайтиФайлы(ОбработчикПоиска, ПолучитьПутьКСоздателям(), "*.epf", Ложь);

КонецПроцедуры

&НаКлиенте
Процедура ОбработчикПоискаКлиентскихПлагиновИСоздателей(Знач НайденныеФайлыСоздателей, Знач ДополнительныеПараметры) Экспорт

	НайденныеФайлы = Новый Массив;
	
	НайденныеФайлыПлагинов = ДополнительныеПараметры.НайденныеФайлыПлагинов;
	Для Каждого Файл Из НайденныеФайлыПлагинов Цикл
		НайденныеФайлы.Добавить(Файл);
	КонецЦикла;

	Для Каждого Файл Из НайденныеФайлыСоздателей Цикл
		НайденныеФайлы.Добавить(Файл);
	КонецЦикла;
...
Процедура ЗагрузкаПлагиновЗавершение() Экспорт
	...
	ОбработатьСобытиеЗагрузкаВсехПлагинов();
...
&НаКлиенте
Процедура ОбработатьСобытиеЗагрузкаВсехПлагинов()

	Для Каждого КлючЗначение Из Объект.Плагины Цикл
		
		ОбъектПлагин = Плагин(КлючЗначение.Ключ);
		
		Попытка
			ОбъектПлагин.ПослеЗагрузкиВсехПлагинов(ЭтаФорма);
		Исключение
			Инфо = ИнформацияОбОшибке();
			Если (Инфо.Описание = "Метод объекта не обнаружен (ПослеЗагрузкиВсехПлагинов)" Или
				Найти(Инфо.Описание, "Object method not found (ПослеЗагрузкиВсехПлагинов)") > 0) И
				Найти(Инфо.ИсходнаяСтрока, "ОбъектПлагин.ПослеЗагрузкиВсехПлагинов(ЭтаФорма);") > 0 И
...
Функция ВыполнитьТестовыйМетодНаКлиенте(Знач КонтекстВыполнения, Знач ЭлементДереваТестов) Экспорт
	...
	Если ВыполнитьИнициализациюКонтекста(КонтекстВыполнения, РезультатТестирования) Тогда
		...
		Исключение
			...
			РезультатТестирования.ПодробноеСообщение = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
...
&НаКлиенте
Функция ПолучитьПутьКСоздателям()
	// Для встроенного в состав конфигурации браузера тестов
	// используемое имя файла не используется
	Если НЕ Объект.ЭтоВстроеннаяОбработка Тогда
		ФайлЯдра = Новый Файл(ЭтаФорма["ИспользуемоеИмяФайла"]);
		Результат = ФайлЯдра.Путь + "plugins/creators";
	КонецЕсли;
