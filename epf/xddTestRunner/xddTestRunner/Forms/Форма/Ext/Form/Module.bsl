...
Процедура ОбновитьИменаИПредставлениеТестов(Знач КонтейнерДереваТестовЗагрузчика)
	...
	КонтейнерДереваТестовЗагрузчика.Имя = ПредставлениеТеста(КонтейнерДереваТестовЗагрузчика.Имя);

	Для каждого ЭлементКоллекции Из КонтейнерДереваТестовЗагрузчика.Строки Цикл
		Если ЭлементКоллекции.Тип = ЭтотОбъект.ТипыУзловДереваТестов.Контейнер Тогда
			ОбновитьИменаИПредставлениеТестов(ЭлементКоллекции);
		ИначеЕсли ЭлементКоллекции.Тип = ЭтотОбъект.ТипыУзловДереваТестов.Элемент Тогда
			ЭлементКоллекции.Представление = ПредставлениеТеста(ЭлементКоллекции.Представление);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Если ФлагОтладки Тогда
		...
		Для Каждого КлючЗначение Из ПараметрыЗапуска Цикл
			...
			Если ТипЗнч(Значение) = Тип("ФиксированныйМассив") Тогда
				Сообщить("Ключ <" + КлючЗначение.Ключ + ">, количество = <" + Значение.Количество() + ">");
				Для каждого Элемент Из Значение  Цикл
					Сообщить("  Значение <" + Элемент + ">");
				КонецЦикла;
			Иначе
				Сообщить("Ключ <" + КлючЗначение.Ключ + ">, Значение = <" + Значение + ">");
			КонецЕсли;
		КонецЦикла;
		Сообщить("");
	КонецЕсли;	
