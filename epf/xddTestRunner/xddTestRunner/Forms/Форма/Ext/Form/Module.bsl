...
Процедура ВыполнитьПакетныйЗапуск(Знач ПараметрЗапуска)
	...
	Если ФлагОтладки Тогда
		...
		Для Каждого КлючЗначение Из ПараметрыЗапуска Цикл // TODO удалить отладку
...
Процедура ПервичнаяНастройка()
	...
	Сообщить(СтрШаблон_("КаталогПроекта = <%1>", КаталогПроекта));
	...
	Для Каждого КлючЗначение Из ПлагинНастроек.ПолучитьНастройки() Цикл
		...
		Настройки.Вставить(КлючЗначение.Ключ, НовоеЗначение);
	КонецЦикла;
	
	ФлагОтладки = ПлагинНастроек.ПолучитьНастройку("Отладка") = Истина;
	ВывестиСообщение(СтрШаблон_("Флаг отладки = <%1>", ФлагОтладки));
...
Функция Заменить_workspaceRoot_на_РабочийКаталогПроекта(Знач ИсходнаяСтрока)
	Возврат СтрЗаменить(ИсходнаяСтрока, "$workspaceRoot", КаталогПроекта);
КонецФункции
