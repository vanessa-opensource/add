...
Процедура ВывестиСообщениеВЛогФайл(Знач Сообщение) Экспорт
	...
	Если ВыводитьЛогВыполненияСценариевВТекстовыйФайл И ЗначениеЗаполнено(ИмяФайлаЛогВыполненияСценариев) Тогда
		...
		ПлагинВыводВЛогФайл.ПутьФайла = ИмяФайлаЛогВыполненияСценариев;
		ПлагинВыводВЛогФайл.Вывести(Сообщение);
		
		// не понятная логика, в форме аналогичный метод не проверяет флаг "открыт", но хотя там данный флаг взводится вызовом ПодготовитьВыводВЛогФайл
		// а тут нет ПодготовитьВыводВЛогФайл, но зато есть проверка на ПлагинВыводВЛогФайл.Открыт(), получается по такой логике в файл записи не будут попадать.
		// да вообще я не уловил смысла проверять ПлагинВыводВЛогФайл.Открыт, в любом случае какой смысл часть логов писать в файл, а часть в сообщить.
		// если хотели избежать ошибки "ошибка совместного доступа к файлу" то нужно реализовать своего рода мьютекс, что бы именно ожидалось когда ресурс будет отпущен
		// но для этого нужен общее пространство в пямяти, проще сделать цикл с попыткой при записи файла. 
		// 
