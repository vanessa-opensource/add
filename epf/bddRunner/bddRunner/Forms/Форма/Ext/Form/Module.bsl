...
Процедура ЗапускВРежимеКоманднойСтроки()
	...
	Если StartFeaturePlayer = Истина Тогда
		...
		Если ЗнВыгружатьСтатусВыполненияСценариевВФайл = "Истина" или ЗнВыгружатьСтатусВыполненияСценариевВФайл = "Да" Тогда
...
Процедура ПолучитьУжеСуществующиеСценарииИзФич(Знач КаталогФич,ДополнительныеПараметры, МассивОбработанныхФайлов)
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		Попытка
			...
			КонецЦикла;
		Исключение
				//загрузить фичу из файла не удалось... ну бывает.
		КонецПопытки;
		Текст.Закрыть();
