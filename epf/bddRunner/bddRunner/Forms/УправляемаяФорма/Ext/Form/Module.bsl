...
Процедура Шаг(Стр, ВыполнятьЧерезВременнуюФичу = Ложь) Экспорт
	...
	Иначе
		ДвДанныеvbFeatureReader = Новый ДвоичныеДанные(ПолучитьПутьКFeatureReader());//TODO потенциальные пожиратели времени
...
Функция ПолучитьТабДокИзTestClientКакМассив(ЭлементФормы,ДопПараметры = Неопределено) Экспорт
	...
	Если НЕ ТекущийTestclientЭтоWebКлиент И ВерсияПриложенияБольшеИлиРавнаЧемЗаданная(СистемнаяИнформация.ВерсияПриложения, "8.3.8.0") Тогда
		...
		Пока Истина Цикл
			...
			Попытка
				ФайлМакета = Новый ДвоичныеДанные(ИмяВременногоФайла);//TODO потенциальный пожиратель времени
				Прервать;
			Исключение
...
Функция НайтиМакетВКаталогеПроекта(ИмяМакета) Экспорт
	...
	ДвоичныеДанные = Новый ДвоичныеДанные(ИмяФайла);//TODO отключить передачу файла на сервер, если используются прямые пути
...
Функция ПолучитьМассивФичаФайловИзКаталогаСогласноНастроек(КаталогСФичами) Экспорт
	...
	Для Каждого ЭлементСпискаФайлов Из СписокФайлов Цикл
		...
		Если НРег(Прав(ИмяФайлаИзСписка,8)) <> ".feature" Тогда
			Продолжить;
		КонецЕсли;

		ДвоичныеДанные = ПутьФайлаИлиДвоичныеДанныеДляСервера(ИмяФайлаИзСписка);

		СтруктураИзСписка = Новый Структура("Каталог,Фича,Уровень,Имя,ПолныйПуть,ДвоичныеДанные", Ложь, Истина, 2,
			ЭлементСпискаФайлов.Значение.Имя, ИмяФайлаИзСписка, ДвоичныеДанные);
...
Функция ПрочитатьСтруктуруИзJSONФайлаКлиент(VBParams) Экспорт
	...
	VBParamsДвоичныеДанные = Новый ДвоичныеДанные(VBParams);//TODO потенциальный пожиратель времени
...
Процедура ОтчетАллюр(РезультатПрохожденияТестовСценария)
	...
	ДвоичныеДанные = Новый ДвоичныеДанные(Объект.КаталогИнструментов + "/vendor/allure-framework/allure.xsd");//TODO потенциальный пожиратель времени
...
Функция ЗагрузитьТелоМодуляВТаблицуЗначений(ПутьКФайлу)
	...
	ДвоичныеДанные = Новый ДвоичныеДанные(ПутьКФайлу);//TODO потенциальный пожиратель времени
...
Процедура СоздатьШаблоныОбработокПродолжение(ДополнительныеПараметры = Неопределено)
	...
	ДвДанныеvbFeatureReader = Новый ДвоичныеДанные(ПолучитьПутьКFeatureReader());//TODO потенциальный пожиратель времени
...
Процедура ОбработатьXMLВUILogToScript(Стр)
	...
	Попытка
		...
		ДвоичныеДанные = Новый ДвоичныеДанные(ПутьКОбработке);//TODO потенциальный пожиратель времени
