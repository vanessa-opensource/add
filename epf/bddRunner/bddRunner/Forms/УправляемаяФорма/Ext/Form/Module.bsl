...
&НаКлиенте
Процедура СброситьКешИзвестныхШаговВНастройках(Команда) Экспорт
	СброситьКешИзвестныхШаговВНастройкахСервер();
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		Обработчик_ПриОткрытииЗавершение = Вычислить("Новый ОписаниеОповещения(""ПриОткрытииПродолжение"", ЭтаФорма)");
		ЗагрузитьПлагины(Ложь, Обработчик_ПриОткрытииЗавершение);
	Иначе
		ЗагрузитьПлагины();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Отладка("захожу в ОбработкаОповещения");
	...
	ИначеЕсли Найти(ИмяСобытия, "ВыполнитьСценарий") > 0 или Найти(ИмяСобытия, "ЗагрузитьСценарий") Тогда
		...
		Отладка("захожу в ОбработкаОповещения - перед ЗагрузитьФичи()");
...
Процедура ВыполнитьСценарии(ТекСценарий = Неопределено, ДопПараметры = Неопределено) Экспорт
	Отладка("захожу в ВыполнитьСценарии");
	...
	Оповестить("ЗапускСценариевVanessaADD");
...
// Получить описание окружения для выполнения и показа в логах
//
// Параметры:
//  СообщениеПрефикс - Строка - доп.расшифровка. По умолчанию - "$Используется окружение$"
// 
// Возвращаемое значение:
//   Строка - текст с описанием окружения
//
&НаКлиенте
Функция ОписаниеОкружения(Знач СообщениеПрефикс = "Используется окружение") Экспорт
	СисИнфо            = Новый СистемнаяИнформация;
	ВерсияПриложения   = СисИнфо.ВерсияПриложения;
	РежимСовместимости = УзнатьРежимСовместимостиКлиент(Локализовать("$Не использовать$"));

	СобираемыеСтроки = Новый Массив;
	СобираемыеСтроки.Добавить(СобратьЛокализованнуюСтроку(СообщениеПрефикс, ""));
	СобираемыеСтроки.Добавить(ПолучитьНаименованиеИВерсиюОбработкиКлиент());
	СобираемыеСтроки.Добавить(СобратьЛокализованнуюСтроку("Версия платформы", ВерсияПриложения));
	СобираемыеСтроки.Добавить(СобратьЛокализованнуюСтроку("Режим совместимости TestManager", РежимСовместимости));
	СобираемыеСтроки.Добавить(СобратьЛокализованнуюСтроку("Есть поддержка асинхронных вызовов", 
		ЕстьПоддержкаАсинхронныхВызовов));
	СобираемыеСтроки.Добавить(СобратьЛокализованнуюСтроку("Строка соединения ИБ", СтрокаСоединенияИнформационнойБазы()));
	СобираемыеСтроки.Добавить(СобратьЛокализованнуюСтроку("Имя исполнителя\компьютера", ИмяКомпьютера()));
	
	Возврат СтрСоединить_(СобираемыеСтроки, Символы.ПС);
КонецФункции

&НаКлиенте
Функция СобратьЛокализованнуюСтроку(Знач СтрокаДляЛокализация, Знач Значение, Знач Разделитель = ":")
	Возврат Локализовать(СтрокаДляЛокализация) + Разделитель + Значение;
КонецФункции
...
//
// Параметры:
//  Строка	 - Строка	 - локализуемая строка\текст
// 
// Возвращаемое значение:
//   Строка - результат локализации
//
&НаКлиенте
Функция Локализовать(Строка) Экспорт
	Значение = ТекстСообщенийПользователю[Строка];
...
Процедура ЗагрузитьФичи() Экспорт
	Отладка("захожу в ЗагрузитьФичи");
...
Процедура ОбработчикПоискаКлиентскихПлагинов(Знач НайденныеФайлы, Знач ДополнительныеПараметры) Экспорт
	Отладка("захожу в ОбработчикПоискаКлиентскихПлагинов");
...
Процедура ОбработчикПоискаКлиентскихПлагиновЗавершение(Знач ПомещенныеФайлы, Знач ДополнительныеПараметры) Экспорт
	Отладка("захожу в ОбработчикПоискаКлиентскихПлагиновЗавершение");
...
Процедура ПодключитьНайденныеКлиентскиеПлагины(Знач ЗагруженныеФайлы, Знач ИдентификаторыПлагинов, ДополнительныеПараметры)
	...
	Отладка("захожу в ПодключитьНайденныеКлиентскиеПлагины");
...
Процедура ЗагрузкаПлагиновЗавершение(Знач ДополнительныеПараметры) Экспорт
	Отладка("захожу в ЗагрузкаПлагиновЗавершение");
...
#Область ЗапускИзКоманднойСтроки

&НаКлиенте
Процедура ПоказатьОкружениеВЛоге()
	ВывестиВЛогФайл(ОписаниеОкружения());	
КонецПроцедуры
...
Процедура ПолучитьУжеСуществующиеШагиИзОбработок(Знач КаталогФич, Знач ДополнительныеПараметры,
	...
	Отладка("захожу в ПолучитьУжеСуществующиеШагиИзОбработок");
...
Процедура ЗаполнитьДеревоФич(СтруктураПараметров)
	Отладка("захожу в ЗаполнитьДеревоФич");
	...
	ИначеЕсли Объект.ИспользоватьПрямыеПутиФайлов Тогда 
		ПереданныеФайлы = ИмитироватьПередачуФайловНаСервер(ОписанияФайловШагов);
	...
	Иначе
		...
		Иначе
			...
			ПоместитьФайлы(ПередаваемыеФайлы, ПереданныеФайлы, , Ложь, ЭтаФорма.УникальныйИдентификатор);
			ОбработчикПодключенияШаговИзПереданныхФайловШагов(ПереданныеФайлы, ПараметрыПродолжения);
		КонецЕсли;
	КонецЕсли;
	// из-за асинхронности никакой код вызывать нельзя
...
&НаКлиенте
Функция ИмитироватьПередачуФайловНаСервер(Знач ОписанияФайловШагов)
	ПереданныеФайлы = Новый Массив;
	Для Каждого ОписаниеФайла Из ОписанияФайловШагов Цикл
		...
		ОписаниеПереданногоФайла.Вставить("Хранение", "");
		ПереданныеФайлы.Добавить(ОписаниеПереданногоФайла);
	КонецЦикла;
	Возврат ПереданныеФайлы;
КонецФункции
...
Процедура ОбработчикПодключенияШаговИзПереданныхФайловШагов(Знач ПереданныеФайлы,
	...
	Отладка("захожу в ОбработчикПодключенияШаговИзПереданныхФайловШагов");
...
Процедура ЗаполнитьДеревоФичПродолжение(ОписанияФайловШагов, КоллекцийПутейФайловКоторыеСовпалиСКеш,
	...
	Отладка("захожу в ЗаполнитьДеревоФичПродолжение");
	//ПодключитьШаги(ОписанияФайловШагов);
...
Процедура ПолучитьУжеСуществующиеСценарииИзФич(Знач КаталогФич, ДополнительныеПараметры,
	...
	Отладка("захожу в ПолучитьУжеСуществующиеСценарииИзФич");
...
Процедура ЗагрузитьФичиПродолжение(ДополнительныеПараметры = Неопределено)
	Отладка("захожу в ЗагрузитьФичиПродолжение");
...
Процедура ЗапускВРежимеКоманднойСтроки()
	Отладка("захожу в ЗапускВРежимеКоманднойСтроки");
	...
	Если ВыполнитьСценарии = Истина Тогда
		Отладка("ЗапускВРежимеКоманднойСтроки - ВыполнитьСценарии = Истина - перед ЗагрузитьФичи");
	...
	ИначеЕсли ВыполнитьСценарииСТекущегоШага = Истина Тогда
		Отладка("ЗапускВРежимеКоманднойСтроки - ВыполнитьСценарииСТекущегоШага = Истина - перед ЗагрузитьФичи");
		...
		ПодключитьОбработчикОжидания("ЗапуститьСценарийСоСтрокиПрисланнойИзвне", 0.1, Истина);
	Иначе
		Отладка("ЗапускВРежимеКоманднойСтроки - Иначе - перед ЗагрузитьФичи");
...
Процедура ПослеОткрытияФормы()
	Отладка("захожу в ПослеОткрытияФормы");
	...
	Если НЕ Объект.ВыполнениеСценарияИзОбработкиОповещения Тогда
		...
		Отладка("ПослеОткрытияФормы - Объект.ЗапускИзКоманднойСтроки - " + Объект.ЗапускИзКоманднойСтроки);
...
&НаКлиенте
Процедура ПриОткрытииПродолжение(Знач НенужноеЗначение = Неопределено, 
								Знач НенужныеДополнительныеПараметры = Неопределено) Экспорт
...
Процедура ПриОткрытииЗавершение() Экспорт
	Отладка("захожу в ПриОткрытииЗавершение");
...
Процедура ПерейтиКВыполнениюСледующегоСценария()
	...
	Отладка("захожу в ПерейтиКВыполнениюСледующегоСценария");
	...
	//Если ЕстьПоддержкаАсинхронныхВызовов И Объект.ВыполнятьШагиАссинхронно Тогда
	Если ЕстьПоддержкаАсинхронныхВызовов И Объект.ВыполнятьШагиАссинхронно Тогда
...
Процедура ОтчетАллюр(РезультатПрохожденияТестовСценария)
	...
	Если Объект.ДелатьСкриншотПриВозникновенииОшибки
		...
		Для Каждого СтруктураСкриншот Из РезультатПрохожденияТестовСценария.МассивСкриншотов Цикл
			...
			СкриншотДвоичныеДанные = СтруктураСкриншот.ДвоичныеДанные;
			Если ТипЗнч(СкриншотДвоичныеДанные) = Тип("ДвоичныеДанные") Тогда
				СтруктураСкриншот.ДвоичныеДанные.Записать(ПутьКФайлу);
			Иначе
				КопироватьФайлКомандаСистемы(СкриншотДвоичныеДанные, ПутьКФайлу);//TODO пожиратель времени
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура АсинхронноВыполнитьСценарий()
	Отладка("захожу в АсинхронноВыполнитьСценарий");
	...
	Если ВсеСценарииВыполнены() Тогда
		Отладка("АсинхронноВыполнитьСценарий - все сценарии выполнены");
...
&НаСервере
Процедура СброситьКешИзвестныхШаговВНастройкахСервер()
...
&НаКлиенте
Процедура СброситьКешИзвестныхШагов() Экспорт
	ТаблицаИзвестныхStepDefinition.Очистить();
	КешStepDefinition.Очистить();
	КешТаблицGherkin.Очистить();
	СброситьКешИзвестныхШаговСервер();
КонецПроцедуры

&НаСервере
Процедура СброситьКешИзвестныхШаговСервер()
...
Процедура ЗагрузитьПлагины(Знач Перезагрузить = Ложь, Знач ОбработчикПослеЗавершенияЗагрузки = Неопределено)
	Отладка("захожу в ЗагрузитьПлагины");
