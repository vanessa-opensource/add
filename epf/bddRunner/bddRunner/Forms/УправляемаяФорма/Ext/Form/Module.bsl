...
Функция ПолучитьТаблицыПоТаблицаИзвестныхStepDefinition()
	...
	Для Каждого СтрТаблицаИзвестныхStepDefinitionСервер Из ТаблицаИзвестныхStepDefinitionСервер Цикл
		МассивВерсийФайловИзТаблицаИзвестныхStepDefinition.Добавить(
				СтрТаблицаИзвестныхStepDefinitionСервер.ВерсияФайла);
...
&НаКлиенте
Процедура УдалитьСтрокиТаблицаИзвестныхStepDefinitionКоторыеНеОбрабатывались(Знач МассивИменФайловКоторыеСовпалиСКеш)
...
Процедура УдалитьСтрокиИз_ТаблицаИзвестныхStepDefinition_ДляКоторыхНетФайловИСтрокиИзмененныхФайлов(ДанныеПоВерсиямОбработок,
	...
	Для Каждого ИмяФайла Из МассивФайлов Цикл
		ЭтоФайлИзКаталогаШтатныхБиблиотек = Найти(ИмяФайла, КаталогИнструментовУниверсальный) <> 0;
		Если ЭтоФайлИзКаталогаШтатныхБиблиотек Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьДеревоФич(СтруктураПараметров)
	...
	УдалитьСтрокиИз_ТаблицаИзвестныхStepDefinition_ДляКоторыхНетФайловИСтрокиИзмененныхФайлов(ДанныеПоВерсиямОбработок,
		ДополнительныеПараметры, МассивФайлов);
	
	МассивИменФайловКоторыеСовпалиСКеш = ПолучитьВсеСуществующиеСнипетыИзОбработок(Путь, КаталогиБиблиотек, 
...
Процедура ПолучитьУжеСуществующиеСценарииИзФич(Знач КаталогФич, ДополнительныеПараметры, МассивУжеОбработанныхаФайлов)
	...
	Для Каждого ФайлОбъект Из МассивФайлов Цикл
		...
		ИмяФайла = УниверсальноеПолноеИмяФайла(ПолноеИмяФайла);
		ИмяФайлаНРег = НРег(ИмяФайла);
		
		Если МассивУжеОбработанныхаФайлов.Найти(ИмяФайлаНРег) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		МассивУжеОбработанныхаФайлов.Добавить(ИмяФайлаНРег);
		...
		Пока Истина Цикл
			...
			Если НашлиТегExportScenarios Тогда
				Если Лев(СтрНРег, 9) = "сценарий:" Тогда
					...
					ТаблицаУжеСуществующихСценариев.Добавить(ПолучитьСтруктуруДляСценария((ИмяСценария), ИмяФайла, ТаблицаШагов, ПолноеИмяФайла, Ложь, ТипШага, Описание, Пример));
				...
				ИначеЕсли Лев(СтрНРег, 19) = "структура сценария:" Тогда
					...
					ТаблицаШагов = Новый Массив;
					ТаблицаУжеСуществующихСценариев.Добавить(ПолучитьСтруктуруДляСценария((ИмяСценария), ИмяФайла, ТаблицаШагов, ПолноеИмяФайла, Истина, ТипШага, Описание, Пример));
...
Процедура ПриОткрытииЗавершение(Знач НенужноеЗначение, Знач ДополнительныеПараметры) Экспорт
	...
	ТаймайтЗагрузкиНастроек = ?(Объект.ПлагиныЗагружены, 0.1, 5);//TODO ?? уменьшить таймаут менее 5 секунд
...
Процедура СоздатьФайлыОбработок(БылиОшибки, СтруктураОписанияEpf, ДополнительныеПараметры, КаталогДляВременныхФайлов)
	...
	Если Не ЗначениеЗаполнено(ВременныйКаталогБазы) Тогда
		ВременныйКаталогБазы = ПолучитьПутьВременногоФайла(КаталогДляВременныхФайлов);
		ДопСообщениеОшибки = "Файл EPF не создан. Не смог создать каталог временной базы.";
		Если Не СоздатьБазу1С(ВременныйКаталогБазы, ДопСообщениеОшибки) Тогда
...
Процедура СделатьGenerateEpf(СтруктураПараметров)
	...
	МассивИменФайловКоторыеСовпалиСКеш = ПолучитьВсеСуществующиеСнипетыИзОбработок(КаталогФич, КаталогиБиблиотек,
		ДанныеПоВерсиямОбработок, ДополнительныеПараметры);
