...
&НаКлиенте
Процедура ДобавитьТекстШагаСравненияТаблиц(Знач ЭлемСтало, Знач Ключ, Стр, ИзмененныеКолонки = Неопределено)
	Изменение = "таблица формы с именем """ + ЭлемСтало.Имя + """ стала равной:";
	ДопСтрока = "таблица """ + ЭлемСтало.Имя + """ содержит строки:";
	...
	Стр = Стр + "# " + КлючевоеСловоДляВставки + ДопСтрока + Символы.ПС;
...
Функция ЗначениеДляТаблицыГеркин(Знач ИсходнаяСтрока)
	Если Найти(ИсходнаяСтрока, """") <> 0 Или Найти(ИсходнаяСтрока, "|") <> 0 Тогда
