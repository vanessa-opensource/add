...
Функция РаспаковатьEPF(СтруктураОписанияEpf)
	...
	Попытка
		...
		// КомандаСистемы = """" + ДополнитьСлешВПуть(Объект.ВерсияПлатформыДляГенерацииEPF) + "1cv8"" DESIGNER /F """ + ВременныйКаталогБазы 
		// 	+ """ /DumpExternalDataProcessorOrReportToFiles  """ + ПутьКИсходникам + """ """ + ИмяФайла 
		// 	+ """ /out """ + ИмяФайлаOUT + """ > """ + ИмяФайлаЛога + """";
		// Отладка(КомандаСистемы);
		// ВыполнитьКомандуОСБезПоказаЧерногоОкна(КомандаСистемы);
		СтрокаЗапуска1С = """" + ДополнитьСлешВПуть(Объект.ВерсияПлатформыДляГенерацииEPF) + "1cv8"" DESIGNER /F """ 
			...
			+ """ """ + ИмяФайла + """";
		ОписаниеЗапуска = Запустить1С(СтрокаЗапуска1С);
...
//
&НаКлиенте
Функция СоздатьБазу1С(Знач КаталогБазы, Знач ДопСообщениеОшибки = "")
	...
	// КомандаСистемы = """" + ДополнитьСлешВПуть(Объект.ВерсияПлатформыДляГенерацииEPF) + "1cv8"" CREATEINFOBASE File="""  
	// + ВременныйКаталогБазы +  """ /out """ + ИмяФайлаOUT + """ > """ + ИмяФайлаЛога + """";
...
//
Функция Запустить1С(Знач СтрокаЗапуска1С)
	Рез = Новый Структура;
	...
	Отладка(КомандаСистемы);
	ВыполнитьКомандуОСБезПоказаЧерногоОкна(КомандаСистемы); //TODO код возврата получить, хотя бы от Конфигуратора??
...
Функция СоздатьСтруктураОписанияEpf(ОтносительныйКаталогФичи, ИмяТекущейФичи, GenerateEpf, 
		ДвДанныеvbFeatureReader, ДополнительныеПараметры, КаталогБазы1С)
...
Процедура СоздатьФайлыОбработок(БылиОшибки, СтруктураОписанияEpf, ДополнительныеПараметры = Неопределено)
	...
	Если ЭтоLinux Тогда
		// СтрокаЗапуска = "" + ДополнитьСлешВПуть(Объект.ВерсияПлатформыДляГенерацииEPF) + "1cv8 DESIGNER /F""" 
		// + ВременныйКаталогБазы + """ /LoadExternalDataProcessorOrReportFromFiles """ + КорневойФайлОбработки 
		// + """ """ + ИмяВременногоEPF + """ /out """ + ИмяФайлаOUT + """ > """ + ИмяФайлаЛога + """";
		// Отладка(СтрокаЗапуска);
		// КомандаСистемы(СтрокаЗапуска);
		СтрокаЗапуска1С = "" + ДополнитьСлешВПуть(Объект.ВерсияПлатформыДляГенерацииEPF) + "1cv8 DESIGNER /F""" 
		+ ВременныйКаталогБазы + """ /LoadExternalDataProcessorOrReportFromFiles """ + КорневойФайлОбработки 
