...
Процедура ОчиститьКешАктивнойТаблицыTestClient() Экспорт
	ОбъектКонтекст.Вставить("АктивнаяТаблицаTestClient",
		Новый Структура("ИмяТаблицы,Значение", Неопределено, Неопределено));
КонецПроцедуры
...
&НаКлиенте
Функция НайтиВсеОкнаТестКлиента(Знач Таймаут = 0) Экспорт
	Таймаут = Макс(Таймаут, Объект.КоличествоСекундПоискаОкна);
	Возврат ОбъектКонтекстСохраняемый.ТестовоеПриложение.НайтиОбъекты(Тип("ТестируемоеОкноКлиентскогоПриложения"),
		, , Таймаут);
КонецФункции

&НаКлиенте
Функция НайтиВсеФормыТестКлиента(Знач ОкноПриложения, Знач Таймаут = 0) Экспорт
	Таймаут = Макс(Таймаут, Объект.КоличествоСекундПоискаОкна);
	Возврат ОкноПриложения.НайтиОбъекты(Тип("ТестируемаяФорма"), , , Таймаут);
КонецФункции

&НаКлиенте
Функция НайтиОкноТестКлиентаПоЗаголовку(Знач ЗаголовокОкна, Знач Таймаут = 0) Экспорт
	Таймаут = Макс(Таймаут, Объект.КоличествоСекундПоискаОкна);
	Возврат ОбъектКонтекстСохраняемый.ТестовоеПриложение.НайтиОбъект(Тип("ТестируемоеОкноКлиентскогоПриложения"),
		ЗаголовокОкна, , Таймаут);
КонецФункции

&НаКлиенте
Функция НайтиОкноVB() Экспорт
	МассивОкон = НайтиВсеОкнаТестКлиента();
	ОкноVB = Неопределено;
...
Функция НайтиРеквизитОткрытойФормыПоЗаголовку(ИмяРеквизита, ИскатьПоИмени = Ложь,
	...
	Нашли = Ложь;
	МассивФорм = НайтиФормыТекущегоОкнаТестКлиента();
...
Функция ПолучитьГлавноеОкноИзТестовоеПриложение() Экспорт
	...
	ОкнаПриложения = НайтиВсеОкнаТестКлиента();
...
&НаКлиенте
Функция НайтиФормыТекущегоОкнаТестКлиента(ДопПараметры = Неопределено) Экспорт
	Если ЗафиксированнаяФормаTestClient <> Неопределено Тогда
		Массив = Новый Массив;
	...
	Если ОкноПриложения = Неопределено Тогда
		...
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	МассивФорм             = НайтиВсеФормыТестКлиента(ОкноПриложения);
	Если ДопПараметры <> Неопределено Тогда
...
//НайтиТекущуюФормуТестКлиента() надо использовать, когда известно, что у текущего окна только одна форма,
// иначе надо использовать НайтиФормыТекущегоОкнаТестКлиента
&НаКлиенте
Функция НайтиТекущуюФормуТестКлиента() Экспорт
	Получилось = Истина;
	Попытка
		ОкноПриложения         = ПолучитьАктивноеОкноИзКонтекста();
...
&НаКлиенте
Функция НаЭкранеДиалогАвторизации(Знач Таймаут = 0) Экспорт
	Если Не ОбъектКонтекстСохраняемый.Свойство("ТестовоеПриложение") Тогда
		Возврат Ложь;
	КонецЕсли;

	Попытка
		НужноеОкно = НайтиОкноТестКлиентаПоЗаголовку("1С:Предприятие. Доступ к информационной базе", Таймаут);
	Исключение
		Возврат Ложь;
	КонецПопытки;

	Возврат НужноеОкно <> Неопределено;
КонецФункции
...
Функция ПолучитьТекстПредупреждения() Экспорт
	...
	Попытка
		ОкнаПредупреждения = НайтиВсеОкнаТестКлиента();
...
Функция ПодключитьУжеЗапущенныйTestClient(ДанныеЗапускаКлиентаТестирования)
	...
	Если НЕ ОбъектКонтекстСохраняемый.Свойство("ТестовоеПриложение") Тогда
		Попытка
			...
			Попытка
				Если НЕ ТестовоеПриложение.ОжидатьОтображениеОбъекта(Тип("ТестируемоеОкноКлиентскогоПриложения"), , ,
						Объект.ТаймаутЗапуска1С) Тогда
					Возврат Ложь;
				КонецЕсли;
			Исключение
				Возврат Ложь;
...
&НаКлиенте
Функция ПолучитьКорректноеИмяТестКлиента(Знач Стр)
	Стр = Транслит(Стр);
	Стр = СтрЗаменить(Стр, " ", "");
	Стр = СтрЗаменить(Стр, ".", "");
	Стр = СтрЗаменить(Стр, "=", "");
...
Функция СоздатьТестовоеПриложение(ДанныеЗапускаКлиентаТестирования)
	...
	Если ДанныеЗапускаКлиентаТестирования.ТипКлиента = "Web" Тогда
		ВтораяЧастьСтроки = ",""" + ПолучитьКорректноеИмяТестКлиента(ДанныеЗапускаКлиентаТестирования.Имя) + """)";
	Иначе
		ВтораяЧастьСтроки = ")";
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьСоединениеТестКлиента(ТестовоеПриложение)
	Если ТипЗнч(ОбъектКонтекстСохраняемый) <> Тип("Структура") Тогда
		ОбъектКонтекстСохраняемый = Новый Структура;
	КонецЕсли;
...
Функция ОткрытьНовыйСеансTestClientИлиПодключитьУжеСуществующийДляНесколькихКлиентов(ДанныеЗапускаКлиентаТестирования,
	...
	Попытка
		...
		УстановитьСоединениеТестКлиента(ТестовоеПриложение);
		...
		НайденДиалогАвторизации = НаЭкранеДиалогАвторизации(1);
		Если НайденДиалогАвторизации Тогда
			ЗакрытьДиалогАвторизации();
			ВызватьИсключение "Не удалось авторизоваться в базе.";
		КонецЕсли;
...
Функция ПолучитьСтрокуЗапускаWebКлиент(ДанныеЗапускаКлиентаТестирования)
	...
	ДобавитьПараметрКWeb(СтрокаЗапуска, "TESTCLIENTID",
		ПолучитьКорректноеИмяТестКлиента(ДанныеЗапускаКлиентаТестирования.Имя));
