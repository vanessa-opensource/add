	...
	Если Объект.ДелатьСкриншотПриВозникновенииОшибки
		...
		Для Каждого СтруктураСкриншот Из РезультатПрохожденияТестовСценария.МассивСкриншотов Цикл
			...
			КопироватьФайлКомандаСистемы(ВременныйФайл,
				ПутьКаталогаРодителяОтчетовАллюрСоСлешем + СтруктураСкриншот.ИмяФайла);
		КонецЦикла;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПутьФайлаПоДвоичнымДаннымСервер(Знач ДвоичныеДанныеИлиПутьФичи, Знач Расширение = "", ИмяВременногоФайла = Неопределено)
	Если ТипЗнч(ДвоичныеДанныеИлиПутьФичи) = Тип("ДвоичныеДанные") Тогда
		Если Расширение = "" Тогда
			Расширение = "feature";
		КонецЕсли;
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла(Расширение);
		ДвоичныеДанныеИлиПутьФичи.Записать(ИмяВременногоФайла);//TODO подумать об ускорении через Потоки или другой алгоритм
	КонецЕсли;
