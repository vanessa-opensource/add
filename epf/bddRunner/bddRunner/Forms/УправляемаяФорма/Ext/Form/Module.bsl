...
//НайтиТекущуюФормуTestClient() надо использовать, когда известно, что у текущего окна только одна форма,
...
Функция НаЭкранеДиалогАвторизации() Экспорт
	...
	Попытка
		НужноеОкно = НайтиОкноТестКлиентаПоЗаголовку("1С:Предприятие. Доступ к информационной базе");
	Исключение
		Возврат Ложь;
	КонецПопытки;
