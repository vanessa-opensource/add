...
&НаКлиенте
Процедура АдресUserSettingsПриИзменении(Элемент)
	ЗагрузитьПеременныеКонтекстаИзФайлаНастроек();
КонецПроцедуры
...
Процедура ЗагрузитьПеременныеКонтекстаИзФайлаНастроек()
	...
	Попытка
		ЗагрузчикНастроек.ЗагрузитьНастройки(Объект.ПоставщикUserSettings, Объект.АдресUserSettings);
	Исключение
...
&НаКлиенте
Процедура ЗаполнитьПоставщикаUserSettingsЕслиОнПустой()
	
	ПоставщикUserSettings = Объект.ПоставщикUserSettings;
	Если ПустаяСтрока(ПоставщикUserSettings)Тогда
		ЗагрузчикНастроек = Плагин("ЗагрузчикПользовательскихНастроек");
		Объект.ПоставщикUserSettings = ЗагрузчикНастроек.ИмяПоставщикаFILE();
	КонецЕсли;
...
Процедура ПослеОткрытияФормы()
	...
	Если НЕ Объект.ВыполнениеСценарияИзОбработкиОповещения Тогда
		...
		ЗаполнитьПоставщикаUserSettingsЕслиОнПустой();
		ЗагрузитьПеременныеКонтекстаИзФайлаНастроек();
