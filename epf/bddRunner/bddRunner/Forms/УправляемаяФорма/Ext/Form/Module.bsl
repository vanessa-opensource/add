...
Процедура ОбработатьСтатусСценария(ИД)
	...
	УстановитьСтатусРодителей(РезультатПрохожденияТестовШагов, ИДСтрокиСценария);
	//ИерархияРодителей = Новый Массив;
	//
	//Родители = Новый Соответствие;
	//Для каждого СтрШаг Из РезультатПрохожденияТестовШагов Цикл
	//	Родитель = МассивСтрокДереваДанныеФормы[СтрШаг.ИДСтроки].ПолучитьРодителя();
	//	ИДСтрокиРодителя = Родитель.ИДСтроки;
	//	Родители.Вставить(ИДСтрокиРодителя, Родитель);
	//КонецЦикла;
	//ИерархияРодителей.Добавить(Родители);
	////ДобавитьИерархиюРодителей(ИерархияРодителей, СтрокаСценария, ИДСтрокиСценария);
...
&НаКлиенте
Процедура УстановитьСтатусРодителей(КоллекцияСтрок, ИДВерхегоРодителя)
	...
	Для Каждого ОчереднаяСтрока Из КоллекцияСтрок Цикл
		...
		КонецЕсли;
	КонецЦикла;
	УстановитьСтатусРодителей(Родители, ИДВерхегоРодителя);
КонецПроцедуры
...
Функция ПолучитьЗначениеДляПередачиВМетод(Знач Значение, Знач Тип)
	...
	ИначеЕсли Тип = "Число" Тогда
		Если СтрДлина(Значение) > 1
			И Лев(Значение, 1) = "0" Тогда // TODO заменить в парсере фич - строки 0001 должны определяться как тип "Строка", а не как тип "Число"
			Возврат """" + Значение + """";
		КонецЕсли;
		Возврат Значение;
