...
Функция ПолучитьАбсолютныйПутьФайлаИзКаталогаПроекта(Знач Путь) Экспорт
  ...
  Если Найти(Путь, "$workspaceRoot") > 0 Тогда
    Путь = СтрЗаменить(Путь, "$workspaceRoot", Ванесса.ДополнитьСлешВПуть(Объект.КаталогПроекта));
  ИначеЕсли Найти(Путь, "$instrumentsRoot") > 0 И НЕ ПустаяСтрока(Объект.КаталогИнструментов) Тогда
    Путь = СтрЗаменить(Путь, "$instrumentsRoot", Ванесса.ДополнитьСлешВПуть(Объект.КаталогИнструментов));
  Иначе
    Путь = Ванесса.ДополнитьСлешВПуть(Объект.КаталогПроекта) + Путь;
  КонецЕсли;
