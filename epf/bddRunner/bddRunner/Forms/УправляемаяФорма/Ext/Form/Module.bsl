...
Процедура ОбработчикНачатьПроверкуСуществования(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Существует Тогда
		...
		АСинк().смв_ЭтоКаталог(ОписаниеОповещения);
...
Процедура ОткрытьИнструмент(Знач ИмяИнструмента, Знач ПутьИнструмента = "", Знач ИмяФормы = "Форма")
	...
	Иначе
		...
		// Этот метод пока не реализован - проблема с обработчиком завершения
		АСинк().смв_ПоместитьФайл(Оповещение, АдресХранилища, ФайлИнструмента.ПолноеИмя , Ложь, ЭтаФорма.УникальныйИдентификатор);
