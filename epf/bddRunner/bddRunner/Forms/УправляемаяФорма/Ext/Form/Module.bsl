...
Функция ПолучитьТекстСообщения(Знач Сообщение)
	...
	ТекстСообщения = Строка(ТекущаяДатаСеанса()) + " " + Сообщение;
...
Функция ПолучитьОтметкуВремениДляИмениФайла()
	ТекДат     = Формат(ТекущаяДатаСеанса(), "ДФ=yyyyMMdd");
...
Процедура УстановитьКонектСTestClient(ТестовоеПриложение)
	...
	МаксДата = ТекущаяДатаСеанса() + Объект.ТаймаутЗапуска1С;
	Пока Истина Цикл
		Если ТекущаяДатаСеанса() > МаксДата Тогда
