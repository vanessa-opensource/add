...
Процедура ПодключитьШагиНаСервере(Знач ОписанияФайловШагов, Знач МассивВажныхСообщений) 
	...
	Для каждого ОписаниеФайлаШага Из ОписанияФайловШагов Цикл
		Файл = Новый Файл(ОписаниеФайлаШага.ПолноеИмя);
		ВерсияФайла = ОписаниеФайлаШага.ВерсияФайла;
		...
		Если КоллекцияШагов.Количество() = 0 Тогда
			...
			ДобавитьПустойСнипетВТаблицуИзвестныхStepDefinition(Файл.ПолноеИмя, ВерсияФайла);
		Иначе
			ДобавитьСнипетыСервер(КоллекцияШагов, Файл.ПолноеИмя, ВерсияФайла, БылиОшибки, МассивВажныхСообщений);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
