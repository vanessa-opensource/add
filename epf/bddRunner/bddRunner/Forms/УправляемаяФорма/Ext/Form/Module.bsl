...
Процедура ПереместитьФайлКомандаСистемы(Знач Откуда, Знач Куда) Экспорт
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		...
		Иначе
			...
			Откуда = СтрЗаменить(Откуда, "/", "\");
			Куда   = СтрЗаменить(Куда, "/", "\");
			ВыполнитьКомандуОСБезПоказаЧерногоОкна("MOVE """ + Откуда + """ """ + Куда + """", Истина, Ложь);
		КонецЕсли;
	Иначе
...
Процедура НайтиФайлыКомандаСистемы(Путь, СписокКаталогов, СписокФайлов, ИскатьВПодкаталогах) Экспорт
	...
	Иначе
		...
		Иначе
			...
			ВыполнитьКомандуОСБезПоказаЧерногоОкна(КомандаКаталоги, Истина, Истина);//TODO проверить быстрое использование ВыполнитьКомандуОСБезПоказаЧерногоОкна(КомандаКаталоги, Истина, Ложь);
		...
		Иначе
			...
			ВыполнитьКомандуОСБезПоказаЧерногоОкна(КомандаФайлы, Истина, Истина);//TODO проверить быстрое использование ВыполнитьКомандуОСБезПоказаЧерногоОкна(КомандаФайлы, Истина, Ложь);
		КонецЕсли;
...
Функция ФайлСуществуетКомандаСистемы(Знач ИмяФайла, ДопПараметры = Неопределено) Экспорт
	Если ЕстьПоддержкаАсинхронныхВызовов Тогда
		...
		Если НЕ ЭтоLinux Тогда
			ИмяФайла = СтрЗаменить(ИмяФайла, "/", "\");
			ВыполнитьКомандуОСБезПоказаЧерногоОкна(
			"(If Exist """ + ИмяФайла + """ (Echo yes) Else (Echo no)) > """ + ИмяВременногоЛог + """",
			Истина, Истина);//TODO проверить запуск
		Иначе
