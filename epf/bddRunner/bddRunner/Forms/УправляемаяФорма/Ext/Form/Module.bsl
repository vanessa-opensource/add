...
Функция ПолучитьЗначениеИзКонтекстаЕслиЭтоВозможно(Знач Значение) Экспорт
	...
	Если (Лев(Значение, 1) = "$") Тогда
		...
		ИначеЕсли Прав(Значение, 1) = "$" Тогда
			...
			ПредполагаемоеЗначение = Неопределено;
			Если ОбъектКонтекст.Свойство(ИмяПеременной, ПредполагаемоеЗначение) Тогда
				Значение = ПредполагаемоеЗначение;
			Иначе
...
Процедура ПодготовитьСценарийКВыполнению() Экспорт
	...
	Если ЗначениеЗаполнено(Объект.КаталогПроекта) Тогда
		КаталогВременногоFeature = Объект.КаталогПроекта + Разделитель + "features";
		//TODO переделать на одну команду проверки и создания каталога
		Если ФайлСуществуетКомандаСистемы(КаталогВременногоFeature) Тогда
	...
	Иначе
		...
		КаталогВременногоFeature = ВременныйФайл.Путь;
	КонецЕсли;

	ИмяВременногоFeature = КаталогВременногоFeature + Разделитель + "vanessa-add-temp.feature";
...
&НаКлиенте
Функция ПодключитьКонтекстОбработкиКлиент(Знач Файл, ВерсияФайла = Неопределено)
...
Процедура АсинхронноПодключитьКонтекстОбработки(Знач ФайлШагов, Знач УниверсальноеПолноеИмяФайлаШага,
	...
	ПараметрыПодключенияВнешнегоФайла = Новый Структура;
	ПараметрыПодключенияВнешнегоФайла.Вставить("ИмяФайла", ИмяФайла);
	ПараметрыПодключенияВнешнегоФайла.Вставить("УниверсальноеПолноеИмяФайлаШага", УниверсальноеПолноеИмяФайлаШага);
	ПараметрыПодключенияВнешнегоФайла.Вставить("РезультатПрохожденияШага", РезультатПрохожденияШага);

	ПередаваемыеФайлы = Новый Массив;
	ПередаваемыеФайлы.Добавить(Вычислить("Новый ОписаниеПередаваемогоФайла(ИмяФайла)"));

	Выполнить("НачатьПомещениеФайлов(
		| Новый ОписаниеОповещения(""ОбработчикАсинхронноПодключитьКонтекстОбработки"",
		|	ЭтаФорма, ПараметрыПодключенияВнешнегоФайла),
		| ПередаваемыеФайлы, , Ложь, ЭтаФорма.УникальныйИдентификатор)");

КонецПроцедуры

&НаКлиенте
Процедура ОбработчикАсинхронноПодключитьКонтекстОбработки(Знач ПомещенныеФайлы,
		Знач ПараметрыПодключенияВнешнегоФайла) Экспорт

	ИмяФайла = ПараметрыПодключенияВнешнегоФайла.ИмяФайла;
	УниверсальноеПолноеИмяФайлаШага = ПараметрыПодключенияВнешнегоФайла.УниверсальноеПолноеИмяФайлаШага;
	РезультатПрохожденияШага = ПараметрыПодключенияВнешнегоФайла.РезультатПрохожденияШага;

	ФайлШагов = Новый Файл(ИмяФайла);
	УспешноПодключили = ПодключитьКонтекстОбработкиКлиент(ФайлШагов);
...
Функция ПолучитьКонтекстТестаПоАдресСнипета(АдресСнипета)
	ИД = КоллекцияИменКонтекстовОбработок.Получить(УниверсальноеПолноеИмяФайла(АдресСнипета, Истина));
	Если ИД = Неопределено Тогда
...
Функция СоздатьФайлОбработкиНаСервере(Знач ПолныйПутьФайла, ИдентификаторОбработки, ЭтоОтчет = Ложь)
	...
	// TODO использовать ОбъектОбработки.Метаданные().Имя
