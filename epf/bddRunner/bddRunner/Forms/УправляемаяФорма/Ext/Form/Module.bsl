...
Процедура ЗагрузитьНастройкиПоУмолчанию(Команда)
	ИмяФайлаИсточника = "https://raw.githubusercontent.com/silverbulleters/add/develop/tools/JSON/VBParams836UF.json"; 
	ИмяФайлаПриемника = "G:\VBParams836UF.json";
	ИмяФайлаНастроекПоУмолчанию = ПолучитьИмяВременногоФайла("json");
	КопироватьФайл(ИмяФайлаИсточника, ИмяФайлаНастроекПоУмолчанию); 
	УдалитьФайлы(ИмяФайлаНастроекПоУмолчанию);
...
Функция СоздатьСтруктураОписанияEpf(ОтносительныйКаталогФичи, ИмяТекущейФичи, GenerateEpf, ДвДанныеvbFeatureReader, ДополнительныеПараметры)
	...
	СтруктураОписанияEpf.ИмяФайлаEpf        = ДополнитьСлешВПуть(ОтносительныйКаталогФичи) + ДополнитьСлешВПуть("step_definitions") + УбратьСпецсимволыИзИмениОбработки(ИмяТекущейФичи) + ".epf";
