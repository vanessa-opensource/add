...
Функция ЗапуститьТестДляПользователя(Знач ОписаниеПользователя, Знач ПутьТестов, 
		Знач ТипКлиента, Знач РабочийКаталогПроекта = "", Знач ПутьОтчетаJUnit = "") Экспорт

	ПутьКонтекстаЯдра = КонтекстЯдра.ИспользуемоеИмяФайла;
	
	Если Не ЗначениеЗаполнено(РабочийКаталогПроекта) Тогда
		РабочийКаталогПроекта = ОбъединитьПути(КаталогВременныхФайлов(), 
			СтрШаблон("%1_%2", ОписаниеПользователя.Логин, Новый УникальныйИдентификатор));
	КонецЕсли;
	
	ОбеспечитьКаталог(РабочийКаталогПроекта);
	РабочийКаталогПроекта = ОбъединитьПути(РабочийКаталогПроекта, "");
	
	ПутьЛогФайла = ОбъединитьПути(РабочийКаталогПроекта, "online.log");
	Если Не ЗначениеЗаполнено(ПутьОтчетаJUnit) Тогда
		ПутьОтчетаJUnit = ОбъединитьПути(РабочийКаталогПроекта, "junit.xml");
	КонецЕсли;
	
	ПутьККонфигурационномуФайлу = СоздатьКонфигурационныйФайл(РабочийКаталогПроекта, "xUnitParams.json", "online.log");
	
	СтрокаЗапускаТестирования = СформироватьСтрокуЗапускаТестирования(ОписаниеПользователя, ПутьТестов, 
		ТипКлиента, ПутьККонфигурационномуФайлу, ПутьОтчетаJUnit);
...
Функция СоздатьКонфигурационныйФайл(Знач РабочийКаталогПроекта, Знач ПутьФайла, Знач ИмяЛогФайла)

	Текст = ПолучитьМакет("xUnitParams_json").ПолучитьТекст();
	Текст = СтрЗаменить(Текст, "%ИмяФайлаЛогВыполненияСценариев%", РабочийКаталогПроекта + ИмяЛогФайла);
	Текст = СтрЗаменить(Текст, "\", "\\");
	
	РезПутьФайла = ОбъединитьПути(РабочийКаталогПроекта, ПутьФайла);
	ЗаписатьФайл(РезПутьФайла, Текст);
	
	Возврат РезПутьФайла;
КонецФункции // СоздатьКонфигурационныйФайл()

Процедура ЗаписатьФайл(Знач ПутьФайла, Знач ТекстФайла)
	Запись = Новый ЗаписьТекста(ПутьФайла);
	Запись.Записать(ТекстФайла);
	Запись.Закрыть();
КонецПроцедуры

Функция СформироватьСтрокуЗапускаТестирования(Знач ОписаниеПользователя, Знач ПутьТестов, 
		Знач ТипКлиента, Знач ПутьККонфигурационномуФайлу, Знач ПутьОтчетаJUnit)
...
Функция ОбъединитьПути(Знач Каталог, Знач Файл)
	Возврат Каталог + ПолучитьРазделительПути() + Файл;
