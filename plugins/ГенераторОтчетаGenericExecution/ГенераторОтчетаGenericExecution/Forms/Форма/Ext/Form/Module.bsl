...
Процедура GenericExecution_ЗаписатьОтчет(ЗаписьXML, Отчет)
	...
	Для Каждого КлючИЗначение Из Отчет Цикл
		GenericExecution_ЗаписатьФайлТеста(ЗаписьXML, КлючИЗначение.Значение);
	КонецЦикла;
...
Процедура GenericExecution_ЗаписатьФайлТеста(ЗаписьXML, ФайлТеста)
	...
	ЗаписьXML.ЗаписатьАтрибут("path", ФайлТеста.Путь);
	
	Для Каждого РезультатТеста Из ФайлТеста.НаборРезультатов Цикл
...
&НаКлиенте
Функция НовыйФайлТеста()
	
	Результат = Новый Структура;
	Результат.Вставить("Путь", "");
	Результат.Вставить("НаборРезультатов", Новый Массив);
...
Процедура ДобавитьРезультатТеста(Отчет, ПутьТеста, РезультатТеста)
	...
	ФайлТеста = Отчет.Получить(ПутьТеста);
	
	Если ФайлТеста = Неопределено Тогда
		
		ФайлТеста = НовыйФайлТеста();
		ФайлТеста.Путь = ПутьТеста;
		Отчет.Вставить(ПутьТеста, ФайлТеста);
		
	КонецЕсли;
	
	ФайлТеста.НаборРезультатов.Добавить(РезультатТеста);
