...
Процедура НачатьЗагрузкуКаталога(Знач ОбработчикОповещения, Знач КаталогДляЗагрузки, Знач КонтекстЯдра) Экспорт
	...
	НачатьПоискФайлов(Обработчик, КаталогДляЗагрузки.ПолноеИмя, "*", Истина);//TODO добавить код для поиска в подкаталогах и обработку подкаталогов
	//НачатьПоискФайлов(Обработчик, КаталогДляЗагрузки.ПолноеИмя, "*", Ложь);//TODO добавить код для поиска в подкаталогах и обработку подкаталогов
...
Процедура ВыбратьПутьИнтерактивноЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе
		КонтекстЯдра.ЗагрузитьТесты(Описание.Идентификатор, Результат); // TODO проверить правильность перехода на асинхронные вызовы, а ранее были синхронные вызовы
	КонецЕсли;
