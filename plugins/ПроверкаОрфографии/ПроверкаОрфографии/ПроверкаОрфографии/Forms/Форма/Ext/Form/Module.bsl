...
&НаКлиенте
Функция СловоВСловареИсключений(Слово, Словарь)
	...
	Иначе
		...
		Результат = (Словарь.Найти(Слово) <> Неопределено);
...
Функция РазобратьРезультатПроверки(ОтветСервиса, СловарьИсключений)
	...
	Для Каждого СтрокаДанных Из ОтветСервиса Цикл
		...
		Если НЕ СловоВСловареИсключений(Ошибка.ИсходноеСлово, СловарьИсключений) Тогда
...
Функция ВыполнитьПроверкуТекста(ТекстНаПроверку) Экспорт
	...
	Если Ответ.КодСостояния = КодКорректноВыполненногоЗапроса Тогда 
		...
		ТелоОтвета = Ответ.ПолучитьТелоКакТекст();
		Отладка(ТелоОтвета);
		
		ДанныеОтвета = КонтекстЯдра.ПрочитатьСоответствиеИзJSONФайлаКлиент(ТелоОтвета);
		...
		Иначе
			...
			Результат = РазобратьРезультатПроверки(ДанныеОтвета, СловарьИсключений);
...
Процедура Лог(ТекстСообщения, Важное = Ложь)
	...
	Если Важное Тогда
		...
		КонтекстЯдра.ВывестиПредупреждение(ТекстСообщения);
