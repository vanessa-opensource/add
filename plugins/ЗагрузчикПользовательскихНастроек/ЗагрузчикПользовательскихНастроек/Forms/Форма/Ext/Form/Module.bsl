...
Функция ЗапроситьНастройкиCONSUL(АдресНастроек = "")
	...
	ЧастиАдреса = КонтекстЯдра.РазложитьСтрокуВМассивПодстрок(АдресБезHTTP, "/", Истина);
	...
	АдресБезСервера = СтрЗаменить(АдресБезHTTP, СерверСПортом, "");
	...
	Если Ответ.КодСостояния <> 200 Тогда
		ВызватьИсключение "Ошибка! Запрос по адресу " + АдресНастроек
			+ " вернул статус " + Строка(Ответ.КодСостояния);
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиКоманда(Команда)
	ЗагрузитьНастройки(ПоставщикUserSettings, АдресUserSettings);
КонецПроцедуры
