...
Функция СерверныеМодулиРасширения(МетаданныеРасширения)
	...
	Для Каждого Модуль Из МодулиМенеджера Цикл
		Для Каждого ИмяОбъекта Из МетаданныеРасширения[Модуль] Цикл
			
			Если ОбщийМодуль.Сервер И НЕ ОбщийМодуль.Глобальный Тогда
				МодулиРасширения.Добавить(СтрШаблон("%1.%2", Модуль, ИмяОбъекта));
			КонецЕсли;
...
Функция МодулиОбъектаРасширения(МетаданныеРасширения)
	...
	Для Каждого Модуль Из МодулиОбъекта Цикл
		Для Каждого ИмяОбъекта Из МетаданныеРасширения[Модуль.Значение] Цикл
			МодулиРасширения.Добавить(СтрШаблон("%1.%2.%3", Модуль.Значение, ИмяОбъекта, Модуль.Представление));	
		КонецЦикла; 
	КонецЦикла;
