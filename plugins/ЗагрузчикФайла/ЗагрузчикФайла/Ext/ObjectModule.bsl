...
Функция ЗагрузитьФайл(ПостроительДереваТестов, ФайлОбработки, КонтекстЯдра, ИмяОбработки = "") Экспорт
	...
	Исключение
		...
		Если ЭтоНовыйФорматОбработки(ТекстОшибки) Тогда
			ВызватьИсключение;
		Иначе
			Контейнер = ЗагрузитьТестыВСтаромФормате(ПостроительДереваТестов, Обработка, ФайлОбработки, КонтекстЯдра);
		КонецЕсли;
	КонецПопытки;
...
Функция ЗагрузитьТестыВНовомФормате(ПостроительДереваТестов, Обработка, ФайлОбработки, ИмяОбработки, КонтекстЯдра)
	...
	Если КонтейнерТестов.Строки.Количество() > 0 И КонтекстЯдра.ГруппироватьТестыПоБазам() Тогда
		ГруппаБаза = КэшПостроительДереваТестов.СоздатьКонтейнер(ИмяБазы(), КэшПостроительДереваТестов.ИконкиУзловДереваТестов.Группа);
		Для Каждого Стр Из КонтейнерТестов.Строки Цикл
			ГруппаБаза.Строки.Добавить(Стр);
		КонецЦикла;
		
		КонтейнерТестов.Строки.Очистить();
		КонтейнерТестов.Строки.Добавить(ГруппаБаза);
	КонецЕсли;
...
Функция ЗагрузитьТестыВСтаромФормате(ПостроительДереваТестов, Обработка, ФайлОбработки, КонтекстЯдра)
	Попытка
	...
	Исключение
		...
		КонецЕсли;
		КонтекстЯдра.ВывестиСообщение(СтрШаблон("При загрузке теста ""%1"" произошла ошибка ""%2""", ФайлОбработки.Имя, КраткоеПредставлениеОшибки(Инфо)), СтатусСообщения.Внимание); 
		КонтекстЯдра.Отладка(СтрШаблон("При загрузке теста ""%1"" произошла ошибка:
										|%2", ФайлОбработки.Имя, ПодробноеПредставлениеОшибки(Инфо))); 
	КонецПопытки;
