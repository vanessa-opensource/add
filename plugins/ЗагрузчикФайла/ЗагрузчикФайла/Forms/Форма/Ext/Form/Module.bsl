...
Функция ЗагрузитьФайлНаКлиенте(ПостроительДереваТестов, ФайлОбработки, КонтекстЯдра, ИмяОбработки)
	...
	УстановитьУровеньКритичности(ФормаОбработки, ЭтаФорма, Контейнер);
...
Процедура УстановитьУровень(Знач Уровень = "") Экспорт
	...
	МассивУровнейКритичности = Новый Массив;
	МассивУровнейКритичности.Добавить("blocker");
	МассивУровнейКритичности.Добавить("critical");
	МассивУровнейКритичности.Добавить("normal");
	МассивУровнейКритичности.Добавить("minor");
	МассивУровнейКритичности.Добавить("trivial");
	
	Если МассивУровнейКритичности.Найти(Уровень) = Неопределено Тогда
		
		Уровень = УровеньКритичностиПоУмолчанию();
		
	КонецЕсли; 
	
	УровеньКритичности = Уровень;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьУровеньКритичности(Тест, Загрузчик, КонтейнерТестов)
	Попытка
		
		УровеньКритичности = Неопределено;
		Тест.УстановитьУровеньТеста(Загрузчик);
		КонтейнерТестов.Уровень = УровеньКритичности;
		
	Исключение
		
		КонтейнерТестов.Уровень = УровеньКритичностиПоУмолчанию();
		
	КонецПопытки;
КонецПроцедуры

&НаКлиенте
Функция УровеньКритичностиПоУмолчанию()
	Возврат "normal";
КонецФункции
