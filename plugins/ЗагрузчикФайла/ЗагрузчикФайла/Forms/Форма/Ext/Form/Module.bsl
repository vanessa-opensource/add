...
Функция ЗагрузитьФайлНаКлиенте(ПостроительДереваТестов, ФайлОбработки, КонтекстЯдра, ИмяОбработки)
	...
	ЭтоАнглийский = Найти(ИмяОбработки, "DataProcessorObject") > 0;
...
Функция ПолучитьКонтекстЯдраНаСервере(Знач ОбъектКонтекстаЯдра)
	...
	Если НЕ МетаданныеЯдра = Неопределено
		...
		ИмяОбработкиКонекстаЯдра = СтрЗаменить(ПолноеИмяБраузераТестов, "DataProcessor", "DataProcessors");
	...
	Иначе
		...
		ИмяОбработкиКонекстаЯдра = СтрЗаменить(ПолноеИмяБраузераТестов, "ExternalDataProcessor", "ExternalDataProcessors");
...
Процедура ПодключитьВнешнююОбработку(КонтекстЯдра, Знач ФайлОбработки)
	Если Не КонтекстЯдра.Объект.ИспользоватьПрямыеПутиФайлов Тогда
