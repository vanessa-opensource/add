...
Функция ЗагрузитьТестыВНовомФормате_НаКлиенте(ПостроительДереваТестов, ФормаОбработки, ФайлОбработки, ИмяОбработки, 
	...
	Попытка
		НаименованиеКонтейнера = ФормаОбработки.НаименованиеКонтейнераТестов();
	Исключение
		НаименованиеКонтейнера = ПрефиксПутейСФормами() + ИмяОбработки;
	КонецПопытки;
	КонтейнерТестов = ПостроительДереваТестов.СоздатьКонтейнер(НаименованиеКонтейнера, ПостроительДереваТестов.Объект.ИконкиУзловДереваТестов.Форма);
	Попытка
