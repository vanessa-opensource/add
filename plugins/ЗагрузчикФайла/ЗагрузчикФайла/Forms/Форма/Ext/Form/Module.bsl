...
Функция ЗагрузитьТестыВНовомФормате_НаКлиенте(ПостроительДереваТестов, ФормаОбработки, ФайлОбработки, ИмяОбработки, 
	...
	ИмяКонтейнера = ПрефиксПутейСФормами() + ИмяОбработки;
	Если КонтекстЯдра.ГруппироватьТестыПоБазам() Тогда
		ИмяКонтейнера = ИмяБазы();
	КонецЕсли;
	
	КонтейнерТестов = ПостроительДереваТестов.СоздатьКонтейнер(ИмяКонтейнера, ПостроительДереваТестов.Объект.ИконкиУзловДереваТестов.Форма);
	Попытка
	...
	Если КонтейнерТестов.Строки.Количество() > 0 И КонтекстЯдра.ГруппироватьТестыПоБазам() Тогда
		ГруппаБаза = КэшПостроительДереваТестов.СоздатьКонтейнер(ПрефиксПутейСФормами() + ИмяОбработки, ПостроительДереваТестов.Объект.ИконкиУзловДереваТестов.Форма);
