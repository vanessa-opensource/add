...
//	  	* ДокументОбъект - для доступа к реквизитам документа, для которого используется проверка движений.
...
//	  * НастройкиПроверкиРегистров - Соответствие - содержит дополнительные настройки проверок 
//	  	(можно указать исключаемые из проверок поля и др.). Настраивается индивидуально для каждого регистра.
//	  	* Ключ - полное имя регистра, для которого выполняются настройки.
...
//	  	* БезСохраненияНовыхДвижений - Булево - движения документа не будут изменены в результате его проведения. 
...
//  ДопСообщениеОшибки			 - Строка	 - Текст сообщения, который будет выведен пользователю 
...
Процедура ПроверитьДвиженияДокумента(Знач ДокументОбъект, Знач НастройкаПроверкиДвижений = Неопределено, 
	...
	Если ОтклонениеНабораЗаписей.Количество() > 0 Тогда
		...
		Для Каждого СтрокаОтклонений Из ОтклонениеНабораЗаписей Цикл
			КлючМетаданные 	= СтрокаОтклонений.Ключ.Метаданные();
			ИмяРегистра 	= КлючМетаданные.Имя + " (" + КлючМетаданные.Синоним + ") ";
			
			СтрокаОшибок 	= СтрокаОшибок + Символы.ПС + ИмяРегистра;	
			
			// Формируем таблицу для вывода отклонений в движениях документа. 
			// Для просмотра таблицы необходим моношрифт - например - Courier.
			МаксимальныеДлиныКолонок = Новый Соответствие;
			ТаблицаДвиженияРегистров = СтрокаОтклонений.Значение;
...
// 
// Возвращаемое значение:
// 		Соответствие - прочитанные движения документа.
...
Функция ЗапомнитьДвиженияДокумента(Знач ДокументОбъект, Знач НастройкаПроверкиДвижений)
	...
	Для Каждого НаборЗаписейРегистра из ДокументОбъект.Движения Цикл
		...
		// Если регистр добавлен в исключения, то пропускаем проверку.
		...
		Если НастройкаПроверкиРегистра <> Неопределено Тогда
			...
			Если НастройкаПроверкиРегистра.НастройкаСвертки.Свернуть Тогда
				...
				Иначе	
					// Сворачиваем движения регистра с группировкой колонкам с типом значения "Число".
					КолонкиГруппировок = "";
					КолонкиСуммирования = ""; 
					Для Каждого КолонкаЗаписейРегистра Из КолонкиЗаписейРегистра Цикл
...
Функция РазницаТаблицЗначений(Таблица0, Таблица1, ИмяРегистра =  Неопределено, Измерения = Неопределено)
	...
	// Предварительно свернем ТЗ и просуммируем ресурсы.
...
Функция ПроверитьДвиженияДокументаНаИзменения(НабораЗаписейОбъекта,НабораЗаписейОбъектаПосле)
	...
	Для каждого НаборЗаписей Из НабораЗаписейОбъекта Цикл
		...
		Если ТаблицаРазличий.Количество() > 0 Тогда
			СоответствиеРазличий[НаборЗаписей.Ключ] = ТаблицаРазличий;
		КонецЕсли;	
	КонецЦикла;
	
	Возврат СоответствиеРазличий;
