...
// Функция - Создает конструктор для создания нового "Вычисляемого поля"
// 
...
//		* Тип - ОписаниеТипов - Тип значения нового вычисляемого поля. Не обязательно для заполнения. Рекомендуется указывать если выполняется 
//		* Формула - Строка - содержит имена полей (измерений, реквизитов, ресурсов и т.д.), по которым будет выполняться проверка движений регистров. Для использования также доступны переменные:
...
//			* КолонкиГруппировок - Строка - через запятую указать имена колонок, по которым движения регистра будут сгруппированы. 
//			* КолонкиСуммирования - Строка - через запятую указать имена колонок, данные которых будут просуммированы.
//			!!! - Если КолонкиГруппировок и КолонкиСуммирования оставить пустыми, но Свернуть = Истина, тогда колонки группировок будут определены по типу значения колонок. Все "Числовые" колонки будут отнесены к  колонкам суммирования.
...
// Процедура - Проверяет движения документа путем его перепроведения. При наличии разницы в движения (до и после перепроведения) формирует таблицу отклонений.
//
// Параметры:
//  ДокументОбъект				 - ДокументОбъект, ДокументСсылка - документ, для которого необходимо проверить движения.
//  НастройкаПроверкиДвижений	 - Структура - Настройка проверки движений документа см. функцию НоваяНастройкаПроверкиДвиженийДокумента() 
//  ДопСообщениеОшибки			 - Строка	 - Текст сообщения, который будет вывведен пользователю при наличии изменений в движениях документа.
//
&НаСервере
Процедура ПроверитьДвиженияДокумента(Знач ДокументОбъект, Знач НастройкаПроверкиДвижений = Неопределено, Знач ДопСообщениеОшибки = "") Экспорт
	...
	Если ОтклонениеНабораЗаписей.Количество() > 0 Тогда
		...
		Для Каждого СтрокаОтклоненией Из ОтклонениеНабораЗаписей Цикл
			...
			// Формируем таблицу для вывода отколонений в движениях документа. Для просмотра таблицы необходим моношрифт - например - Courier.
			...
			СтрокаОшибок = СтрокаОшибок +Символы.ПС + ВывестиСтрокуТаблицы(ТаблицаДвиженияРегистров,МаксимальныеДлиныКолонок);			// Прорисовываем рамку таблицы.
			СтрокаОшибок = СтрокаОшибок +Символы.ПС + ВывестиСтрокуТаблицы(ТаблицаДвиженияРегистров,МаксимальныеДлиныКолонок,Истина); 	// Прорисовываем шапку с именами колонок.
			СтрокаОшибок = СтрокаОшибок +Символы.ПС + ВывестиСтрокуТаблицы(ТаблицаДвиженияРегистров,МаксимальныеДлиныКолонок);			// Прорисовываем рамку.
			
			// Выводим непосредственно саму таблицу движений.
			Для Каждого СтрокаТз Из ТаблицаДвиженияРегистров Цикл
				СтрокаОшибок = СтрокаОшибок +Символы.ПС + ВывестиСтрокуТаблицы(ТаблицаДвиженияРегистров,МаксимальныеДлиныКолонок,СтрокаТЗ);					 
			КонецЦикла;	
			СтрокаОшибок = СтрокаОшибок +Символы.ПС + ВывестиСтрокуТаблицы(ТаблицаДвиженияРегистров,МаксимальныеДлиныКолонок);			// Прорисовываем рамку.
...
// Функция - считывает движения документа в соответствии с переданными настройками
//
// Параметры:
//  ДокументОбъект				 - ДокументОбъект	- документ, для которого необходимо прочитать движения 
//  НастройкаПроверкиДвижений	 - Структура	-  настройка проверки движений документа. См. ф-ию НоваяНастройкаПроверкиДвиженийДокумента().
// 
// Возвращаемое значение:
// 		Соответсвие - прочитанные движения документа.
//			* Ключ - Набор записей регистра.
//			* Значение -  ТаблицаЗначений - таблица, содержащая движения регистра, сформированные в соотв. с переданными настройками.
//
&НаСервере
...
// 
&НаСервере
Функция РазницаТаблицЗначений(Таблица0, Таблица1, Измерения = Неопределено, ИмяРегистра =  Неопределено)
...
Функция ПроверитьДвиженияДокументаНаИзменения(НабораЗаписейОбъекта,НабораЗаписейОбъектаПосле)
	...
	Для каждого НаборЗаписей Из НабораЗаписейОбъекта Цикл
		ТаблицаРазличий = РазницаТаблицЗначений(НаборЗаписей.Значение, НабораЗаписейОбъектаПосле[НаборЗаписей.Ключ],,НаборЗаписей.Ключ.Метаданные().Имя);
...
Функция ВывестиСтрокуТаблицы(Знач ТаблицаДвиженияРегистров, Знач МаксимальныеДлиныКолонок, Знач ВыводимоеЗначение = Неопределено)
