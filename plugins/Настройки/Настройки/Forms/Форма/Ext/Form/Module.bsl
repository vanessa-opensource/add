...
Функция ПрочитатьНастройкиИзФайлаJSon(Знач ПутьФайлаНастроек)
	...
	ФайлНастроекСуществует = Не КонтекстЯдра.ЕстьПоддержкаАсинхронныхВызовов И ФайлНастроек.Существует();
	...
	Если ФайлНастроекСуществует Тогда
		...
		Исключение
			...
			Если Найти(ИнфоОшибки.Описание, "Неправильный путь к файлу") = 0 Тогда
				СообщениеОшибки = СтрШаблон("Не удалось прочитать файл json-настроек %1
				|
				|Ошибка: %2" + КраткоеПредставлениеОшибки(ИнфоОшибки), ПутьФайлаНастроек);
