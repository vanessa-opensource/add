...
//    РезультатПроверки = ЕстьНастройка("Ключ1.Ключ2.Ключ3", ЗначениеНастройки);
// В результате получим ВремЗнач == Структура("Ключ3", 42)
//
&НаКлиенте
...
&НаКлиенте
// (Не использовать) Добавить настройку
//
// Параметры:
//  Ключ	 - 	 -
//  Значение - 	 -
...
&НаКлиенте
// (Не использовать) Добавить настройки
//
// Параметры:
//  ИсточникНастроек - 	 -
//
...
// Загружает в настройки новые данные из переданной структуры
//
// Параметры:
//  СтруктураНастроек	 - Структура	 - данные, которые требуется загрузить в настройки
//
&НаКлиенте
Процедура ЗагрузитьНастройки(Знач СтруктураНастроек = Неопределено) Экспорт
	
	Если ТипЗнч(СтруктураНастроек) = Тип("Структура") Тогда
		Объект.Настройки = Новый ФиксированнаяСтруктура(СтруктураНастроек);
	ИначеЕсли СтруктураНастроек = Неопределено Тогда
		Объект.Настройки = Новый ФиксированнаяСтруктура();
	Иначе
		Возврат;
	КонецЕсли;
