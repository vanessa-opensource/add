...
Функция ТестКлиентЗапущен(Порт)
	...
	П = XMLСтрока(Порт);
	Если ЭтоLinux() Тогда
		ЗапуститьПриложение(СтрШаблон("ps aux | grep -ie TESTCLIENT | grep -ie 1cv8c | grep ""TPort[\s]*%1"" -iP > %2", П, ИмяВременногоФайла));
	Иначе
	...
	Ответ = (ЭтоLinux() И СтрЧислоСтрок(Текст.Прочитать()) > 1) ИЛИ (Не ЭтоLinux() И СтрЧислоСтрок(Текст.Прочитать()) > 0);
	Текст.Закрыть();
	
	Возврат Ответ;
КонецФункции
 
// Функция - Функция получает последнее (по порядку) открытое окно
//
// Параметры:
...
Процедура ИдентифицироватьОкноПредупреждение(Знач ТестКлиент, Знач Пояснение = "", ИгнорируемыйМассивТекстовИсключений = Неопределено) Экспорт
	...
	Если ТипЗнч(ТекущаяИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда 
		...
		КонецЕсли;  
	Иначе
		Ошибка = НаличиеОкнаОшибки(ТестКлиент);
		Если ЗначениеЗаполнено(Ошибка) И (Не ЗначениеЗаполнено(ИгнорируемыйМассивТекстовИсключений) ИЛИ Не МассивСоджержит(ИгнорируемыйМассивТекстовИсключений, Ошибка)) Тогда
			ЗакрытьВсеОткрытыеОкна(ТестКлиент);
	...
	// #992
	// при совершении разлычных действий тестом (открытие формы, клик по кнопке) может появится окно длительной операции
	// если оно отрыто мы ждем n секунд пока не закроется, если не закроется, ошибка
	Таймаут = ТекущаяДата() + 10; // 10 сек             
	ОкноДлительныхОперацийОткрыто = Истина;
	Пока ТекущаяДата() < Таймаут Цикл
		ОкноДлительныхОперацийОткрыто = ОткрытоОкноДлительныхОпераций(ТестКлиент);     
		Если Не ОкноДлительныхОперацийОткрыто Тогда
			Прервать;	
		КонецЕсли;
	...
	Если ОкноДлительныхОперацийОткрыто Тогда
	...
	//		просто взять закрыть все окна было бы не верно т.к. могут быть случаи когда мы открыли форму объекта и в этой форме выполняем различные действия (кликаем по строкам, переходим по ком. интерфейсу)
	// 		такое поведение например в тесте тесты_КомандныйИнтерфейс, и мы должны закрывать только те окна которые появились в следствии выполняемых действий, форму объекта не должны трогать 
	СписокПропускаемыхФорм.Очистить();
	ЗакрытьВсеОткрытыеОкнаСУчетомНастроек(ТестКлиент);
КонецПроцедуры

&НаКлиенте
Функция НаличиеОкнаОшибки(Знач ТестКлиент)   
	// не все ошибки ловятся через ТестКлиент.ПолучитьТекущуюИнформациюОбОшибке(), например ошибки дин. списка так не поймать
	// для этого есть данная функция
	                                 
	ОткрытыеОкна = ПодчиненныеОбъекты(ТестКлиент);
	Для Каждого ТекОкно Из ОткрытыеОкна Цикл    
		Форма = ТекОкно.НайтиОбъект(Тип("ТестируемаяФорма"));
		Если Форма <> Неопределено И НРег(Форма.ИмяФормы) = "errorwindow" Тогда
...
Процедура ЗакрытьВсеОткрытыеОкнаСУчетомНастроек(ТестКлиент) Экспорт
	...
	Для Каждого ТекОкно Из ОткрытыеОкна Цикл
		...
		ОкноЗакрыто = Ложь;
		Если ОкноПредупреждение = Неопределено Тогда                   
			// как по мне это только хуже делает, много "ошибочных" кликов по кнопке на форме документа, справочника.
			...
			// уж лучше для окон с ошибкой или модальных задавать правила в конфиге
...
&НаКлиенте
Функция ОткрытоОкноДлительныхОпераций(ТестКлиент)    
	Ответ = Ложь;
	...
	Попытка
		Для Каждого Кнопуля Из Окно.НайтиОбъекты(Тип("ТестируемаяКнопкаФормы")) Цикл
			Если Кнопуля.Вид = ВидКнопкиФормы.ОбычнаяКнопка ИЛИ Кнопуля.Вид = ВидКнопкиФормы.КнопкаКоманднойПанели И Не СтрНачинаетсяС_(Кнопуля.ТекстЗаголовка, "Command") Тогда
				...
				КонтекстЯдра.Отладка(СтрШаблон("	-->> ОКНА: %1. Кнопка ""%2""", Кнопки.Количество(), Кнопуля.Имя));
			КонецЕсли;
		КонецЦикла;
	Исключение
		Возврат Кнопки;
...
Функция ОткрытыеОкна(ТестКлиент) Экспорт
	...
	Для Каждого ТекОкно Из ВсеОкна Цикл
		Если ТекОкно = Неопределено Или ТекОкно.Основное Или ТекОкно.НачальнаяСтраница Тогда
			Продолжить;
		КонецЕсли;                
		Заголовок = СокрЛП(СтрЗаменить(ТекОкно.Заголовок, "*", ""));
...
Процедура ПроверитьНаНовыеМодальныеОкна(ТестКлиент, ОткрытыеОкнаДо) Экспорт 
	...
	Попытка
		...
		Для Каждого ТекОкно Из ОткрытыеОкна Цикл  
			Заголовок = СокрЛП(СтрЗаменить(ТекОкно.Заголовок, "*", ""));  // удаляем из заголовка пробелы и *, т.к. нам не нужно что б ругалось если у формы взведен флаг модифицированность
...
Функция ПодчиненныеОбъекты(ТестКлиент) 
	...
	Исключение
		...
		Для Каждого ТК Из ЗапущенныеТестКлиенты Цикл     
			ДопСведения.Добавить(СтрШаблон("Данные тест-клиена. Порт ""%1"", PID клиента ""%2""", ТК.Порт, СведенияОЗанятыхПортах.Процессы[ТК.Порт]));
		КонецЦикла;
