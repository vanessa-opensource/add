...
Функция ТестКлиентПоУмолчанию() Экспорт
	...
	Если ЗначениеЗаполнено(ЗапущенныеТестКлиенты) Тогда
		КонтекстЯдра.Отладка(СтрШаблон("ТестКлиентПоУмолчанию: Был использован один из запущенных ранее тест клиентов.
										 |                       ИмяПользователя: %1
										 |                       Порт: %2
	...
	КонтекстЯдра.Отладка(СтрШаблон("ТестКлиентПоУмолчанию: Был запущен новый тест клиент.
...
Процедура ИдентифицироватьОкноПредупреждение(Знач ТестКлиент, Знач Пояснение = "", 
	...
	// нет смысла получать окно по настройкам, если б были настройки окно закрылось бы. 
	// Куда полезней вывести список 
	//ТекстИсключения = "";
	...
	Окна = ПолучитьСписокОткрытыхОкон(ТестКлиент);
	Если Не ЗначениеЗаполнено(Окна) И Не ЗначениеЗаполнено(ПодробноеПредставлениеОшибки) Тогда
		Возврат;		
	КонецЕсли;

	Если ПропускатьПриОтсутствииПрав И СтрНайти(Окна, "Недостаточно прав для просмотра") > 0 Тогда
		КонтекстЯдра.ПропуститьТест(Окна);
	КонецЕсли;
...
Функция ОкноПредупреждение(ТестКлиент)
	...
	ПодчиненныеОбъекты = ПопыткаПолучитьПодчиненныеОбъекты(ТестКлиент);
	...
	Для Каждого ПодчиненныйОбъект Из ПодчиненныеОбъекты Цикл
		...
		Если ПолучитьОписаниеМодальногоОкна(ПодчиненныйОбъект) <> Неопределено Тогда
...
&НаКлиенте
Функция ПолучитьОписаниеМодальногоОкна(ОкноПриложения)
	Перем Поля;
...
Функция ОсновноеОкно(ТестКлиент) Экспорт
	Попытка
		КлиентскиеОкнаТестируемогоПриложения = ПопыткаПолучитьПодчиненныеОбъекты(ТестКлиент);
...
&НаКлиенте
Функция ПолучитьСписокОткрытыхОкон(ТестКлиент) Экспорт
	ОткрытыеОкна = ПопыткаПолучитьПодчиненныеОбъекты(ТестКлиент);
...
Процедура ЗакрытьВсеОткрытыеОкнаСУчетомНастроек(ТестКлиент) Экспорт
	...
	Для Каждого ТекОкно Из ОткрытыеОкна Цикл
		...
		//ДанныеОкна = СодержимоеОкна(ТекОкно);
...
Функция ЗакрытьОкноПредупреждения(Кнопки, ОкноПриложения)
	Результат = Ложь;
	ОписаниеМодальногоОкна = ПолучитьОписаниеМодальногоОкна(ОкноПриложения);
...
// Функция - Получить подчиненные объекты
//           функция обертка над ПолучитьПодчиненныеОбъекты, выполняется n раз. Попытка обойти ошибку "Ошибка сетевого взаимодействия при вызове"
// Параметры:
//  ТестКлиент	 - 	 - 
// 
// Возвращаемое значение:
...
//
&НаКлиенте
Функция ПопыткаПолучитьПодчиненныеОбъекты(ТестКлиент) Экспорт 
	...
	Исключение
		...
		Текст = СтрШаблон("Не удалось получить подчиненные объекты
							|Ошибка: 
							|%1", Ошибка);
		
		ЗаписатьОшибкуВЖурналеРегистрации(Текст);
		КонтекстЯдра.ВызватьОшибкуПроверки(Текст);
		
		// Если открыт диалог работы с файлами все дальнейшие тесты будут падать с такой же ошибкой, по этому нет смысла их продолжать выполнять
		Если Ошибка = "В клиенте открыт системный диалог для работы с файлами" Тогда
			КонтекстЯдра.ВызватьОшибкуПроверки(Ошибка);	
		КонецЕсли;
