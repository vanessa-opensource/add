...
Процедура ПодключитьТестКлиент_ПакетныйРежим(Параметры_xddTestClient, ДопПараметры) Экспорт
	...
	Если ДопПараметры.Количество() > 0 И ТипЗнч(ДопПараметры[0]) <> Тип("ФиксированныйМассив") Тогда
		НовыйМассивПараметров = Новый Массив;
		НовыйМассивПараметров.Добавить(ДопПараметры);
		ДопПараметры = НовыйМассивПараметров;
	КонецЕсли;
