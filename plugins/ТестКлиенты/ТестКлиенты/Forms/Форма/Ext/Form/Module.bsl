...
Процедура ИдентифицироватьОкноПредупреждение(Знач ТестКлиент, Знач Пояснение = "", ИгнорируемыйМассивТекстовИсключений = Неопределено) Экспорт
	...
	ОкноПредупреждение = ОкноПредупреждение(ТестКлиент);  // ежели среди открытых окон нет окон подходящих под наши правила, ну и не очень то и хотелось ☺
	Если ПодробноеПредставлениеОшибки = "" И ОкноПредупреждение = Неопределено Тогда
		Возврат;		
	КонецЕсли;
