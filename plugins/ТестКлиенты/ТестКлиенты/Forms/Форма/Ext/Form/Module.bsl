...
Функция ТестКлиентЗапущен(Порт)
	...
	Попытка
		...
		Результат = (ЭтоLinux() И СтрЧислоСтрок(Текст.Прочитать()) > 1) ИЛИ (Не ЭтоLinux() И СтрЧислоСтрок(Текст.Прочитать()) > 0);
		Текст.Закрыть();
...
Процедура ИдентифицироватьОкноПредупреждение(Знач ТестКлиент, Знач Пояснение = "", ИгнорируемыйМассивТекстовИсключений = Неопределено) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Ошибка) Тогда
			Если Не ЗначениеЗаполнено(ИгнорируемыйМассивТекстовИсключений) ИЛИ Не МассивСоджержит(ИгнорируемыйМассивТекстовИсключений, Ошибка) Тогда
				ЗакрытьВсеОткрытыеОкна(ТестКлиент);  
