...
Функция ОкноПредупреждение(ТестКлиент)
	...
	Для Каждого ПодчиненныйОбъект Из ПодчиненныеОбъекты Цикл
		Если ТипЗнч(ПодчиненныйОбъект) <> Тип("ТестируемоеОкноКлиентскогоПриложения") Тогда
			Продолжить;
		КонецЕсли;
		ОписаниеМодальногоОкна = ЗаголовкиМодальныхОкон.Получить(ДляСравнения(ПодчиненныйОбъект.Заголовок));
...
Процедура ЗакрытьОкноПредупреждения(ОкноПриложения) Экспорт
	...
	Если НажатьКнопкуМодальногоДиалога(ОкноПриложения.ТекстЗаголовка, Кнопки) Тогда
		Возврат;
	КонецЕсли;
...
Функция НажатьКнопкуМодальногоДиалога(Знач ПроверяемыйТекст, Знач Кнопки, Знач ПроверятьПоля = Ложь)
	...
	ПроверяемыйТекст = ДляСравнения(ПроверяемыйТекст);
	Если ПроверятьПоля Тогда
		ОписаниеМодальногоОкна = ПоляМодальныхОкон.Получить(ПроверяемыйТекст);
	Иначе
		ОписаниеМодальногоОкна = ЗаголовкиМодальныхОкон.Получить(ПроверяемыйТекст);
	КонецЕсли;
	Если ОписаниеМодальногоОкна = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ИндексКнопки >= Кнопки.Количество() Тогда
		КонтекстЯдра.ВывестиСообщение("Индекс кнопки не должен быть количества кнопок в форме " + Кнопки.Количество(),
