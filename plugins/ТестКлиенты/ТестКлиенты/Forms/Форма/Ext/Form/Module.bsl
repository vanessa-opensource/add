...
Процедура ПодключитьТестКлиент_ПакетныйРежим(Параметры_xddTestClient, ДопПараметры = Неопределено) Экспорт
	КонтекстЯдра.Отладка("ПодключитьТестКлиент: Подключаем тест клиент пакетный режим");
	...
	Для Каждого ОчередныеПараметры Из Параметры_xddTestClient Цикл
		Попытка
			...
			Если ПользовательПарольПорт.Количество() = 3 Тогда
				КонтекстЯдра.Отладка(СтрШаблон("ПодключитьТестКлиент: параметры %1", 
		...
		Исключение
			...
			ВызватьИсключение КонтекстЯдра.Отладка(ОписаниеОшибки);
...
Функция ПодключитьТестКлиент(Знач ИмяПользователя = "", Знач Пароль = "", Знач Порт = 0, Знач ДопПараметры = "") Экспорт
	...
	КонтекстЯдра.Отладка(СтрШаблон("ПодключитьТестКлиент:
		|ИмяПользователя: %1
		|Пароль: %2
		|Порт: %3
	...
	КонтекстЯдра.Отладка("ПодключитьТестКлиент: Команда выполнена");
	...
	Если Подключен И ОсновноеОкно(ПодключенныйТестКлиент) = Неопределено Тогда
		...
		Если ОсновноеОкно(ПодключенныйТестКлиент) = Неопределено Тогда
			...
			ВызватьИсключение КонтекстЯдра.Отладка(НСтр("ru = 'ПодключитьТестКлиент: Превышено время ожидания ввода пароля.'"));
		КонецЕсли;
	КонецЕсли;
	...
	КонтекстЯдра.Отладка(СтрШаблон("ПодключитьТестКлиент: Успешно подключен, порт %1" ,Порт));
