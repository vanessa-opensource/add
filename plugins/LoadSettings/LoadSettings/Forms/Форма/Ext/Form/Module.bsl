...
Процедура ЗагрузитьНастройкиВРежимеКоманднойСтроки(ОбъектЯдра, СтруктураДействий) Экспорт
	...
	АдресUserSettings = ПолучитьЗначение(СтруктураПараметров, "usersettingspath");
	АдресUserSettings = ?(АдресUserSettings = Неопределено, ПолучитьЗначение(ПеременныеСреды, "usersettingspath"), АдресUserSettings);
	
	Если Не ПустаяСтрока(АдресUserSettings) Тогда
		ОбъектЯдра.АдресUserSettings = АдресUserSettings;
	КонецЕсли;
	
	ПоставщикUserSettings = ПолучитьЗначение(СтруктураПараметров, "usersettingsprovider");
	ПоставщикUserSettings = ?(ПоставщикUserSettings = Неопределено, ПолучитьЗначение(ПеременныеСреды, "usersettingsprovider"), ПоставщикUserSettings);
	
	Если Не ПустаяСтрока(ПоставщикUserSettings) Тогда
		ОбъектЯдра.ПоставщикUserSettings = ПоставщикUserSettings;
	КонецЕсли;
...
Функция СоздатьОбщуюКоллекциюКомандЗапуска(Знач ЭтоLinux)
	...
	Описание.Вставить("ТипЗапуска", ТипыЗапуска.ВсеВиды);
	Рез.Вставить("АдресUserSettings", Описание);
	
	Описание = Новый Структура("Р, А, ПоУмолчанию, УстанавливатьЕслиЗаполнено, ЭтоМассив, УстанавливатьЕслиИстина, ПреобразоватьПуть, ПростоУстановить, РазрешаюшийКлюч",
		"usersettingsprovider", "usersettingsprovider", "", Истина, Ложь, Ложь, Ложь, Ложь, "");
	Описание.Вставить("ТипЗапуска", ТипыЗапуска.ВсеВиды);
	Рез.Вставить("ПоставщикUserSettings", Описание);
