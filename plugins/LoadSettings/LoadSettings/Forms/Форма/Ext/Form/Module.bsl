...
Процедура ЗагрузитьНастройкиВРежимеКоманднойСтроки(ОбъектЯдра, СтруктураДействий) Экспорт
	...
	Если ОбъектЯдра.КаталогиБиблиотек.Количество() = 0 Тогда
		Если ЗначениеЗаполнено(ОбъектЯдра.КаталогИнструментов) Тогда
			ДополнитьКаталогиБиблиотекПоУмолчанию(ОбъектЯдра.КаталогиБиблиотек);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ДополнитьКаталогиБиблиотекПоУмолчанию(КаталогиБиблиотек) Экспорт

	КаталогиБиблиотек.Добавить(ПутьБиблиотекПродукта());
	КаталогиБиблиотек.Добавить(ПутьБиблиотекПроекта());

КонецПроцедуры

&НаКлиенте
Процедура ДополнитьТегиИсключенийПоУмолчанию(ТегиИсключений) Экспорт
	
	ТегиИсключений.Добавить("IgnoreOnCIMainBuild");
...
Функция СоздатьОбщуюКоллекциюКомандЗапуска(Знач ЭтоLinux)
	...
	ДополнитьКаталогиБиблиотекПоУмолчанию(СписокКаталогиБиблиотекПоУмолчанию);
	...
	ДополнитьТегиИсключенийПоУмолчанию(СписокТеговИсключениеПоУмолчанию);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПутьБиблиотекПроекта()
	 Возврат "$workspaceRoot/libraries";
КонецФункции
