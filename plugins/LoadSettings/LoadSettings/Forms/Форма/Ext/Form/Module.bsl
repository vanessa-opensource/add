...
Процедура ЗагрузитьНастройкиВРежимеКоманднойСтроки(ОбъектЯдра, СтруктураДействий) Экспорт
	...
	//ПутьКОбработке = КонтекстЯдра.ПолучитьПутьКОбработкеКлиент();
		...
		ЗначКаталогиБиблиотек = ПолучитьЗначение(СтруктураJSON, "КаталогиБиблиотек");
		...
		ЗнИмяФайлаВнешнихКомандФреймворка = ПолучитьЗначение(СтруктураJSON, "ИмяФайлаВнешнихКомандФреймворка");
		...
		ЗнПриравниватьPendingКFailed = ПолучитьЗначение(СтруктураJSON, "ПриравниватьPendingКFailed");
		Если ЗначениеЗаполнено(ЗнИмяФайлаВнешнихКомандФреймворка) Тогда			
			ОбъектЯдра.ПриравниватьPendingКFailed = ЗнПриравниватьPendingКFailed;
		КонецЕсли;
		
		ЗнОстановкаПриВозникновенииОшибки = ПолучитьЗначение(СтруктураJSON, "ОстановкаПриВозникновенииОшибки");
		...
		ЗнКаталогИнструментов = ПолучитьЗначение(СтруктураJSON, "КаталогИнструментов");
		...
		ЗначСписокТеговИсключение = ПолучитьЗначение(СтруктураJSON, "СписокТеговИсключение");
		...
		ЗначСписокТеговОтбор = ПолучитьЗначение(СтруктураJSON, "СписокТеговОтбор");
		Если ТипЗнч(ЗначСписокТеговОтбор) = Тип("Массив") Тогда
			ОбъектЯдра.СписокТеговОтбор.ЗагрузитьЗначения(ЗначСписокТеговОтбор);
		КонецЕсли;  
		
		ЗнДелатьСообщенияТранслитом = ПолучитьЗначение(СтруктураJSON, "ДелатьСообщенияТранслитом");
		Если ЗнДелатьСообщенияТранслитом = Истина Тогда
			ОбъектЯдра.ДелатьСообщенияТранслитом = Истина;
		КонецЕсли;	 
		
		ЗнСоздаватьИнструкциюHTML = ПолучитьЗначение(СтруктураJSON, "СоздаватьИнструкциюHTML");
		Если ЗнСоздаватьИнструкциюHTML = Истина Тогда
			ОбъектЯдра.СоздаватьИнструкциюHTML = Истина;
		КонецЕсли;	 
		
		ЗнСоздаватьИнструкциюMarkdown = ПолучитьЗначение(СтруктураJSON, "СоздаватьИнструкциюMarkdown");
		Если ЗнСоздаватьИнструкциюMarkdown = Истина Тогда
			ОбъектЯдра.СоздаватьИнструкциюMarkdown = Истина;
		КонецЕсли;	 
		
		ЗнСоздаватьИнструкциюВидео = ПолучитьЗначение(СтруктураJSON, "СоздаватьИнструкциюВидео");
		Если ЗнСоздаватьИнструкциюВидео = Истина Тогда
			ОбъектЯдра.СоздаватьИнструкциюВидео = Истина;
		КонецЕсли;	 
		
		ЗнДелатьЛогВыполненияСценариевВЖР = ПолучитьЗначение(СтруктураJSON, "ДелатьЛогВыполненияСценариевВЖР");
		Если ЗнДелатьЛогВыполненияСценариевВЖР = Истина Тогда
			ОбъектЯдра.ДелатьЛогВыполненияСценариевВЖР = Истина;
		КонецЕсли;	  		
		
		ЗнДелатьЛогВыполненияСценариевВТекстовыйФайл = ПолучитьЗначение(СтруктураJSON, "ДелатьЛогВыполненияСценариевВТекстовыйФайл");
		Если ЗнДелатьЛогВыполненияСценариевВТекстовыйФайл = Истина Тогда
			...
			ОбъектЯдра.ИмяФайлаЛогВыполненияСценариев             = КонтекстЯдра.ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначение(СтруктураJSON, "ИмяФайлаЛогВыполненияСценариев"));
		КонецЕсли;	  		
		
		ЗнВерсияПлатформыДляГенерацииEPF = ПолучитьЗначение(СтруктураJSON, "ВерсияПлатформыДляГенерацииEPF");
		...
		ЗнЗапускатьТестКлиентВРежимеОтладки = ПолучитьЗначение(СтруктураJSON, "ЗапускатьТестКлиентВРежимеОтладки");
		Если ЗнЗапускатьТестКлиентВРежимеОтладки = Истина Тогда
			ОбъектЯдра.ЗапускатьТестКлиентВРежимеОтладки = Истина;
		КонецЕсли;
		
		ЗнДиапазонПортовTestclient = ПолучитьЗначение(СтруктураJSON, "ДиапазонПортовTestclient");
		...
		ЗнДелатьСкриншотПриВозникновенииОшибки = ПолучитьЗначение(СтруктураJSON, "ДелатьСкриншотПриВозникновенииОшибки");
		Если ЗнДелатьСкриншотПриВозникновенииОшибки = Истина Тогда
			ОбъектЯдра.ДелатьСкриншотПриВозникновенииОшибки = Истина;
		КонецЕсли;	 
		
		ЗнСниматьСкриншотКаждогоОкна1С = ПолучитьЗначение(СтруктураJSON, "СниматьСкриншотКаждогоОкна1С");
		Если ЗнСниматьСкриншотКаждогоОкна1С = Истина Тогда
			ОбъектЯдра.СниматьСкриншотКаждогоОкна1С = Истина;
		КонецЕсли;	 
		
		ЗнКомандаСделатьСкриншот = ПолучитьЗначение(СтруктураJSON, "КомандаСделатьСкриншот");
		...
		Если ОбъектЯдра.ДелатьСкриншотПриВозникновенииОшибки Тогда
			ЗнКаталогOutputСкриншоты = ПолучитьЗначение(СтруктураJSON, "КаталогOutputСкриншоты");
		...
		ОбъектЯдра.ЗаписьВидеоКомандаНачатьЗаписьВидео                     = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоКомандаНачатьЗаписьВидео");
		ОбъектЯдра.ЗаписьВидеоФайлКурсораМышки                             = КонтекстЯдра.ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоФайлКурсораМышки"));
		ОбъектЯдра.ЗаписьВидеоКаталогДляВременныхФайлов                    = КонтекстЯдра.ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоКаталогДляВременныхФайлов"));
		ОбъектЯдра.ЗаписьВидеоФайлВодянойЗнак                              = КонтекстЯдра.ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоФайлВодянойЗнак"));
		ОбъектЯдра.ЗаписьВидеоКомандаFfmpeg                                = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоКомандаFfmpeg");
		ОбъектЯдра.ЗаписьВидеоКомандаConvert                               = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоКомандаConvert");
		ОбъектЯдра.ЗаписьВидеоКаталогМузыки                                = КонтекстЯдра.ПреобразоватьПутьСТочкамиКНормальномуПути(ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоКаталогМузыки"));
		ОбъектЯдра.ЗаписьВидеоДелатьНаложениеTTS                           = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоДелатьНаложениеTTS");
		ОбъектЯдра.ЗаписьВидеоПутьКДвижкуTTS                               = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоПутьКДвижкуTTS");
		ОбъектЯдра.ЗаписьВидеоИмяTTS                                       = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоИмяTTS");
		...
		ОбъектЯдра.ЗаписьВидеоЭкранШирина                                  = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоЭкранШирина");
		ОбъектЯдра.ЗаписьВидеоЭкранВысота                                  = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоЭкранВысота");
		ОбъектЯдра.ЗаписьВидеоКоличествоКадров                             = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоКоличествоКадров");
		ОбъектЯдра.ЗаписьВидеоПодсвечиватьАктивныеЭлементыФорм             = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоПодсвечиватьАктивныеЭлементыФорм");
		ОбъектЯдра.ЗаписьВидеоПеремещатьКурсорМышкиКАктивномуЭлементуФормы = ПолучитьЗначение(СтруктураJSON, "ЗаписьВидеоПеремещатьКурсорМышкиКАктивномуЭлементуФормы");
		//по записи видео - окончание
		ОбъектЯдра.ИмяТекущейСборки = ПолучитьЗначение(СтруктураJSON, "ИмяСборки");
		ЗнЗакрытьTestClientПослеЗапускаСценариев = ПолучитьЗначение(СтруктураJSON, "ЗакрытьTestClientПослеЗапускаСценариев");
		...
		Если ОбъектЯдра.СоздаватьИнструкциюHTML Тогда
			ЗнКаталогOutputИнструкцияHTML = ПолучитьЗначение(СтруктураJSON, "КаталогOutputИнструкцияHTML");
		...
		Если ОбъектЯдра.СоздаватьИнструкциюMarkdown Тогда
			ЗнКаталогOutputИнструкцияMarkdown = ПолучитьЗначение(СтруктураJSON, "КаталогOutputИнструкцияMarkdown");
		...
		Если ОбъектЯдра.СоздаватьИнструкциюВидео Тогда
			ЗнКаталогOutputИнструкцияВидео = ПолучитьЗначение(СтруктураJSON, "КаталогOutputИнструкцияВидео");
		...
		Если ОбъектЯдра.ЗапускатьТестКлиентВРежимеОтладки Тогда
			ОбъектЯдра.КлючиОтладки 	= ПолучитьЗначение(СтруктураJSON, "КлючиОтладки");
			ОбъектЯдра.АдресОтладчика 	= ПолучитьЗначение(СтруктураJSON, "АдресОтладчика");
		КонецЕсли;
		
		
		//для отчетов о прохождении сценариев
		ЗнДобавлятьКИмениСценарияУсловияВыгрузки = ПолучитьЗначение(СтруктураJSON, "ДобавлятьКИмениСценарияУсловияВыгрузки");
		Если ЗнДобавлятьКИмениСценарияУсловияВыгрузки = Истина Тогда
			ОбъектЯдра.ДобавлятьКИмениСценарияУсловияВыгрузки = Истина;
		КонецЕсли;	 
		
		ЗнВыгружатьСтатусВыполненияСценариевВФайл = ПолучитьЗначение(СтруктураJSON, "ВыгружатьСтатусВыполненияСценариевВФайл");
		Если ЗнВыгружатьСтатусВыполненияСценариевВФайл = Истина Тогда
			ОбъектЯдра.ВыгружатьСтатусВыполненияСценариевВФайл = Истина;
		КонецЕсли;	 
		
		ЗнПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев = ПолучитьЗначение(СтруктураJSON, "ПутьКФайлуДляВыгрузкиСтатусаВыполненияСценариев");
		...
		//для отчета jUnit
		ЗнДелатьОтчетВФорматеjUnit = ПолучитьЗначение(СтруктураJSON, "ДелатьОтчетВФорматеjUnit");
		...
		ЗнДелатьОтчетВФорматеCucumberJson = ПолучитьЗначение(СтруктураJSON, "ДелатьОтчетВФорматеCucumberJson");
		...
		//для отчета Allure
		ЗнДелатьОтчетВФорматеАллюр = ПолучитьЗначение(СтруктураJSON, "ДелатьОтчетВФорматеАллюр");
		Если ЗнДелатьОтчетВФорматеАллюр = Истина Тогда
			ОбъектЯдра.ДелатьОтчетВФорматеАллюр = Истина;
		КонецЕсли;	

		ЗнОтображатьОтчетAllureВБраузере = ПолучитьЗначение(СтруктураJSON, "ОтображатьОтчетAllureВБраузере");
		...
		ВыполнятьШагиАссинхронноЗн = ПолучитьЗначение(СтруктураJSON, "ВыполнятьШагиАссинхронно");
		Если ВыполнятьШагиАссинхронноЗн <> Неопределено Тогда
			ОбъектЯдра.ВыполнятьШагиАссинхронно = ВыполнятьШагиАссинхронноЗн;
		КонецЕсли;
		
		//ОбъектЯдра.КоманднаяСтрокаОткрытияФичи = ПолучитьЗначение(СтруктураJSON, "КоманднаяСтрокаОткрытияФичи");
