...
// Функция - Представление коллекции, переданое значение в параметре КлючЗначение рекурсивно преобразуется в строку 
//    
// Параметры:
...
//  Строка - Представление переданной коллекции преобразованное в строку 
//
&НаКлиенте
Функция ПредставлениеКоллекции(КлючЗначение, Символ = "
	|", Сдвиг = 0) Экспорт  
	...
	Для Каждого Кз Из КлючЗначение Цикл 
		Если ТипЗнч(Кз) = Тип("КлючИЗначение") Тогда
			Стр = СтрШаблон("Ключ = ""%1"": Значение = ""%2""", Кз.Ключ, ПредставлениеЗначения(Кз.Значение, Символ, Сдвиг));
		Иначе
			Стр = СтрШаблон("Значение = ""%1""", ПредставлениеЗначения(Кз, Символ, Сдвиг));
		КонецЕсли;
...
&НаКлиенте
Функция ПредставлениеЗначения(Значение, Символ, Сдвиг) 
	ЗначениеСтрокой = "";
	Если ТипЗнч(Значение) = Тип("Структура") Тогда
		ЗначениеСтрокой = СтрШаблон("Структура(
		|%1
		|)", ПредставлениеКоллекции(Значение, Символ, Сдвиг+1));
	ИначеЕсли ТипЗнч(Значение) = Тип("Массив") Тогда                      
		Врем = Новый Массив();
		Для Каждого Знч Из Значение Цикл
			Если ТипЗнч(Знч) = Тип("Структура") Тогда
				Врем.Добавить(СтрШаблон("Структура(%1)", ПредставлениеКоллекции(Знч, ", ", Сдвиг)));
			Иначе
...
Функция ЕстьВИсключаемойКоллекции(Знач ЧтоИщем, Знач КоллекцияДляПоиска) Экспорт
	...
	Для Каждого СтрокаГдеИщем Из КоллекцияДляПоиска Цикл
		Если  КонтекстЯдра.СтрокаСоответствуетШаблону(ЧтоИщем, СтрокаГдеИщем) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
