...
Процедура ВывестиДанныеОтчетаТестированияРекурсивно(НаборЗаписейXML, ЗаписьXML, РезультатыТестирования, Фабрика, Знач Уровень, Знач Контейнер = Неопределено, Знач НаборТестов = Неопределено)
	...
	ИначеЕсли РезультатыТестирования.Тип = ТипыУзловДереваТестов.Контейнер
		...
		УровеньКритичности = РезультатыТестирования.Уровень;
		Если УровеньКритичности = "" Тогда
			УровеньКритичности = ОпределитьУровеньКритичности(РезультатыТестирования);
		КонецЕсли;
		...
		СписокМеток.label.Добавить(Allure_ПолучитьМетку(Фабрика, "severity", УровеньКритичности));
...
Функция ОпределитьУровеньКритичности(РезультатыТестирования)
	
	УровеньКритичности = "normal";
	
	Для Каждого ЭлементКоллекции Из РезультатыТестирования.Строки Цикл
		
		Если ЭлементКоллекции.Свойство("Строки")Тогда
			
			УровеньКритичности = ЭлементКоллекции.Уровень;
			
		КонецЕсли;
		
	КонецЦикла;

	Возврат УровеньКритичности;
