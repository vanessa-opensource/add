...
Процедура ПроставитьПометкиВыгружаемыхДанных(СтрокиИсходногоДерева, СтрокиЗаменяемогоДерева)
	...
	КолонкаПараметрыКомпоновщика = СтрокиЗаменяемогоДерева.ВыгрузитьКолонку("ПараметрыКомпоновщика");
	СтрокиИсходногоДерева.ЗагрузитьКолонку(КолонкаПараметрыКомпоновщика, "ПараметрыКомпоновщика");
	
	КолонкаИспользоватьПараметры = СтрокиЗаменяемогоДерева.ВыгрузитьКолонку("ИспользоватьПараметры");
	СтрокиИсходногоДерева.ЗагрузитьКолонку(КолонкаИспользоватьПараметры, "ИспользоватьПараметры");
...
Процедура ДеревоМетаданныхПриАктивизацииСтрокиОбработкаОжидания()
	...
	Если НЕ ТекущиеДанные=Неопределено Тогда
		...
		Если Не Родитель=Неопределено Тогда
			Если Родитель.ПолноеИмяМетаданных="КритерииОтбора" Тогда
				Элементы.ГруппаКомповщикНастроек.ТекущаяСтраница = Элементы.ГруппаКомпоновщикПараметрыДанных;
				НастроитьПараметрыКомпоновщика();
			Иначе
				Элементы.ГруппаКомповщикНастроек.ТекущаяСтраница = Элементы.ГруппаКомпоновщикОтбор;
...
// Служит для настройки параметров комповнощика настроек
//
// Параметры:
//
&НаКлиенте
Процедура НастроитьПараметрыКомпоновщика()
	...
	Если НЕ ОпределитьПоСтрокеДереваДоступенПостроитель(ТекущаяСтрока) Тогда
		...
		ДоступностьКомпоновщика = ЛОЖЬ;
		Объект.КомпоновщикНастроекКомпоновкиДанных.Настройки.ПараметрыДанных.Элементы.Очистить();
	...
	Элементы.ГруппаКомпоновщикПараметрыДанных.Доступность = ДоступностьКомпоновщика;
...
&НаКлиенте
Процедура КомпоновщикПараметрыДанныхПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	
	ОбновитьПараметрыКомпоновщикаВТаблицеДляВыгрузки();
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПараметрыКомпоновщикаВТаблицеДляВыгрузки()
	
	ТекущаяСтрока = Элементы.ДеревоМетаданных.ТекущиеДанные;
	Если Объект.КомпоновщикНастроекКомпоновкиДанных.Настройки.ПараметрыДанных.Элементы.Количество() > 0 Тогда
		
		ТекущаяСтрока.ПараметрыКомпоновщика = Объект.КомпоновщикНастроекКомпоновкиДанных.Настройки.ПараметрыДанных;
		ТекущаяСтрока.ИспользоватьПараметры	= ИСТИНА;
		ТекущаяСтрока.Выгружать = Истина;
		
	Иначе
		
		ТекущаяСтрока.ПараметрыКомпоновщика = Неопределено;
		ТекущаяСтрока.ИспользоватьПараметры	= ЛОЖЬ;
...
Процедура СброситьНастройкиДерева(УзелДерева)
	...
	Для каждого Лист из ЛистьяДерева Цикл
		...
		Лист.ИспользоватьПараметры = Ложь;
		Лист.ПараметрыКомпоновщика = Неопределено;
