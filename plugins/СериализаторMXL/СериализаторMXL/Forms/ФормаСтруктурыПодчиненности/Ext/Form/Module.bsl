...
Процедура ОбновитьДеревоСтруктурыПодчиненности(ВозвратНазад = Ложь)
	...
	Иначе
		Сообщить(
			НСтр("ru='Объект, для которого сформирован отчет о структуре подчиненности, стал недоступен.';en='The object for which a report is generated about the structure of command, become unavailable.'"));
	КонецЕсли;
