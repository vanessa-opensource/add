...
Функция ОчиститьОбъект(Знач Объект, Знач НовыйПустойОбъект, ПропускаемыеРеквизиты)
	...
	Иначе
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке(); ОписаниеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
...
Процедура НачальнаяИнициализация() Экспорт
	...
	ДеревоМетаданных.Колонки.Добавить("ПараметрыКомпоновщика");
	ДеревоМетаданных.Колонки.Добавить("ИспользоватьПараметры");
...
Процедура ЗаполнитьОписаниеМетаданных()
	...
	//////////////////////////////////
	// Конфигурации.КритерииОтбора
	ОписаниеКласса = СтэкДереваЗначенийСтроки[0].Добавить();
	ОписаниеКласса.Класс = "КритерииОтбора";
	ОписаниеКласса.Выгружаемый = истина;
	ОписаниеКласса.Менеджер = КритерииОтбора;
	ОписаниеКласса.ДляЗапроса  = ВернутьЗначениеСоответствия("КритерийОтбора") + ".";
	ОписаниеКласса.Свойства.Добавить("Тип");
	ОписаниеКласса.ИндексКартинки = 29;
...
Функция ПодготовитьКомпоновщикДляВыгрузки(СтрокаДереваМетаданных, СтрокаПолейДляВыборки = "") Экспорт
	...
	Если ТипЗнч(СтрокаДереваМетаданных.НастройкиКомпоновщика) = Тип("ОтборКомпоновкиДанных") Тогда
		ЕстьДополнительныеОтборы = Ложь;
		Для Каждого ЭлементОтбора Из СтрокаДереваМетаданных.НастройкиКомпоновщика.Элементы Цикл
			Если ЭлементОтбора.Использование Тогда
				ЕстьДополнительныеОтборы = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	Иначе
		ЕстьДополнительныеОтборы = (СтрокаДереваМетаданных.НастройкиКомпоновщика <> Неопределено);
	КонецЕсли;
	
	ЕстьДополнительныеПараметры = Ложь;
	Если ТипЗнч(СтрокаДереваМетаданных.ПараметрыКомпоновщика) = Тип("ЗначенияПараметровДанныхКомпоновкиДанных") Тогда
		Для Каждого ЗначениеПараметра Из СтрокаДереваМетаданных.ПараметрыКомпоновщика.Элементы Цикл
	...
	ИтоговыйТекстЗапроса = ПолучитьТекстЗапросаПоСтроке(СтрокаДереваМетаданных, ЕстьДополнительныеОтборы, СтрокаПолейДляВыборки);
	
	МассивПараметров = ПараметрыКомпоновкиПоСтроке(СтрокаДереваМетаданных, ЕстьДополнительныеПараметры);
	...
	Если ЕстьДополнительныеОтборы Или ЕстьДополнительныеПараметры Тогда
		...
		Если ЕстьДополнительныеПараметры Тогда
			ПараметрыКомпоновщикаXDTO = СериализаторXDTO.ЗаписатьXDTO(СтрокаДереваМетаданных.ПараметрыКомпоновщика);
			СКДXDTO.defaultSettings.dataParameters = ПараметрыКомпоновщикаXDTO;
		КонецЕсли;
	...
	Для Каждого ЭлементМассива Из МассивПараметров Цикл
		...
		ЗаполнитьЗначенияСвойств(Параметр, ЭлементМассива, "Параметр, Значение, Использование");
	КонецЦикла;
...
Функция ПолучитьТекстЗапросаПоСтроке(СтрокаДереваМетаданных, ЕстьДопОтборы, СтрокаПолейДляВыборки = "") Экспорт
	...
	ИначеЕсли   Метаданные.КритерииОтбора.Содержит(ОбъектМетаданных) Тогда
		...
		ТекстЗапроса = ПолучитьТекстЗапросаДляКритерияОтбора(ИмяМетаданных, ОбъектМетаданных);
		Возврат ТекстЗапроса;
...
Функция ПараметрыКомпоновкиПоСтроке(СтрокаДереваМетаданных, ЕстьДополнительныеПараметры) Экспорт
	...
	Если Метаданные.КритерииОтбора.Содержит(ОбъектМетаданных) Тогда
		
		ОграничениеТипа = Новый ОписаниеТипов(ОбъектМетаданных.Тип);
		
		СтрокаКлючей = "Параметр, Значение, Использование";
	    НовыйПараметр = Новый Структура(СтрокаКлючей);
		НовыйПараметр.Параметр = Новый ПараметрКомпоновкиДанных("Тип");
		НовыйПараметр.Значение = ОграничениеТипа.ПривестиЗначение(Неопределено);
    	НовыйПараметр.Использование = Истина;		
		МассивПараметров.Добавить(НовыйПараметр);
		
	Иначе
		
		МассивПараметров = ПараметрыКомпоновкиПоУмолчанию(); 
...
Функция ПолучитьТекстЗапросаДляКритерияОтбора(ИмяМетаданных, ОбъектМетаданных)

	Если  НЕ Метаданные.КритерииОтбора.Содержит(ОбъектМетаданных) Тогда
		ВызватьИсключение "должны были передать критерий отбора";
	КонецЕсли;

	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ Ссылка ИЗ "+ИмяМетаданных+"(&Тип) КАК _";
...
Процедура СоздатьПоМетаданным(массивДанных)
	...
	Попытка
		...
		Для Каждого СтрокаТаблицыВыгрузки Из СоставПолнойВыгрузки Цикл
			...
			ИначеЕсли Метаданные.КритерииОтбора.Содержит(СтрокаДереваМетаданных.ОбъектМД) Тогда
...
Функция ПараметрыКомпоновкиПоУмолчанию()
	...
	НовыйПараметр.Значение = ДатаНачала;
	НовыйПараметр.Использование = Истина;		
	МассивПараметров.Добавить(НовыйПараметр);
	
	НовыйПараметр = Новый Структура(СтрокаКлючей);
	НовыйПараметр.Параметр = Новый ПараметрКомпоновкиДанных("ДатаОкончания");
	НовыйПараметр.Значение = ДатаОкончания;
	НовыйПараметр.Использование = Истина;
	МассивПараметров.Добавить(НовыйПараметр);
