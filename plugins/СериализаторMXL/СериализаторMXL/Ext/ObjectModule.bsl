...
Функция ЭтоПлатформа8_3_3БезРежимаСовместимости()
	...
	Если СравнитьВерсии(ВерсияПриложения, "8.3.3") >= 0 Тогда
		Версия = СтрЗаменить(Метаданные.РежимСовместимости, "Версия", "");
		Версия = СтрЗаменить(Версия, "Version", "");
		ВерсияРежимаСовместимости = СтрЗаменить(Версия, "_", ".");
		Если ВРег(ВерсияРежимаСовместимости) = ВРег("НеИспользовать") 
			Или ВРег(ВерсияРежимаСовместимости) = ВРег("DontUse") 
			Или СравнитьВерсии(ВерсияРежимаСовместимости, "8.3.3") >= 0 Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
