...
Функция ОписаниеДобавляемыхТестов(ТестКлиент)
	...
	Если ЗначениеЗаполнено(ПанельРазделов) Тогда
...
Процедура НажатьПодходящуюКнопку(ОкноПриложения)
	...
	Для Каждого ПолеФормы Из ОкноПриложения.НайтиОбъекты(Тип("ТестируемоеПолеФормы")) Цикл
		Если СтрНачинаетсяС_(НРег(ПолеФормы.ТекстЗаголовка), "данные были изменены") Тогда
			Кнопки[1].Нажать();
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Функция СтрНачинаетсяС_(Знач Строка, Знач НачалоСтроки)
