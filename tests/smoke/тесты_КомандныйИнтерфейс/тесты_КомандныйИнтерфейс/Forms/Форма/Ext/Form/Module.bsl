...
Процедура ЗаполнитьНаборТестов(НаборТестов, КонтекстЯдраПараметр) Экспорт
	...
	Для Каждого ГруппаТестов Из ОписаниеДобавляемыхТестов Цикл
		...
		Для Каждого КлючЗначение Из ГруппаТестов.Тесты Цикл
			...
			//Если Не ИсключитьИзПроверки(КлючЗначение.Ключ, ФильтрПоИмени.ИсключаемыеОперации) Тогда
			//	СписокДляСортировки.Добавить(КлючЗначение, КлючЗначение.Значение.ПредставлениеТеста);
			//КонецЕсли;
		КонецЦикла;
...
Функция ПодготовитьФильтрыПоИмени()
	...
	СтруктураПроверятьТолькоИзбранные = ЗначениеИзСтруктуры(Объект.Настройки, "ПроверятьТолькоИзбранные", Новый Структура);
...
Функция НовыйФильтрПоИмени()
	...
	Результат.Вставить("ВидыМетаданных", Новый Массив);
...
Функция ВключитьПоНавигационнойСсылке(Знач Имя, Знач ФильтрПоИмени)
	...
	//ИТакуюСсылкуПропускаем = Сегменты.Количество() <= КоличествоПравильныхСегментов;
	//Если ИТакуюСсылкуПропускаем Тогда
	//	Возврат Истина;
	//КонецЕсли;
...
Функция ВключитьПоИмени(Знач ИмяКорня, Знач Имя, Знач ФильтрПоИмени, Знач КоллекцияДляИсключения)
	...
	//ФильтрМетаданных = НайтиКоллекциюФильтрМетаданныхИзНастроек(ИмяКорня, Объект.Настройки, "ПроверятьТолькоИзбранные");
	//ЕстьОтборТолькоДляВключенияМетаданных = ЗначениеЗаполнено(ФильтрМетаданных );
	//ФильтроватьПоИзбранным = ФильтрПоИмени.ФильтроватьПоИзбранным;
...
Функция ОписаниеДобавляемыхТестов(ТестКлиент, ОсновноеОкно, ФильтрПоИмени)
	...
	Если ТипЗнч(ПанельРазделов) = Тип("ТестируемаяГруппаКомандногоИнтерфейса") Тогда
		...
		Для Каждого ТекКнопкаРаздел Из КнопкиРазделов Цикл
			...
			//Если ИсключитьИзПроверки(ТекстЗаголовка, ИсключаемыеРазделы) Тогда
			//	Продолжить;
			//КонецЕсли;
