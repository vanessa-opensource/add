...
&НаКлиенте
Процедура ДобавитьИсключения(Знач ИсключаемыеОперации, ИмяНастройки, ИмяДляШаблона)
	
	Если ВидМетаданныхИсключенИзТестирования(ИмяНастройки) Тогда 
		
		ИсключаемыеОперации.Добавить(ИмяДляШаблона);
		
	ИначеЕсли ЕстьНастройка(ИмяНастройки) Тогда
		
		Для Каждого Параметр Из Объект.Настройки[ИмяНастройки] Цикл
			ИсключаемыеОперации.Добавить(ИмяДляШаблона + Параметр);
		КонецЦикла;
