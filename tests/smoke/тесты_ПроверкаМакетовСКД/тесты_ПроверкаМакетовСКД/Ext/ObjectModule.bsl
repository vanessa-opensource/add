...
Функция НастройкиПоУмолчанию()
	...
	Результат.Вставить("ПрефиксОбъектов", "");
...
Процедура ДобавитьОбщиеМакеты(НаборТестов)
	...
	мОбщиеМакеты = ОбщиеМакеты(Настройки.ОтборПоПрефиксу, Настройки.ПрефиксОбъектов);
...
Процедура ДобавитьМакетМетаданных(НаборТестов)
	...
	ГруппыМакетовМетаданных = ГруппыМакетовМетаданных(Настройки.ОтборПоПрефиксу, Настройки.ПрефиксОбъектов);
...
Функция ОбщиеМакеты(ОтборПоПрефиксу, ПрефиксОбъектов) Экспорт
	...
	Для Каждого ОбщийМакет Из Метаданные.ОбщиеМакеты Цикл
		...
		Если ОбщийМакет.ТипМакета <> Метаданные.СвойстваОбъектов.ТипМакета.СхемаКомпоновкиДанных Тогда
			Продолжить;
		КонецЕсли;
		Если ОтборПоПрефиксу И Не ИмяСодержитПрефикс(ОбщийМакет.Имя, ПрефиксОбъектов) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ГруппыМакетовМетаданных(ОтборПоПрефиксу, ПрефиксОбъектов) Экспорт
	...
	Для Каждого ПроверяемыйОбъект Из ПроверяемыеОбъекты Цикл
		...
		Для Каждого ТекОбъект Из Метаданные[ПроверяемыйОбъект] Цикл
			Если ОтборПоПрефиксу И Не ИмяСодержитПрефикс(ТекОбъект.Имя, ПрефиксОбъектов) Тогда
				Продолжить;
			КонецЕсли;
...
//   Булево - Истина если в имени есть префикс
//
Функция ИмяСодержитПрефикс(Имя, Префикс) Экспорт 
