...
### Есть возможность настройки, с помощью файла конфигурации.
На текущий момент тест поддерживает 2 опции настройки:
- ПроверкаЧтенияНеАдминистраторами - старый вариант. не рекомендуется
- SmokeNonAdminReading - Новый вариант, с поддерпжкой исключения объектов методанных из проверки

#### Пример настройки опции SmokeNonAdminReading

```javascript
"SmokeNonAdminReading" :{
        "DefaultRole" : [
            "Администратор",
            "ПолныеПрава"
        ],
		"ExclusionObjects" : [
			...
			"РегистрСведений.МойРегистр1",
			"РегистрСведений.МойРегистр2"
        ]
```
...
#### Для исключения объекта из проверки необходимо добавить в файл настроек следующее:
