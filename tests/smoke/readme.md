    ...
    - в обоих настройках возможен поиск 2х видов
      ...
      - возможен поиск по шаблону со звездочкой - `Счет*` или `*Счет` или `*Счет*` или `Счет*Реестр`
	   - вложенный массив с ключом `ПроверятьТолькоЭтиОбъекты` позволяет ограничить роли для проверки, например если задать такой массив 
			...
			```
			будут проверяться все справочники и документы начинающиеся с имени PTG, другие объекты метаданных не будут проверяться. Данная настройка не исключает настройку `ИсключенияПоИмениМетаданных` т.е. если в списке справочников есть справочник попадающий под исключение, он будет исключен из проверки. Данная настройка полезна в тех случаях, если у нас есть типовые объекты которые нет смысла проверять, а все "свой" объекты (т.е. те которые добавлены в типовую конфигурацию) имеют префикс.
      - `ДополнятьЗависимымиОбъектами` - флаг означающий, что дополнительно будут проверяться зависимые объекты метаданных.
