...
- ключ настройки `ПроведениеДокументов`
    ...
    - во этой коллекции возможен поиск 2х видов
      - возможен поиск по полному наименованию - `СчетФактура`
      - возможен поиск по шаблону со звездочкой - `Счет*` или `*Счет` или `*Счет*` или `Счет*Реестр`
	- `СравнениеДвиженийБезНомераСтроки` по умолчанию истина, данный флаг отвечает за то что движения до и после проведения документа будут вверяться без учета порядка строк
	- `Отбор` массив структур при меняется для отбора документов, например:
		```json
		  "Отбор": [
			{
			  "Дата": {
		...
		Попустимые операторы сравнения: 
		* lt: меньше чем
		* le: меньше или равно
