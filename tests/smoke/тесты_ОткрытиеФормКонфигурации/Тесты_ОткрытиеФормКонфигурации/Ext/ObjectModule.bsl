...
Функция ПолучитьСуществующуюЗаписьРегистраИВернутьКлючЗаписи(ИД, НужноИсключениеЕслиНеНайденоЗаписей, ДопОписаниеДляИсключения)
	Выборка = РегистрыСведений[ИД].Выбрать();
	Успешно = Выборка.Следующий();
	Если Не Успешно Тогда
		...
		КонецЕсли;
	КонецЕсли;
	КлючЗаписи = СоздатьПустойКлючЗаписиРегистраСведений(ИД);
...
Функция СоздатьПустойКлючЗаписиРегистраСведений(ИД)
	
	КлючЗаписи = Новый Структура;
	
	Измерения = Метаданные.РегистрыСведений[ИД].Измерения;
