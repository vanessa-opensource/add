...
Функция ПолучитьСписокИсключений_БизнесПроцессы_Списки() Экспорт
	...
	Результат = ПолучитьИсключения(ВидыМетаданных.БизнесПроцессы, ВидыПроверок.Списки);
	Если ЗначениеЗаполнено(Результат) Тогда
		Возврат Результат;
	КонецЕсли;
...
Функция ПолучитьСписокИсключений_БизнесПроцессы_Существующие() Экспорт
	...
	Результат = ПолучитьИсключения(ВидыМетаданных.БизнесПроцессы, ВидыПроверок.Существующие);
	Если ЗначениеЗаполнено(Результат) Тогда
		Возврат Результат;
	КонецЕсли;
...
Функция ПолучитьСписокИсключений_БизнесПроцессы_Новые() Экспорт
	...
	Результат = ПолучитьИсключения(ВидыМетаданных.БизнесПроцессы, ВидыПроверок.Новые);
	Если ЗначениеЗаполнено(Результат) Тогда
		Возврат Результат;
	КонецЕсли;
...
Функция СоздатьБизнесПроцесс(ИД, Наименование, ПроверяемоеПравоДоступа = Неопределено)
...
Функция СоздатьБизнесПроцессИВернутьСсылку(ИД, Наименование, ПроверяемоеПравоДоступа = Неопределено) Экспорт
	Элем = СоздатьБизнесПроцесс(ИД, Наименование, ПроверяемоеПравоДоступа);
	Возврат ?(Элем = Неопределено, Неопределено, Элем.Ссылка);
КонецФункции
#КонецОбласти
