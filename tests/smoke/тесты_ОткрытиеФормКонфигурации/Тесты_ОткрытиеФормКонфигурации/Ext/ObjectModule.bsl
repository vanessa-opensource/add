...
Функция СоздатьЭлемент(ИД, Наименование, ПроверяемоеПравоДоступа = Неопределено)
	...
	Если Не РазрешеноИзменятьОбъект(Мета) Тогда
		ВывестиСообщение(СтрШаблон("Метаданные ""%1"" пропущены", Мета.ПолноеИмя()), УровеньЖурналаРегистрации.Информация);
		Возврат Неопределено;
	КонецЕсли;
...
Функция РазрешеноИзменятьОбъект(МетаОбъект)
