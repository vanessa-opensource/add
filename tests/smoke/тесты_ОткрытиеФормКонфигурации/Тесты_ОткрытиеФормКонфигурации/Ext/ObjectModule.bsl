...
Функция СоздатьНаборНастроекПоУмолчанию() Экспорт
	...
	Рез.Вставить("ИсключитьФормыЗависящиеОтОтключенныхФункциональныхОпций", Ложь);
...
Процедура ДобавитьИсключенияПоОтключеннымФункциональнымОпциям(ВидМетаданного, СписокЗначений)
	...
	Если Не ФормыОтключенныеЧерезФункциональныеОпции.Свойство(ВидМетаданного) Тогда
		...
		ТаблицаМетаданныхФО = Новый ТаблицаЗначений;
		ТаблицаМетаданныхФО.Колонки.Добавить("ИмяМетаданного");
		ТаблицаМетаданныхФО.Колонки.Добавить("ДоступноПоФО");
			...
			Если ТипЗнч(Значение) = Тип("Булево") Тогда
				Для каждого СтрОбъект Из ФО.Состав Цикл
					...
					Если Метаданные[ВидМетаданного].Содержит(СтрОбъект.Объект) Тогда
						ИмяМетаданного = СтрОбъект.Объект.Имя;
						СтрокаТЗ =  ТаблицаМетаданныхФО.Найти(ИмяМетаданного, "ИмяМетаданного");
						Если СтрокаТЗ = Неопределено Тогда
							НоваяСтрокаТЗ = ТаблицаМетаданныхФО.Добавить();
							НоваяСтрокаТЗ.ИмяМетаданного = ИмяМетаданного;
							НоваяСтрокаТЗ.ДоступноПоФО = Ложь;
							
							СтрокаТЗ = НоваяСтрокаТЗ;
						КонецЕсли;
						СтрокаТЗ.ДоступноПоФО = Макс(СтрокаТЗ.ДоступноПоФО, Значение);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;		

		МассивНайденныхИсключений = ТаблицаМетаданныхФО.НайтиСтроки(Новый Структура("ДоступноПоФО", Ложь));
		Для Каждого ЭлементМассива Из МассивНайденныхИсключений Цикл
			СписокМетаданныхФО.Добавить(ЭлементМассива.ИмяМетаданного);	
		КонецЦикла;	
...
Функция РазрешеноИзменятьОбъектВМоделиСервиса(МетаОбъект)
	...
	Попытка
		...
		ПодсистемаСуществует = МОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность");
	Исключение
