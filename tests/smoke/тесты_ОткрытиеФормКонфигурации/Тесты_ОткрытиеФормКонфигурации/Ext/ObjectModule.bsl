...
Функция ПолучитьИсключения(Знач ВидМетаданного, Знач ВидПроверки)
	...
	ДобавитьИсключенияПоОбъектамБезФорм(ВидМетаданного, Результат);
...
Процедура ДобавитьИсключенияПоОбъектамБезФорм(ВидМетаданного, СписокЗначений)
	ИсключитьОбъектыБезФорм = Истина;
	ЕстьНастройка = Настройки.Свойство("ИсключитьОбъектыБезФорм", ИсключитьОбъектыБезФорм);
	Если ЕстьНастройка И Не ИсключитьОбъектыБезФорм
		ИЛИ (ТипЗнч(СписокЗначений) = Тип("Булево") И Не СписокЗначений) Тогда
	...
	ОбъектыБезФорм = Новый Структура;

	Если Не ОбъектыБезФорм.Свойство(ВидМетаданного) Тогда
		СписокМетаданныхБезФорм = Новый СписокЗначений;
		ОбъектыБезФорм.Вставить(ВидМетаданного, СписокМетаданныхБезФорм);

		Для каждого ОбъектМетаданных Из Метаданные[ВидМетаданного] Цикл
			
			ФормыОМ = ОбъектМетаданных.Формы;
			
			Если ФормыОМ.Количество() = 0 Тогда
				СписокМетаданныхБезФорм.Добавить(ОбъектМетаданных.Имя);
			КонецЕсли;
	...
	СписокМетаданного = Неопределено;
	Если ОбъектыБезФорм.Свойство(ВидМетаданного, СписокМетаданного) Тогда
