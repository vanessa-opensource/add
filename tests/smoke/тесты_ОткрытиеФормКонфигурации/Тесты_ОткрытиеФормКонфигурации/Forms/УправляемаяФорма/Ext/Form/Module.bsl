	...
	Для Каждого ОписаниеМетаданного Из ОтобранныеМетаданные Цикл 
		...
		Если МетаОбъект.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.Независимый Тогда
			...
			Если ПравоДоступа("Изменение", МетаОбъект) Тогда
				...
				НастройкиТеста = НастройкиТеста(ТолькоУправляемыеФормы,
						СписокИсключений_Новые, ИменаОсновныхФорм_Объекты_Сервер,
						"ТестДолжен_ОткрытьФормуНовойЗаписиРегистраСведений", "Новые");
			...
			НастройкиТеста = НастройкиТеста(ТолькоУправляемыеФормы,
					СписокИсключений_Существующие, ИменаОсновныхФорм_Объекты_Сервер,
					"ТестДолжен_ОткрытьФормуСуществующейЗаписиРегистраСведений", "Существующие");
...
Процедура ТестироватьФормуЗаписиРегистраСведений(Мета_ПолноеИмя, КлючЗаписи) 
	...
	ПараметрыФормы = Новый Структура("КлючЗаписи", КлючЗаписи);
	ТестироватьФорму(Мета_ПолноеИмя, ПараметрыФормы);
...
Процедура ТестироватьФормуНаКлиентеТестирования(ПолноеИмяФормы, ПараметрыФормы)
	...
	Исключение
		...
		ПолныйТекстОшибки = СтрШаблон("Не удалось выполнить команду %1 на тест-клиенте.
			|Возможно, сеанс тест-клиента заблокирован модальным окном
			|%2", ПарамНавигационнаяСсылка, ПолныйТекстОшибки);

		ЗаписатьПредупреждениеВЖурналРегистрацииСервер(ПолныйТекстОшибки);
...
Функция ИмяТеста()
	...
	Возврат "Тесты ""Открытие форм конфигурации""";
КонецФункции
