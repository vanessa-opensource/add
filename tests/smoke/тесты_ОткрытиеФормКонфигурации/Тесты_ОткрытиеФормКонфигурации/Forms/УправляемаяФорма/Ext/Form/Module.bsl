...
&НаКлиенте
Перем ОткрываемФормыНаКлиентеТестирования; //Ложь - открываем в текущем сеансе 1С, Истина - используем клиенты тестированияы
...
Процедура ДобавитьТестыДляДокументовСервер(ОписанияТестов, ТолькоУправляемыеФормы, 
	...
	Для Каждого МетаОбъект Из менеджерМетаданного Цикл
		...
		Если ПравоДоступа("ИнтерактивноеДобавление", МетаОбъект) Тогда
			...
			ДобавитьТестыПереданныхВидовФормПоОдномОбъектуМетаданных(НаборОписанийТестов, ОсновнойОбъект, МетаОбъект, ТолькоУправляемыеФормы, СписокИсключений, ИменаОсновныхФорм, имяТеста, ПрефиксПредставленияТеста);
		КонецЕсли;		
...
&НаСервере
Процедура ДобавитьТестыДляОтчетовОбработокСервер(ОписанияТестов, ТолькоУправляемыеФормы, Знач ДобавлятьОбработки, Знач НастройкаГруппировки)
	ОсновнойОбъект = Объект();
...
&НаСервере
Функция ДобавитьТестыДляБизнесПроцессовСервер(ОписанияТестов, ТолькоУправляемыеФормы, 
				ИменаОсновныхФорм_Объекты_Сервер, мИменаОсновныхФорм_Списки_Сервер, НастройкаГруппировки)
...
Функция ДобавитьИменаМетаФормДляОдногоОбъектаМетаданного(CписокИменМетаФорм, ОсновнойОбъект, МетаОбъект, ТолькоУправляемыеФормы, списокИсключений, ИменаОсновныхФорм)
	...
	Если ПравоДоступа("Просмотр", МетаОбъект) Тогда
		...
		Для каждого МетаФорма Из МетаФормы Цикл
			...
			Если СписокИсключений.НайтиПоЗначению(Сред(МетаФорма_ПолноеИмя, Найти(МетаФорма_ПолноеИмя, ".") + 1)) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
	...
	Если Не ФормыДобавлены Тогда
		CписокИменМетаФорм.Добавить(Мета_ПолноеИмя);
		Результат = Истина;
	КонецЕсли;	
...
Функция ПолучитьОбъектСуществующегоЭлементаСправочникаИВернутьСсылкуСервер(Мета_ПолноеИмя)
	...
	Если МетаданныеОбъекта.Иерархический 
		...
		И МетаданныеОбъекта.ОсновнаяФормаГруппы <> Неопределено Тогда
		
		ИмяОсновнойФормыГруппы = МетаданныеОбъекта.ОсновнаяФормаГруппы.Имя;
		Если Найти(Мета_ПолноеИмя, ИмяОсновнойФормыГруппы) <> 0 Тогда
			Отбор.Вставить("ЭтоГруппа", "Истина");
		КонецЕсли; 
	КонецЕсли; 
	
	Элем = ОсновнойОбъект.ПолучитьОбъектСуществующегоЭлементаСправочникаИВернутьСсылку(ИД, Ложь, "ТестДолжен_ОткрытьФормуСуществующегоЭлементСправочника", Отбор);
	Возврат Элем;
КонецФункции
...
Процедура ТестДолжен_ОткрытьФормуСуществующегоБизнесПроцесса(Мета_ПолноеИмя) Экспорт
	...
	Если ЗначениеЗаполнено(СуществующийЭлемент) Тогда
		ТестироватьФормуСсылочногоОбъекта(Мета_ПолноеИмя, СуществующийЭлемент);
	Иначе
		СоздатьБизнесПроцессИВернутьСсылкуСервер(Мета_ПолноеИмя);
		Если ЗначениеЗаполнено(CоздаваемыйЭлемент) Тогда
			ТестироватьФормуСсылочногоОбъекта(Мета_ПолноеИмя, CоздаваемыйЭлемент);	
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьОбъектСуществующегоБизнесПроцессаИВернутьСсылкуСервер(Мета_ПолноеИмя)
	...
	ОсновнойОбъект = Объект();
	Элем = ОсновнойОбъект.ПолучитьОбъектСуществующегоБизнесПроцессаИВернутьСсылку(ИД, Ложь, "ТестДолжен_ОткрытьФормуСуществующегоБизнесПроцесса");
	Возврат Элем;
КонецФункции

&НаСервере
Процедура СоздатьБизнесПроцессИВернутьСсылкуСервер(Мета_ПолноеИмя)
	ИД = ПолучитьВидМетаданного(Мета_ПолноеИмя);

	ОсновнойОбъект = Объект();
	CоздаваемыйЭлемент = ОсновнойОбъект.СоздатьБизнесПроцессИВернутьСсылку(ИД, "существующий");
КонецПроцедуры
...
Функция ПолучитьВидМетаданного(Мета_ПолноеИмя)
	...
	Иначе	
		// Если нет формы и мы открываем по имени справочника, то в качестве его ИД просто берем 2ю часть наименования //
...
Процедура ТестироватьФормуНаКлиентеТестирования(ПолноеИмяФормы, ПараметрыФормы) Экспорт
	...
	Попытка
		...
		Если ТипЗнч(ПараметрыФормы) = Тип("Структура") И ПараметрыФормы.Свойство("Ключ",СсылкаНаОбъект) И ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
...
Процедура ЗакрытьВсеОткрытыеОкна()
	...
	//ОкнаПредупреждений = ТестКлиент.НайтиОбъекты(Тип("ТестируемоеОкноКлиентскогоПриложения"), НСтр("ru = 'Предупреждение безопасности'"));
