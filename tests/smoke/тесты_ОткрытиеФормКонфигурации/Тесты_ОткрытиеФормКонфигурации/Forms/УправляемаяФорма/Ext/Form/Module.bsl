...
Процедура ДобавитьТестыДляДокументовСервер(ОписанияТестов, НастройкиГруппыТестов,
	...
	Для Каждого ОписаниеМетаданного Из ОтобранныеМетаданные Цикл
		...
		//НастройкиТеста = НовыеНастройкиТеста();
		...
		Если ПравоДоступа("ИнтерактивноеДобавление", МетаОбъект) Тогда
			...
			//	ТолькоУправляемыеФормы, СписокИсключений_Новые, ИменаОсновныхФорм_Объекты_Сервер, имяТеста, "Новые", КонтекстЯдра);
			...
			ДобавитьТестыПереданныхВидовФормПоОдномОбъектуМетаданныхНастройки(
		...
		ДобавитьТестыПереданныхВидовФормПоОдномОбъектуМетаданных(НаборОписанийТестов, ОсновнойОбъект, МетаОбъект,
			ТолькоУправляемыеФормы, СписокИсключений_Существующие, ИменаОсновныхФорм_Объекты_Сервер, имяТеста, "Существующие", КонтекстЯдра);

		имяТеста = "ТестДолжен_ОткрытьФормуПоПолномуИмениФормы";

		ДобавитьТестыПереданныхВидовФормПоОдномОбъектуМетаданных(НаборОписанийТестов, ОсновнойОбъект, МетаОбъект,
			ТолькоУправляемыеФормы, СписокИсключений_Списки, мИменаОсновныхФорм_Списки_Сервер, имяТеста, "Списки", КонтекстЯдра);
...
// TODO ДобавитьТестыПереданныхВидовФормПоОдномОбъектуМетаданныхНастройки вместо этого метода 
&НаСервере
...
&НаСервере
Функция ДобавитьТестыПереданныхВидовФормПоОдномОбъектуМетаданныхНастройки(ОписанияТестов, ОсновнойОбъект, МетаОбъект,
	...
	Успешно = ДобавитьИменаМетаФормДляОдногоОбъектаМетаданногоНастройки(СписокИменМетаФорм, ОсновнойОбъект, МетаОбъект,
		НастройкиТеста, КонтекстЯдра);
		
	ДобавитьТестПоПереданнымМетаФормамСсылочногоОбъектаСерверНастройки(ОписанияТестов, СписокИменМетаФорм, НастройкиТеста);

	Возврат Успешно;
КонецФункции

// TODO ДобавитьТестПоПереданнымМетаФормамСсылочногоОбъектаСерверНастройки вместо этого метода 
&НаСервере
...
&НаСервере
Процедура ДобавитьТестПоПереданнымМетаФормамСсылочногоОбъектаСерверНастройки(ОписанияТестов, СписокИменМетаФорм, НастройкиТеста)
...
// TODO ДобавитьИменаМетаФормДляОдногоОбъектаМетаданногоНастройки вместо этого метода 
...
&НаСервереБезКонтекста
Функция ДобавитьИменаМетаФормДляОдногоОбъектаМетаданногоНастройки(СписокИменМетаФорм, ОсновнойОбъект, МетаОбъект,
		НастройкиТеста, КонтекстЯдра)
...
Функция ВключаемПоИмени(Знач КлючМетаданных, Знач ИмяМетаданного, Знач Настройки, Знач КонтекстЯдра = Неопределено)
	...
	ФмльтрМетаданных = НайтиКоллекциюФмльтрМетаданныхИзНастроек(КлючМетаданных, Настройки, "ПроверятьТолькоУказанные");
	...
	ФмльтрИсключаемыхМетаданных = НайтиКоллекциюФмльтрМетаданныхИзНастроек(КлючМетаданных, Настройки, "ИсключатьПоИмени"); // TODO
...
Функция НайтиКоллекциюФмльтрМетаданныхИзНастроек(Знач КлючМетаданных, Знач Настройки, Знач ИмяОбщейКоллекции)
	...
	//ВсеНастройкиВОбщейКоллекции = Неопределено;
	//Если Не ОбщаяКоллекция.Свойство(КлючМетаданных, ВсеНастройкиВОбщейКоллекции) Или Не ЗначениеЗаполнено(ВсеНастройкиВОбщейКоллекции)  Тогда
	//	ВсеНастройкиВОбщейКоллекции = Новый Структура;
	//КонецЕсли;
	
	//ФмльтрМетаданных = Неопределено;
	//Если Не ВсеНастройкиВОбщейКоллекции.Свойство(ИМЯ_ОБЩЕЙ_НАСТРОЙКИ, ФмльтрМетаданных) Или Не ЗначениеЗаполнено(ФмльтрМетаданных)  Тогда
	//	ФмльтрМетаданных = Новый Массив;
	//КонецЕсли;
...
Функция ОтобратьМетаданные(КлючМетаданных, МенеджерМетаданного, Настройки, ОсновнойОбъект)
	...
	Для Каждого МетаОбъект Из МенеджерМетаданного Цикл
		...
		Если ОсновнойОбъект.ЭтоУстаревшийМетаОбъектДляУдаления(МетаОбъект)
			//Или ИсключаемПоИмени(ИмяМетаОбъекта, СписокИсключений)
			Или Не ВключаемПоИмени(КлючМетаданных, ИмяМетаОбъекта, Настройки)
			Или Не ПравоДоступа("Просмотр", МетаОбъект) Тогда
...
Функция НовыеНастройкиГруппыТестов()
	...
	Результат.Вставить("КоллекцияПроверятьТолькоУказанные", Объект.Настройки.ПроверятьТолькоУказанные);
