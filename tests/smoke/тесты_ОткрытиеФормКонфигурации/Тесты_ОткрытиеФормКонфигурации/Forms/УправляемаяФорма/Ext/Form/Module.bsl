...
//Ложь - открываем в текущем сеансе 1С, Истина - используем клиенты тестированияы
&НаКлиенте
Перем ОткрываемФормыНаКлиентеТестирования;
...
Функция ДобавитьТестыПереданныхВидовФормПоОдномОбъектуМетаданных(ОписанияТестов, ОсновнойОбъект, МетаОбъект,
	...
	Успешно = ДобавитьИменаМетаФормДляОдногоОбъектаМетаданного(CписокИменМетаФорм, ОсновнойОбъект, МетаОбъект,
		ТолькоУправляемыеФормы, СписокИсключений, ИменаОсновныхФорм);
	ДобавитьТестПоПереданнымМетаФормамСсылочногоОбъектаСервер(ОписанияТестов, CписокИменМетаФорм, имяТеста,
		ПрефиксПредставленияТеста);
...
&НаСервере
Процедура ДобавитьТестПоПереданнымМетаФормамСсылочногоОбъектаСервер(ОписанияТестов, CписокИменМетаФорм, имяТеста,
		ПрефиксПредставленияТеста)
...
Процедура УдалитьСозданныеОбъекты()
	...
	Если CоздаваемыйЭлемент.Метаданные().Владельцы.Количество() > 0
		И Не CоздаваемыйЭлемент.Владелец.Пустая() Тогда
		
		Попытка
			CозданныйОбъектВладелец = CоздаваемыйЭлемент.Владелец.ПолучитьОбъект();
			CозданныйОбъектВладелец.Удалить();
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				...
				CозданныйОбъектВладелец,
				CоздаваемыйЭлемент.Владелец,
	...
	Попытка
		...
		СозданныйОбъект = CоздаваемыйЭлемент.ПолучитьОбъект();
...
Процедура ЗакрытьВсеОткрытыеОкна()
	...
	Для Каждого ТекОкно Из ОткрытыеОкна Цикл
		...
		Если ТипЗнч(ТекОкно) = Тип("ТестируемоеОкноКлиентскогоПриложения")
				И ТекОкно.Заголовок = НСтр("ru = '1С:Предприятие'") Тогда
