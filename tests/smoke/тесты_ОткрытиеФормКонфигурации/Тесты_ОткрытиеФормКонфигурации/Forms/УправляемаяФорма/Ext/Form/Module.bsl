...
//Ложь - открываем в текущем сеансе 1С, Истина - используем клиенты тестирования
&НаКлиенте
...
Процедура СоздатьЭлементИВернутьСсылкуСервер(Мета_ПолноеИмя)
	...
	ОсновнойОбъект = Объект();
	СоздаваемыйЭлемент = ОсновнойОбъект.СоздатьЭлементИВернутьСсылку(ИД, "существующий");
КонецПроцедуры
...
Процедура СоздатьБизнесПроцессИВернутьСсылкуСервер(Мета_ПолноеИмя)
	...
	ОсновнойОбъект = Объект();
	СоздаваемыйЭлемент = ОсновнойОбъект.СоздатьБизнесПроцессИВернутьСсылку(ИД);
КонецПроцедуры
