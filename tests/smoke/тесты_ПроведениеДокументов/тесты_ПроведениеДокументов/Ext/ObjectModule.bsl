...
Процедура Тест_ПровестиДокумент(ДокументСсылка) Экспорт
	...
	// ОтменаПроведения нужно делать т.е. есть документы у которых режим проведения стоит "Удалять автоматически при отмене проведения"
	// и без отмеры проведения старые движения не будут удаляться и тесты не проходят. Однако при перепроведении в интерфейсе, старые движения удаляются
	МетаОбъект = ДокументСсылка.Метаданные();
	Если МетаОбъект.УдалениеДвижений = Метаданные.СвойстваОбъектов.УдалениеДвижений.УдалятьАвтоматическиПриОтменеПроведения Тогда 
