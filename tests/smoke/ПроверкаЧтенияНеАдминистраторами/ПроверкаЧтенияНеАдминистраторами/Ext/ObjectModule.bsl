...
Процедура ЗагрузитьНастройки()
	...
	ПутьНастройкиВерсия1 = "ПроверкаЧтенияНеАдминистраторами";
	ПутьНастройкиВерсия2 = "SmokeNonAdminReading";
	ПлагинНастроек = КонтекстЯдра.Плагин("Настройки");
	ПлагинНастроек.Инициализация(КонтекстЯдра);
	МассивПривилегированныеРоли = ПлагинНастроек.ПолучитьНастройку(ПутьНастройкиВерсия1);
	ИсключенныеОбъекты = Новый Массив;
	Если МассивПривилегированныеРоли <> Неопределено Тогда	
		Сообщение = "Настройка ПроверкаЧтенияНеАдминистраторами является устаревшей. Используйте настройку SmokeNonAdminReading.";
		ЗаписьЖурналаРегистрации("ПроверкаЧтенияНеАдминистраторами",УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
	Иначе
		НастройкиТеста = ПлагинНастроек.ПолучитьНастройку(ПутьНастройкиВерсия2);
		Если НастройкиТеста.Свойство("DefaultRole") Тогда
			МассивПривилегированныеРоли = НастройкиТеста.DefaultRole
		КонецЕсли;
		Если НастройкиТеста.Свойство("ExclusionObjects") Тогда	
			ИсключенныеОбъекты = НастройкиТеста.ExclusionObjects;		
		КонецЕсли; 
	КонецЕсли; 
