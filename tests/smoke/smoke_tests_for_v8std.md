...
- [Тесты](#тесты)
    ...
    - [Описание тестов](#описание)
        ...
        - [Тесты для проверки объектов метаданных на соответствие внутренним стандартам](#тесты_внутренние_стандарты)
			...
			- [2.8. Проверка прав на чтение объектов с ролями RLS и без](#Тесты_ЕстьПравоНаЧтениеПерекрывающееПравоRLS)
            - [2.9. Проверка макетов СКД внешних отчетов и обработок](#Тесты_ПроверкаМакетовСКДВнешнихОтчетовОбработок)
            - [2.10. Проверка объектов метаданных конфигурации поставщика](#Тесты_ПроверкаОбъектовМетаданныхКонфигурацииПоставщика)
            - [2.11. Проверка каноничности префикса новых объектов и реквизитов](#Тесты_ПроверкаКаноничностиПрефиксаОбъектовСвойств)
            - [2.12. Проверка индексирования устаревших объектов метаданных](#Тесты_ПроверкаИндексированияУстаревшихОбъектовМетаданных)
            - [2.13. Проверка устаревших типов данных](#Тесты_ПроверкаУстаревшихТиповДанных)
            - [2.14. Проверка авторегистрации планов обмена](#Тесты_ПроверкаАвтоРегистрацииПлановОбмена)
            - [2.15. Переход на управляемые формы](#Тесты_ПереходНаУправляемыеФормы)
            - [2.16. Проверка периодичности нумерации документов](#Тесты_ПроверкаПериодичностиНумерацииДокументов)
            - [2.17. Проверка регистров для отчета "Движения документа"](#Тесты_ПроверкаРегистровДляОтчетаДвиженияДокумента)
...
<a id="markdown-Тесты_ЕстьПравоНаЧтениеПерекрывающееПравоRLS" name="Тесты_ЕстьПравоНаЧтениеПерекрывающееПравоRLS"></a>
+ **[2.8.](#тесты) Проверка перекрывающих прав на чтение объектов с RLS** 
([Тесты_ЕстьПравоНаЧтениеПерекрывающееПравоRLS](./Тесты_ЕстьПравоНаЧтениеПерекрывающееПравоRLS))

    + В отличии от [Тесты_ПроверкаПравНаЧтениеОбъектовRLS](#Тесты_ПроверкаПравНаЧтениеОбъектовRLS) данный тест реагирует только на те объекты у которых есть две роли, одна с RLS на чтение, вторая с правом на чтения но **без** RLS, за исключением неосновных ролей. 
    + Настройки:
        - [Используется](#Настройки_Используется)
        - [Выводить исключения](#Настройки_ВыводитьИсключения)
        - [Исключения из проверок](#Настройки_ИсключенияИзПроверок)
...
+ **[2.9.](#тесты) Проверка макетов СКД внешних отчетов и обработок** 
...
+ **[2.10.](#тесты) Проверка объектов метаданных конфигурации поставщика** 
...
+ **[2.11.](#тесты) Проверка каноничности префикса новых объектов и реквизитов** 
...
+ **[2.12.](#тесты) Проверка индексирования устаревших объектов метаданных** 
...
+ **[2.13.](#тесты) Проверка устаревших типов данных** 
...
+ **[2.14.](#тесты) Проверка авторегистрации планов обмена** 
...
+ **[2.15.](#тесты) Переход на управляемые формы** 
...
+ **[2.16.](#тесты) Проверка периодичности нумерации документов** 
...
+ **[2.17.](#тесты) Проверка регистров для отчета ["Движения документа"](https://its.1c.ru/db/bsp246doc#content:50:1)** 
