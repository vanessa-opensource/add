...
Процедура ЗагрузитьНастройки()
    КлючТеста = "ФормированиеПечатныхФорм";
	...
	Иначе
		...
		//КонтекстЯдра.ВывестиСообщение("Настройки.ИсключенияПоИдентификатору.Количество() " + Настройки.ИсключенияПоИдентификатору.Количество());
		//Для Каждого ИсключаемыйИдентификатор Из Настройки.ИсключенияПоИдентификатору Цикл
		//	КонтекстЯдра.ВывестиСообщение("ИсключаемыйИдентификатор " + ИсключаемыйИдентификатор);
		//КонецЦикла;
	КонецЕсли;
...
Процедура ДобавитьТестыДляДокумента(НаборТестов, МетаОбъект)
	...
	Для Каждого ТекущаяКомандаПечати Из КомандыПечати Цикл
		...
		ИдентификаторДляСравнения = НРег(ТекущаяКомандаПечати.Идентификатор);
		...
		Исключать = Ложь;
		Для Каждого ИсключаемыйИдентификатор Из Настройки.ИсключенияПоИдентификатору Цикл
			Если КонтекстЯдра.СтрокаСоответствуетШаблону(ИдентификаторДляСравнения, НРег(ИсключаемыйИдентификатор)) Тогда
				Исключать = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если Исключать Тогда
			КонтекстЯдра.Отладка("Пропускаем из-за исключения по идентификатору - " + ПредставлениеТеста);
			Продолжить;
		КонецЕсли;
