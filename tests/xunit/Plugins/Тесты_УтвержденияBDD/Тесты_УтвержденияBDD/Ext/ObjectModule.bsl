...
Процедура ТестДолжен_Проверить_ЧтоМетодСПростымПараметромСтрока_ВыбрасываетИсключение() Экспорт
	
	Контекст = Новый Структура;
	Ожидаем.Что(Контекст).Метод("Вставить", "Плохой Ключ")
		.ВыбрасываетИсключение("Задано неправильное имя атрибута структуры");
	
КонецПроцедуры

Процедура ТестДолжен_Проверить_ЧтоМетодСПростымПараметромЧисло_ВыбрасываетИсключение() Экспорт
	
	Контекст = Новый Структура;
	Ожидаем.Что(Контекст).Метод("Вставить", 10)
		.ВыбрасываетИсключение("Задано неправильное имя атрибута структуры");
