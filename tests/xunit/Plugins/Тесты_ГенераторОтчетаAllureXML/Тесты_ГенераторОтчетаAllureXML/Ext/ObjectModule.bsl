...
Процедура ТестДолжен_СоздатьОтчет() Экспорт
	...
	ГенераторОтчета = КонтекстЯдра.Плагин("ГенераторОтчетаAllureXML");
...
Процедура ТестДолжен_Экспортировать_ОтчетОТестированииПоПутиФайла() Экспорт
	...
	ИмяФайла = КаталогОтчета + "allure.xml";
	...
	ФайлыОтчета = НайтиФайлы(КаталогОтчета, "*.xml");
	Ожидаем.Что(ФайлыОтчета).Заполнено();
	КаталогОтчета = Новый Файл(КаталогОтчета); // почистим после теста
...
Процедура ТестДолжен_Экспортировать_ОтчетОТестированииПоПутиПапки() Экспорт
	...
	ФайлыОтчета = НайтиФайлы(КаталогОтчета.ПолноеИмя, "*.xml");
	Ожидаем.Что(ФайлыОтчета).Заполнено();
