...
Процедура ЗаполнитьНаборТестов(НаборТестов) Экспорт
	...
	НаборТестов.Добавить("ТестДолжен_СохранитьГлобальныйКлиентскийКонтекст");
	НаборТестов.Добавить("ТестДолжен_ПолучитьГлобальныйКлиентскийКонтекст");
КонецПроцедуры
...
&НаКлиенте
Процедура ТестДолжен_СохранитьГлобальныйКлиентскийКонтекст() Экспорт
	ОписаниеОшибки = "";
	Попытка
		ЗначениеКлиентскогоКонтекста = Новый Структура();
		КонтекстЯдра.СохранитьГлобальныйКлиентскийКонтекст(ЗначениеКлиентскогоКонтекста);
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
	КонецПопытки;
	Ожидаем.Что(ОписаниеОшибки, "Ожидали, что сохранение глобального клиентского контекста пройдет без ошибок, но это не так")
		.Не_().Заполнено();
КонецПроцедуры

&НаКлиенте
Процедура ТестДолжен_ПолучитьГлобальныйКлиентскийКонтекст() Экспорт
	ОписаниеОшибки = "";
	Попытка
		ЗначениеКлиентскогоКонтекста = КонтекстЯдра.ПолучитьГлобальныйКлиентскийКонтекст();
	Исключение
		ОписаниеОшибки = ОписаниеОшибки();
	КонецПопытки;
	Ожидаем.Что(ОписаниеОшибки, "Ожидали, что Получение глобального клиентского контекста пройдет без ошибок, но это не так")
		.Не_().Заполнено();
		
	Ожидаем.Что(ЗначениеКлиентскогоКонтекста, "Ожидали, что полученный глобальный клиентский контекст имеет тип Структура, но это не так")
