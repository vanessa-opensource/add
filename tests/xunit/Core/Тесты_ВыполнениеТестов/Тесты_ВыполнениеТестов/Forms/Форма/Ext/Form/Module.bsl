...
Процедура ЗаполнитьНаборТестов(НаборТестов, КонтекстЯдраПараметр) Экспорт
	...
	Если КонтекстЯдраПараметр.ЕстьПоддержкаАсинхронныхВызовов Тогда

		НаборТестов.НачатьГруппу("Асинхронное ожидание - продолжение");
	    НаборТестов.СтрогийПорядокВыполнения();	
		НаборТестов.Добавить("ТестДолжен_СоздатьДанныеДляДинамическогоСписка");
		НаборТестов.Добавить("ТестДолжен_ОткрытьФормуИПодождатьТаймаут");
		НаборТестов.Добавить("ТестДолжен_УстановитьОтборДинамическогоСпискаИЖдать");
		НаборТестов.Добавить("ТестДолжен_ПроверитьТекущиеДанныеСписка");
		НаборТестов.Добавить("ТестДолжен_ОповеститьФормуИПроверитьРезультат");		
		НаборТестов.ДобавитьДеструктор("ТестДолжен_УдалитьДанные");
...
// { Ждать - Возобновить тестирование
&НаКлиенте
Процедура ТестДолжен_СоздатьДанныеДляДинамическогоСписка() Экспорт
	Ссылка = ПолучитьСсылкуНаСервере();
	КонтекстЯдра.СохранитьКонтекст(Новый Структура("Ссылка",Ссылка)); 
КонецПроцедуры

&НаКлиенте
Процедура ТестДолжен_ОткрытьФормуИПодождатьТаймаут() Экспорт
	КонтекстСценария = КонтекстЯдра.ПолучитьКонтекст();
	КонтекстСценария.Вставить("Ключ",Новый УникальныйИдентификатор);
	Ф = ОткрытьФорму("ВнешняяОбработка.Тесты_ВыполнениеТестов.Форма.ТестЖдатьПродолжить",,,КонтекстСценария.Ключ,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);	
	КонтекстСценария.Вставить("УИД", Ф.УникальныйИдентификатор);
	КонтекстЯдра.Ждать(0.1); //не важно сколько ждат, главное подождать
	КонтекстЯдра.СохранитьКонтекст(КонтекстСценария);	
КонецПроцедуры

&НаКлиенте
Процедура ТестДолжен_УстановитьОтборДинамическогоСпискаИЖдать() Экспорт
	КонтекстСценария = КонтекстЯдра.ПолучитьКонтекст();
	Ф = ОткрытьФорму("ВнешняяОбработка.Тесты_ВыполнениеТестов.Форма.ТестЖдатьПродолжить",,,КонтекстСценария.Ключ,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);	
	Ожидаем.Что(Ф.УникальныйИдентификатор, "Получение открытой формы").Равно(КонтекстСценария.УИД);
	Ф.Ссылка = КонтекстСценария.Ссылка;
	Ф.ОтборНаСервере(); //Багофича - вызов приватного серверного метода из "другой" формы.
	Оповещение = КонтекстЯдра.Ждать(5); //

	Ожидаем.Что(ТипЗнч(Оповещение)=Тип("ОписаниеОповещения"),"Оповещение").ЭтоИстина(); //TechDebt: не возможно проверить значения сущ. только на клиенте
	Ожидаем.Что(Оповещение.ДополнительныеПараметры,"Оповещение.ДополнительныеПараметры").Заполнено();
	
	Ф.ТестШпион = Новый Структура("ТаблицаПриАктивизацииСтроки", Новый Массив());
	Ф.ТестШпион.ТаблицаПриАктивизацииСтроки.Добавить(Оповещение);
		
КонецПроцедуры

&НаКлиенте
Процедура ТестДолжен_ПроверитьТекущиеДанныеСписка() Экспорт
	КонтекстСценария = КонтекстЯдра.ПолучитьКонтекст();
	Ф = ОткрытьФорму("ВнешняяОбработка.Тесты_ВыполнениеТестов.Форма.ТестЖдатьПродолжить",,,КонтекстСценария.Ключ,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);	
	Ожидаем.Что(Ф.Элементы.Таблица.ДанныеСтроки(КонтекстСценария.Ссылка)).Существует();
КонецПроцедуры

&НаКлиенте
Процедура ТестДолжен_ОповеститьФормуИПроверитьРезультат() Экспорт
	КонтекстСценария = КонтекстЯдра.ПолучитьКонтекст();
	Ф = ОткрытьФорму("ВнешняяОбработка.Тесты_ВыполнениеТестов.Форма.ТестЖдатьПродолжить",,,КонтекстСценария.Ключ,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);	
	
	Оповестить("тест",Неопределено,УникальныйИдентификатор);
	
	Ожидаем.Что(Ф.РезультатОповещения.ИмяСобытия, "тест событие").Равно("тест");
	Ожидаем.Что(Ф.РезультатОповещения.Источник, "мое событие").Равно(УникальныйИдентификатор);
	
КонецПроцедуры

&НаКлиенте
Процедура ТестДолжен_УдалитьДанные() Экспорт
	КонтекстСценария = КонтекстЯдра.ПолучитьКонтекст();
	УдалитьСсылкуНаСервере(КонтекстСценария.Ссылка);
...
&НаСервере
Функция  ПолучитьСсылкуНаСервере() Экспорт
	СсылкаОбъект = Справочники.Справочник1.СоздатьЭлемент();
	СсылкаОбъект.Наименование = "тест";
	СсылкаОбъект.ОбменДанными.Загрузка = Истина;
	СсылкаОбъект.Записать();
	
	Возврат СсылкаОбъект.Ссылка;
КонецФункции

&НаСервере
Процедура  УдалитьСсылкуНаСервере(Ссылка) Экспорт
