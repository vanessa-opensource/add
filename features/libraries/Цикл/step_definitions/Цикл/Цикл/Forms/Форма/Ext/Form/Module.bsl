...
Процедура ДляКаждойСтрокиТаблицыЯВыполняю(ИмяТЧ) Экспорт
	...
	ИдШага = "ИдШага" + XMLСтрока(Ванесса.ПолучитьСостояниеVanessaADD().ТекущийШаг.ИдСтроки);
...
Процедура ОбработчикОжиданияВТечениеСекундЯВыполняю()
	...
	Ванесса.Отладка("Текущий момент: " + ТекущаяДатаСеанса());
	Ванесса.Отладка("Дата окончания ожидания: " + ДатаОкончанияОбработкиОжидания);

	Если ТекущаяДатаСеанса() >= ДатаОкончанияОбработкиОжидания Тогда
...
Процедура ВТечениеСекундЯВыполняю(КоличествоСекунд) Экспорт
	...
	Если НЕ Контекст.Свойство("ОжидатьЗавершениеЦикла")
		ИЛИ Контекст.ОжидатьЗавершениеЦикла <> Истина Тогда
		
		Контекст.Вставить("ОжидатьЗавершениеЦикла", Истина);
...
Процедура ЯПрерываюЦикл() Экспорт
	...
	Контекст.Вставить("ОжидатьЗавершениеЦикла", Ложь);
КонецПроцедуры
