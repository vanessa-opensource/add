...
Процедура ЯПодключаюКлиентТестированияСПараметрами(ТабПарам) Экспорт
	...
	ПроставитьГлобальныеПеременные(ТабПарам);
...
&НаКлиенте
Процедура ПроставитьГлобальныеПеременные(ТабПарам)
...
Функция ЕстьТакоеПолеВСтруктуре(ПроверяемаяСтруктура, ИмяПоля)
	...
	Исключение
		...
		ЗаписатьОшибкуВЖР("Проверяю поле " + ИмяПоля +" в структуре." + Символы.ПС
...
Процедура ЗаписатьОшибкуВЖР(ТекстОшибки)
	ЗаписьЖурналаРегистрации("VanessaADD.ЧтениеГлобальныхПеременных",
...
Функция ПолучитьПараметрыПодключения(ТабПарам,ДопПараметры = Неопределено)
	...
	Для ккк = 1 По ИменаПараметров.Количество() Цикл
		...
		ИначеЕсли ВРег(ИмяПараметра) = ВРег("Запускаемая обработка") Тогда
			...
			КаталогПроектаМетка     = "$workspaceRoot";
			КаталогПроектаВхождение = Найти(СокрЛП(ЗначениеПараметра), КаталогПроектаМетка); 
			
			Если КаталогПроектаВхождение = 1 Тогда
				
				// Ожидается вхождение метки в начало строки с учетом регистра
				// Ожидается что значение будет каталогом и будет существовать в контексте среды менеджера,
				// аналогично запускаемой обработке 
				
				ЗначениеПараметра = СокрЛП(ЗначениеПараметра);
				
				ПараметрыФайла = Новый Файл(Ванесса.Объект.КаталогПроекта);	
				Если ПараметрыФайла.ЭтоКаталог() = Ложь Тогда
					ВызватьИсключение "Не предполагаемое значеначение $workspaceRoot, ресурс не является каталогом";	
				КонецЕсли;	
				Если ПараметрыФайла.Существует() = Ложь Тогда
					ВызватьИсключение "Не предполагаемое значеначение $workspaceRoot, ресурс не существует";	
				КонецЕсли;
				
				ЗначениеПараметра = ПараметрыФайла.ПолноеИмя + Сред(ЗначениеПараметра, 
					СтрДлина(КаталогПроектаМетка) + КаталогПроектаВхождение);
					
				// Корректировка разделителей пути
					
				ПараметрыФайла    = Новый Файл(ЗначениеПараметра);
				ЗначениеПараметра = ПараметрыФайла.ПолноеИмя;
	...
	КонецЕсли;
	Стр.ДопПараметры = СокрЛП(СтрокаАутентификации + " " +ДопПараметр);
