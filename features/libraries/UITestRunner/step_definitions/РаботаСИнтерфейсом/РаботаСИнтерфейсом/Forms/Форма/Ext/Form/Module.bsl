...
Функция ПолучитьРасхожденияТД(Макет, ТабПарам,ТипСравнения)
	...
	Для Каждого СтрокаОжидаемая Из ТабПарам Цикл
		...
		Если СтрокаТаблицыМакета = Неопределено Тогда
			СтрокаРасхождения = Вычислить("СтрШаблон_(""Именованная область %1 не найдена;"", ИмяПараметра)");
		...
		ИначеЕсли ТипСравнения = "точный" Тогда
			Если СтрокаТаблицыМакета.Значение <> ЗначениеПараметра Тогда
				СтрокаРасхождения = Вычислить("СтрШаблон_(""Значение параметра %1: ожидали <%2>, получили <%3>;"", ИмяПараметра, ЗначениеПараметра, СтрокаТаблицыМакета.Значение)");
				МассивРасхождений.Добавить(СтрокаРасхождения);
			КонецЕсли;	 
		ИначеЕсли ТипСравнения = "подстрока" Тогда
			Если Найти(СтрокаТаблицыМакета.Значение,ЗначениеПараметра) = 0 Тогда
				СтрокаРасхождения = Вычислить("СтрШаблон_(""Значение параметра %1: ожидали подстроку <%2>, в <%3>;"", ИмяПараметра, ЗначениеПараметра, СтрокаТаблицыМакета.Значение)");
				МассивРасхождений.Добавить(СтрокаРасхождения);
			КонецЕсли;	 
		Иначе
...
Функция ПолучитьРасхожденияТДRegExp(Макет, ТабПарам)
	...
	Для Каждого СтрокаОжидаемая Из ТабПарам Цикл
		...
		Если ЗначениеПараметраМакета = Неопределено Тогда
			СтрокаРасхождения = СтрШаблон_("%1 не найден;", ИмяПараметра);
			МассивРасхождений.Добавить(СтрокаРасхождения);
		ИначеЕсли Не Ванесса.СтрокаСоответствуетШаблону(ЗначениеПараметраМакета, ЗначениеПараметра) Тогда
			СтрокаРасхождения = СтрШаблон_("Значение параметра %1: ожидали <%2>, получили <%3>;", ИмяПараметра, ЗначениеПараметра, ЗначениеПараметраМакета);
			МассивРасхождений.Добавить(СтрокаРасхождения);
		КонецЕсли;
	КонецЦикла;
...
Процедура ТабличныйДокументСодержитЗначенияОбластей(ИмяЭлементаФормы,ТабПарам) Экспорт
	...
	Если МассивРасхождений.Количество() > 0 Тогда
		СтрокаОшибки = Вычислить("СтрШаблон_(""Ошибка при поиске значений в макете %1:"", ИмяЭлементаФормы)");
		МассивРасхождений.Вставить(0, СтрокаОшибки);
		СтрокаОшибки = Вычислить("Ванесса.СтрСоединить_(МассивРасхождений, Символы.ПС)");
...
Процедура ТабличныйДокументСодержитЗначенияОбластейПоШаблону(ИмяЭлементаФормы,ТабПарам) Экспорт
	...
	Если МассивРасхождений.Количество() > 0 Тогда
		СтрокаОшибки = СтрШаблон_("Ошибка при поиске значений в макете %1:", ИмяЭлементаФормы);
		МассивРасхождений.Вставить(0, СтрокаОшибки);
		СтрокаОшибки = Ванесса.СтрСоединить_(МассивРасхождений, Символы.ПС);
		ВызватьИсключение СтрокаОшибки;
	КонецЕсли;
