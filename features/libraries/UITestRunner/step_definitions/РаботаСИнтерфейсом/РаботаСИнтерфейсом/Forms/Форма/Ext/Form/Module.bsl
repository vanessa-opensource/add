...
Процедура ОбработичкОжиданияВОткрытойФормеЯЖдуПоявленияЭлементаВТеченииСекунд()
	...
	Если ЭлементФормы = Неопределено Тогда
		Если (ТекущаяДатаСеанса() - ДатаНачалаОбработкиОжидания) > КоличествоСекундОбработкаОжидания Тогда
...
Процедура ВОткрытойФормеЯЖдуПоявленияЭлементаВТеченииСекунд(ИмяЭлемента, КоличествоСекунд = 10, ИскатьПоИмени = Ложь)
	...
	ДатаНачалаОбработкиОжидания       = ТекущаяДатаСеанса();
...
Процедура ЯЖдуЗакрытияОкнаВТеченииСекунд(ИмяОкна,КоличествоСекунд) Экспорт
	...
	Пока  (ТекущаяДатаСеанса() - ДатаНачалаОбработкиОжидания) < КоличествоСекундОбработкаОжидания Цикл
	...
	Для Ккк = 1 По ТабПарам.Количество()-1 Цикл //в первой строке шапка
		...
		Для каждого Колонка Из СтрокаШапки Цикл
			...
			Если Не СравнениеПоШаблону Тогда
				Если ТипЗнч(ЗначСтарое) = Тип("Дата") Тогда
					Попытка
						ЗначНовое = СтрокаВДату(ЗначНовое);
					Исключение
						Если НачалоДня(ЗначСтарое) = ЗначСтарое Тогда
							ЗначСтарое = Формат(ЗначСтарое, "ДЛФ=D");
						Иначе
							ЗначСтарое = Формат(ЗначСтарое, "ДЛФ=DT");
						КонецЕсли;
					КонецПопытки;
				ИначеЕсли ТипЗнч(ЗначСтарое) = Тип("Число") Тогда
					Попытка
						ЗначНовое = СтрокаВЧисло(ЗначНовое);
					Исключение
						ЗначСтарое = Строка(ЗначСтарое);
					КонецПопытки;
				КонецЕсли;
			КонецЕсли;
			Если ТипЗнч(ЗначСтарое) = Тип("Строка") Тогда
...
Процедура ВПолеЯВвожуТекущуюДату(ИмяПоля,ИскатьПоИмени = Ложь) Экспорт
	Текст = Формат(ТекущаяДатаСеанса(),"ДФ=dd.MM.yyyy");
...
Процедура ВПолеСИменемЯВвожуНачалоТекущегоМесяца(ИмяПоля) Экспорт
	Текст = Формат(НачалоМесяца(ТекущаяДатаСеанса()),"ДФ=dd.MM.yyyy");
	ВОткрытойФормеВПолеСИменемЯВвожуТекст(ИмяПоля,Текст);
КонецПроцедуры
...
Процедура ВПолеСИменемЯВвожуКонецТекущегоМесяца(ИмяПоля) Экспорт
	Текст = Формат(КонецМесяца(ТекущаяДатаСеанса()),"ДФ=dd.MM.yyyy");
	ВОткрытойФормеВПолеСИменемЯВвожуТекст(ИмяПоля,Текст);
КонецПроцедуры
...
Процедура ВПолеСИменемЯВвожуНачалоСледующегоМесяца(ИмяПоля) Экспорт
	Текст = Формат(НачалоМесяца(ДобавитьМесяц(ТекущаяДатаСеанса(),1)),"ДФ=dd.MM.yyyy");
	ВОткрытойФормеВПолеСИменемЯВвожуТекст(ИмяПоля,Текст);
КонецПроцедуры
...
Процедура ВПолеСИменемЯВвожуКонецСледующегоМесяца(ИмяПоля) Экспорт
	Текст = Формат(КонецМесяца(ДобавитьМесяц(ТекущаяДатаСеанса(),1)),"ДФ=dd.MM.yyyy");
	ВОткрытойФормеВПолеСИменемЯВвожуТекст(ИмяПоля,Текст);
КонецПроцедуры
...
Процедура ВПолеЯВвожуТекущуюДатуИТекущееВремя(ИмяПоля,ИскатьПоИмени = Ложь) Экспорт
	Текст = Строка(ТекущаяДатаСеанса());
...
Функция СообщениеПредупреждения(ПоляФормы, ДекорацииФормы)
	...
	Для Каждого ТекДекорация Из ДекорацииФормы Цикл
		Если ТекДекорация.Имя = "ErrorInfo" Или ТекДекорация.Имя = "Message" Тогда
