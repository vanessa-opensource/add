...
Функция ПроверитьЗавершениеФоновогоЗаданияНаСервере()
	...
	//СтруктураОтбора.Вставить("Начало",				ТекущаяДатаСеанса() - 60);
...
Процедура ПроверитьЗначениеЯчейкиТабличногоДокумента()
	...
	Если ЗначениеЗаполнено(ТекстОбласти) Тогда
		Ванесса.ПродолжитьВыполнениеШагов(Ложь);
	Иначе
		Если ТаймаутОжиданияОжиданияТекстаОбласти < ТекущаяДатаСеанса() Тогда
...
Процедура ЯЖдуКогдаВТабличномДокументеЗаполнитсяЯчейкаВТечениеСекунд(ИмяТабДок,АдресЯчейки,КолСекунд) Экспорт
	ТаймаутОжиданияОжиданияТекстаОбласти      = ТекущаяДатаСеанса() + Число(КолСекунд);
...
&НаКлиенте
//И текущая форма не модифицирована
//@ТекущаяФормаНеМодифицирована()
Процедура ТекущаяФормаНеМодифицирована() Экспорт
...
//@ТекущаяФормаМодифицирована()
Процедура ТекущаяФормаМодифицирована() Экспорт
