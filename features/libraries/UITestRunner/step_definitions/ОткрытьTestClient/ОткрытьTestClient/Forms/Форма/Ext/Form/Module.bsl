...
Процедура ЯЗапускаюСеанс1ССКлючомTESTCLIENTСЛогиномИПаролем(Логин,Пароль) Экспорт
	...
	УстановитьКраткийЗаголовокПриложения("Vanessa-ADD");
	...
	Если КонтекстСохраняемый.Свойство("ТестовоеПриложение") Тогда
		Если КонтекстСохраняемый.ТестовоеПриложение <> Неопределено Тогда
			Попытка
				...
				ГлавноеОкноТестКлиента = Ванесса.НайтиОкноТестКлиентаПоЗаголовку("");
			Исключение
	...
	ИмяФайлаПриложения1С = "";
	Если Ванесса.ЭтоLinux Тогда
		ИмяФайлаПриложения1С = "1cv8c";
		КаталогПрограммы = КаталогПрограммы();
	Иначе
		ИмяФайлаПриложения1С = "1cv8c.exe";
		КаталогПрограммы = Ванесса.СтрШаблон_("C:\Program Files (x86)\1cv8\%1\bin\", ВерсияПриложения);
	КонецЕсли;
	ПутьК1С = КаталогПрограммы + ИмяФайлаПриложения1С;

	Если НЕ Ванесса.ФайлСуществуетКомандаСистемы(ПутьК1С) Тогда
		ВызватьИсключение Ванесса.СтрШаблон_("Не нашел путь к %1: %2", ИмяФайлаПриложения1С, ПутьК1С);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьГлавноеОкноТестируемогоПриложения()
	...
	Попытка
		ГлавноеОкноТестируемого = Ванесса.НайтиОкноТестКлиентаПоЗаголовку("");
