...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ОтменяюВсеФоновыеЗаданияУКоторыхИмяМетодаОтРегламентногоЗадания(Парам01)","ОтменяюВсеФоновыеЗаданияУКоторыхИмяМетодаОтРегламентногоЗадания","Дано отменяю все фоновые задания, у которых имя метода от регламентного задания ""Регламентное задание1""", "Отмена выполнения фоновых заданий, у которых имя метода совпадает с имени метода от указанного регламентного задания. В наименования задания допустимо использовать шаблонный поиск через *.", "Данные.Регламентные\фоновые задания");
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ОтменяюВсеФоновыеЗаданияУКоторыхИмяМетода(Парам01)","ОтменяюВсеФоновыеЗаданияУКоторыхИмяМетода","Дано отменяю все фоновые задания, у которых имя метода ""СлужебныйМодуль.МетодРегламентногоЗадания1""", "Отмена выполнения фоновых заданий, у которых имя метода совпадает с именем метода от указанного регламентного задания. В имени задания допустимо использовать шаблонный поиск через *.", "Данные.Регламентные\фоновые задания");
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЖдуЗавершенияРегламентногоЗаданияВТечениеСекунд(Парам01,Парам02)","ЖдуЗавершенияРегламентногоЗаданияВТечениеСекунд","И жду завершения регламентного задания ""Регламентное задание1"" в течение 2 секунд", "Ожидать завершения ранее запущенного фонового задания. В наименовании задания допустимо использовать шаблонный поиск через *.", "Данные.Регламентные\фоновые задания");
...
Функция ФоновоеЗаданиеЗавершеноСервер(ИмяРегламентногоЗадания, ОписаниеЗадания)
	МассивФоновых = НайтиФоновые(ИмяРегламентногоЗадания, ОписаниеЗадания);
	...
	ИначеЕсли МассивФоновых.Количество() > 1 Тогда //TODO вернуть
		ВызватьИсключение "Количество фоновых заданий больше одного. Имя регламентного задания " + ОписаниеЗадания.Имя;
	КонецЕсли;
	...
	ИначеЕсли ФЗ.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		ВызватьИсключение "Фоновое задание завершено аварийно. Имя регламентного задания " + ОписаниеЗадания.Имя;
	ИначеЕсли ФЗ.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ВызватьИсключение "Фоновое задание завершено отменено. Имя регламентного задания " + ОписаниеЗадания.Имя;
	КонецЕсли;
КонецФункции

&НаСервереБезКонтекста
Функция НайтиФоновые(ИмяРегламентногоЗадания, ОписаниеЗадания)
	МетаЗадание = НайтиРегламентноеЗаданиеПоИмениСервер(ИмяРегламентногоЗадания);
	Отбор = Новый Структура("ИмяМетода,Начало", МетаЗадание.ИмяМетода, ОписаниеЗадания.Начало);
