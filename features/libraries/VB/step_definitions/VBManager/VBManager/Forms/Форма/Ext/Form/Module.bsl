...
Процедура ЯЗагрузилВРаннерТекущийФичаФайлИЗапустилНаВыполнение() Экспорт
	...
	Если Контекст.ИдТекущегоРаннера = 0 Тогда
		...
		Если (КонтекстСохраняемый.ОднопоточныйМассивФичаФайлов.Количество()-1) >= (КонтекстСохраняемый.ТекущийОднопоточныйИДМассивФичаФайлов+1) Тогда
			...
			Сообщить("" + ТекущаяДатаСеанса() + ". Однопоточная фича №" + (КонтекстСохраняемый.ТекущийОднопоточныйИДМассивФичаФайлов+1) + " из " + КонтекстСохраняемый.ОднопоточныйМассивФичаФайлов.Количество() + ". РаннерID="+Контекст.ИдТекущегоРаннера + ". " + ТекущаяФича);
		КонецЕсли;
	КонецЕсли;
	...
	Если ТекущаяФича = Неопределено Тогда
		...
		Если (КонтекстСохраняемый.МассивФичаФайлов.Количество()-1) >= (КонтекстСохраняемый.ТекущийИДМассивФичаФайлов+1) Тогда
			...
			Сообщить("" + ТекущаяДатаСеанса() + ". Обычная фича №" + (КонтекстСохраняемый.ТекущийИДМассивФичаФайлов+1) + " из " + КонтекстСохраняемый.МассивФичаФайлов.Количество() + ". РаннерID="+Контекст.ИдТекущегоРаннера + ". " + ТекущаяФича);
		КонецЕсли;
	КонецЕсли;
