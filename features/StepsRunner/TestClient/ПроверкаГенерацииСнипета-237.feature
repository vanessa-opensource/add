...
Функционал: Проверка генерации снипета
	Как Разработчик
	Я Хочу чтобы чтобы у меня была возможность проверить генерацию снипета в Vanessa-ADD в режиме TestClient
	...
	Сценарий: Генерации снипета TestClient
		...
		Когда я открыл форму VanessaADD в TestClient
		...
		И установил флаг в  VanessaADD в TestClient ПроверкаРаботыVanessaADDВРежимеTestClient
				И В открытой форме я перехожу к закладке с заголовком "Сервис"
				И В открытой форме я изменяю флаг с заголовком "Проверка работы Vanessa-ADD в режиме test client"

		И в VanessaADD в TestClient я загрузил специальную тестовую фичу "ПростаяФичаДляПроверкиРаботыВыполненияСценария"
		...
		И в VanessaADD в TestClient я загружаю сценарии
				...
				И В форме "* Vanessa ADD" в ТЧ "КаталогиБиблиотек" я завершаю редактирование строки
