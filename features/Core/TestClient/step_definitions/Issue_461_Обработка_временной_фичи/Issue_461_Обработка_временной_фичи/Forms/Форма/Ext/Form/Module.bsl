...
Процедура ВПолеНаходитсяТекстИзМакета(Парам01,Парам02) Экспорт
	...
	ОткрытаяФормаVanessaBehavoir                    = Контекст.ОткрытаяФормаVanessaBehavoir;
	Если СокрЛП(ОткрытаяФормаVanessaBehavoir.Объект.СгенерированныйСценарий) <> СокрЛП(ТекстСценария) Тогда
		Сообщить("ОткрытаяФормаVanessaBehavoir.Объект.СгенерированныйСценарий:");
		Сообщить(СокрЛП(ОткрытаяФормаVanessaBehavoir.Объект.СгенерированныйСценарий));
		Сообщить("ТекстСценария:");
		Сообщить(СокрЛП(ТекстСценария));

		Строка = СокрЛП(ОткрытаяФормаVanessaBehavoir.Объект.СгенерированныйСценарий);
		ЗаписатьЛогВЖРИнформация("Тесты",
			"ОткрытаяФормаVanessaBehavoir.Объект.СгенерированныйСценарий:
