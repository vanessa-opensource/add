...
Процедура ИнициализацияОкружения()
	...
	ИмяФайлаНастройки = СтрШаблон("./tools/JSON/env.json.%1", Аргументы.Команда);
	...
	СтрокаЗапуска = СтрШаблон("oscript ./tools/runner.os compileepf ./tools ./%1tools --ibname /F./build/ibservice", ПодкаталогСборки);
	ИсполнитьКоманду(СтрокаЗапуска);
