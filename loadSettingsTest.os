...
Функция СоздатьОбщуюКоллекциюКомандЗапуска()
	...
	Рез.Вставить("КаталогиБиблиотек", //TODO
		Новый Структура("Р, А, ПоУмолчанию, УстанавливатьЕслиЗаполнено, ЭтоМассив, УстанавливатьЕслиИстина, ПреобразоватьПуть, ПростоУстановить, РазрешаюшийКлюч",
			"КаталогиБиблиотек", "librarycatalogs", Новый Массив, Ложь, Ложь, Ложь, Истина, Ложь, ""));
...
Функция ЗагрузитьНастройки(Знач СтруктураJSON, Знач КонтекстЯдра, ОбъектЯдра)
	...
	Для каждого КлючЗначение Из КоллекцияКомандЗапуска Цикл
		...
		Ожидаем.Что(ПараметрыКлюча.ПростоУстановить И ПараметрыКлюча.ПреобразоватьПуть,
			...
			Если Ключ <> "StartFeaturePlayer" 
				И Ключ <> "КаталогФич" 
				И Ключ <> "КаталогПроекта"
				И Ключ <> "КаталогOutputAllure" 
				И Ключ <> "КаталогOutputAllureБазовый"
				И Ключ <> "ДелатьОтчетВФорматеjUnit" 
				И Ключ <> "КаталогOutputjUnit" 
				И Ключ <> "ДелатьОтчетВФорматеCucumberJson" 
				И Ключ <> "КаталогOutputCucumberJson" 
				И Ключ <> "СоздаватьПодкаталогВКаталогеAllureДляЭтойСборки" 
				И Ключ <> "ВыполнитьСценарии" 
				И Ключ <> "ВыполнитьСценарииСТекущегоШага" 
				И Ключ <> "НомерСтрокиДляЗапускаСТекущегоШагаСценария" 
				И Ключ <> "ЗагрузитьСценарии" 
				И Ключ <> "ЗавершитьРаботуСистемы" 
				И Ключ <> "ИмяСборки" 
				И Ключ <> "ЗаписьВидеоСловарьЗамен" 
				И Ключ <> "ЗаписьВидеоСловарьЗаменСписок"
				И Ключ <> "ДополнительныеПараметры"
				И Ключ <> "КлиентыТестирования"
				И Ключ <> "ТаймаутЗапуска1С"
				И Ключ <> "ОсновнойЯзык"
				Тогда
