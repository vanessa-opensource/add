...
Функция ДобавитьПараметрStepDefinition(Знач Стр, КолПараметров)
	...
	Если (Найти(Стр, "||ПараметрЧисло||") > 0) Или (Найти(Стр, "||ПараметрСтрокаА||") > 0) 
			Или (Найти(Стр, "||ПараметрСтрокаК||") > 0)
			//Или (Найти(Стр, "||ПараметрСтрокаК||") > 0) Или (Найти(Стр, "||ПараметрСтрокаМ||") > 0)
			Или (Найти(Стр, "||ПараметрДата||") > 0) Тогда
...
Процедура ОбработатьМногострочнуюСтрокуЭмуляцияRegExp(Стр, ПараметрыСтроки, ДелатьОберткуПараметров = Ложь)
	...
	Если ДелатьОберткуПараметров Тогда
		СтрКолПараметров = XMLСтрока(ПараметрыСтроки.Количество());
	КонецЕсли;
	//НоваяСтрока = НоваяСтрока + " ||ПараметрСтрокаМ" + СтрКолПараметров + "|| ";
	
	Стр = СтрПолучитьСтроку(Стр, 1) + " ||ПараметрСтрокаК" + СтрКолПараметров + "|| ";
...
Процедура ПодготовитьСтрокуСОберткойПараметров(Стр, ПараметрыСтрокиА, ПараметрыСтрокиК, ПараметрыДаты, ПараметрыЧисла)
	...
	КонецЦикла;
	//TODO ПодготовитьСтрокуСОберткойПараметров - добавить обработку ПараметрСтрокаМ аналогично ПараметрСтрокаК
...
Функция ПолучитьStepDefinitionПоСтроке(Знач Стр,ЗначенияПараметров, СтрокаПараметров = "",
	...
	ЭмуляцияRegExp(Стр, ПараметрыСтрокиА, ПараметрыСтрокиК, ПараметрыСтрокиМ, ПараметрыЧисла, ПараметрыДаты, ПараметрыИменованные);
	...
	Для Каждого Элем Из МассивПодстрок Цикл
		...
		//ИначеЕсли Элем = "||ПараметрСтрокаМ||" Тогда //TODO устранить дублирование обработки ПараметрСтрокаА, ПараметрСтрокаК и ПараметрСтрокаМ
		//	КолПараметров        = КолПараметров       + 1;
		//	КолПараметровСтрокаМ = КолПараметровСтрокаМ + 1;

		//	СтрокаПараметров = СтрокаПараметров + ДобавитьПараметрStepDefinition(Элем, КолПараметров);

		//	Если (ПараметрыСтрокиМ.Количество()-1) < КолПараметровСтрокаМ-1 Тогда
		//		ТекстСообщения = "Не смог обработать параметры в строке <%1>";
		//		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", СтрокаОригинал);
		//		ВызватьИсключение ТекстСообщения;
		//	КонецЕсли;
		...
		//	СтруктураЗначенияПараметра = Новый Структура;
		//	СтруктураЗначенияПараметра.Вставить("Значение", ПараметрыСтрокиМ[КолПараметровСтрокаМ - 1]);
		//	СтруктураЗначенияПараметра.Вставить("Тип", "Строка");
