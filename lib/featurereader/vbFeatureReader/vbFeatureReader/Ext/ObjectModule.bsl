...
Функция ПолучитьДеревоФич(СтруктураПараметров) Экспорт
	...
	ЗаполнитьДерево(ДеревоЗначений, ПутьКФичам.ПолноеИмя, ТаблицаИзвестныхStepDefinition, СтруктураПараметров);
...
Процедура ПолучитьУжеСуществующиеСнипетыИзОбработок(Знач КаталогФич, ТаблицаКонтекстовОбработок,
	ТаблицаИзвестныхStepDefinition,ТаблицаВерсийEPF,КонтекстVanessaBehavoir) Экспорт
	...
	Для каждого Стр Из МассивСтрокФайла Цикл
		...
		Если ПервыйСимвол = "#" Тогда
			...
			Если  ПозицияПоиска > 0 Тогда
				СтрокаЯзыка = СокрЛП(Сред(Стр, ПозицияПоиска + ДлинаТэгаУказанияЯзыка));
...
Процедура ОбойтиТаблицуОтступов(Тзн, КолСтрок, ЕстьПримеры)
	Для Ккк = 0 По КолСтрок-1 Цикл
		...
		ИначеЕсли ВЭтойСтрокеКлючевоеСловоПримеры(ПредСтрокаБезПробелов) Тогда
			ПредСтрокаТзн.ЗначениеОтступа = ТекСтрокаТзн.ЗначениеОтступа;
	...
	Пока СчетчикСтрокФичи < КолСтрокТаблицаТекстФичи Цикл
		...
		Если Стр = СимволМногострочныйТекст Тогда
			Если ТаблицаШагов.Количество() = 0 Тогда
				ОписаниеОшибки = "Строка №" + НомСтр + ", обнаружен символ " + СимволМногострочныйТекст + ", хотя не было строки с шагом.";
				СтруктураПараметров.Вставить("УдалитьСтрокуФичиИзДерева", Истина);
				Возврат Ложь;
			КонецЕсли;
			УспешноПрочитали = ПолучитьМногострочныйТекст(СчетчикСтрокФичи, ТаблицаТекстФичи, КолСтрокТаблицаТекстФичи, ТаблицаШагов);
...
Функция ДобавитьПараметрStepDefinition(Знач Стр, КолПараметров)
	...
	НашлиПараметр = Ложь;
	Для каждого КлючЗначение Из МассивПараметровХХХ() Цикл
		Если Найти(Стр, КлючЗначение.Ключ) > 0 Тогда
...
Процедура ОбработатьМногострочнуюСтрокуЭмуляцияRegExp(Стр, ПараметрыСтроки, ДелатьОберткуПараметров = Ложь)
	...
	СобираемаяСтрока = СтрПолучитьСтроку(Стр, 2);
...
Процедура ПодготовитьСтрокуСОберткойПараметров(Стр, ПараметрыСтрок, ПараметрыДаты, ПараметрыЧисла)
	Для каждого КлючЗначение Из ПараметрыСтрок Цикл
		...
		ПараметрыСтроки = КлючЗначение.Значение;
		Ном = 0;
	...
	СтруктураПрочихПараметров.Вставить("ПараметрыДаты", ПараметрыДаты);
	СтруктураПрочихПараметров.Вставить("ПараметрыЧисла", ПараметрыЧисла);
	Для каждого КлючЗначение Из ПараметрыСтрок Цикл
		ИмяПараметра = КлючЗначение.Ключ;
...
Процедура ЭмуляцияRegExp(Стр, ПараметрыСтрокиА, ПараметрыСтрокиК, ПараметрыСтрокиМ, ПараметрыЧисла, ПараметрыДаты,
		ПараметрыИменованные, ДелатьОберткуПараметров = Ложь, ДопПараметры = Неопределено)
	...
	Если ДелатьОберткуПараметров Тогда
		...
		ПараметрыСтрок.Вставить(ПараметрыСтрокиА, "ПараметрыСтрокиА");
		ПараметрыСтрок.Вставить(ПараметрыСтрокиК, "ПараметрыСтрокиК");
		ПараметрыСтрок.Вставить(ПараметрыСтрокиА, "ПараметрыСтрокиА");
...
Функция ПолучитьStepDefinitionПоСтроке(Знач Стр,ЗначенияПараметров, СтрокаПараметров = "",
	...
	ЭмуляцияRegExp(Стр, ПараметрыСтрокиА, ПараметрыСтрокиК, ПараметрыСтрокиМ, ПараметрыСтрокиМ, ПараметрыЧисла,
	...
	Если ДанныеОбработкиПараметров <> Неопределено Тогда
		...
		ДанныеОбработкиПараметров.Вставить("ПараметрыСтрокиМ",ПараметрыСтрокиК);
	...
	СоответствиеТипов.Вставить("||ПараметрЧисло||", НастройкиСтрокиМ);
	НастройкиДат = Новый Структура("Счетчик,Тип,Параметры", 0, "ПараметрДата", ПараметрыДаты);
	СоответствиеТипов.Вставить("||ПараметрДата||", НастройкиСтрокиМ);
	...
	Для Каждого Элем Из МассивПодстрок Цикл
		...
		Если ЗначениеЗаполнено(НастройкиЭлемента) Тогда
			...
			НастройкиЭлемента.Количество = НастройкиЭлемента.Количество + 1;

			СтрокаПараметров = СтрокаПараметров + ДобавитьПараметрStepDefinition(Элем, КолПараметров);

			Если (НастройкиЭлемента.Параметры.Количество()-1) < НастройкиЭлемента.Количество - 1 Тогда
			...
			СтруктураЗначенияПараметра.Вставить("Значение",
				НастройкиЭлемента.Параметры[НастройкиЭлемента.Количество - 1]);
...
Функция ВставитьВПараметрыСимволыПроцента(Знач Стр)
	...
	Для Каждого Элем Из МассивПодстрок Цикл
		...
		ИначеЕсли Элем = "||ПараметрСтрокаМ||" Тогда // TODO дублирование обработки ПараметрСтрокаА, ПараметрСтрокаК и ПараметрСтрокаМ
			...
			ЗначениеПараметра = ДанныеОбработкиПараметров.ПараметрыСтрокиА[КолПараметровСтрокаМ - 1];
