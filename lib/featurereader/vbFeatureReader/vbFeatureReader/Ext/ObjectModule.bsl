...
Процедура ЗакрытьПредыдущийСценарий(ИмяФичи, ПолноеИмяФичи, ТекущийТипСценария, ПредыдущееКлючевоеСлово,
	...
	Если СокрЛП(ТекущийТипСценария) <> "" И ТекущийТипСценария <> "background" Тогда
		Если СтруктураПараметров.ИдетЗагрузкаИзКаталога Тогда

			//по исключению сценариев
			Если НайтиТегИзСпискаТегов(СписокТеговИсключение, ТегиСценария,
					"В сценарии <" + ОписаниеСценария + "> найден тег исключение") Тогда
				НайденТегИсключениеВСценарии = Истина;
			КонецЕсли;

			Если НайтиТегИзСпискаТегов(СписокТеговИсключение, МассивТеговФичи,
					"В фиче <" + ПолноеИмяФичи + "> сценария <" + ОписаниеСценария + "> найден тег исключение") Тогда
				НайденТегИсключениеВФиче = Истина;
			КонецЕсли;

			//по отбору сценариев
			Если НайтиТегИзСпискаТегов(СписокТеговОтбор, ТегиСценария,
					"В сценарии <" + ОписаниеСценария + "> найден тег отбор") Тогда
				НайденТегОтборВСценарии = Истина;
			КонецЕсли;

			Если НайтиТегИзСпискаТегов(СписокТеговОтбор, МассивТеговФичи,
					"В фиче <" + ПолноеИмяФичи + "> сценария <" + ОписаниеСценария + "> найден тег отбор") Тогда
				НайденТегОтборВФиче = Истина;
			КонецЕсли;

			// //значит это загрузка именно фичи, а не вложенного сценария
			// Если СписокТеговОтбор.Количество() > 0
			// 	И Не НайденТегОтборВСценарии И НЕ НайденТегОтборВФиче
			// 	И НЕ ЭтоЗагрузкаПодчиненногоСценария Тогда
			...
			// //значит надо проигнорировать этот сценарий
			// //у отбора приоритет над исключением, значит надо оставить сценарий
			// //значит это загрузка именно фичи, а не вложенного сценария
			// Если СписокТеговИсключение.Количество() > 0
			// 	И НЕ НайденТегОтборВСценарии И НЕ НайденТегОтборВФиче
			// 	И НЕ ЭтоЗагрузкаПодчиненногоСценария Тогда
			// 	И ( НайденТегИсключениеВСценарии Или НайденТегИсключениеВФиче )
			// 		Сообщить("Сценарий <" + ОписаниеСценария + "> не загружен.");
			// 		Возврат;
			// КонецЕсли;
		...
		Если Не НайденТегОтборВСценарии И НЕ НайденТегОтборВФиче
			И НЕ ЭтоЗагрузкаПодчиненногоСценария Тогда
