...
//   ДеревоСтроки - <Тип.Вид> - <описание параметра>
//   ТаблицаИзвестныхШагов - <Тип.Вид> - <описание параметра>
//   СтруктураПараметров - <Тип.Вид> - <описание параметра>
//
...
//
// Параметры:
//   СтруктураПараметров - Структура - <описание параметра>
//
//  Возвращаемое значение:
...
Функция ПолучитьДеревоФич(СтруктураПараметров) Экспорт
	...
	КонтекстВанессы			= СтруктураПараметров.КонтекстVanessaBehavoir;
...
Процедура ОпределитьТегиФичи(ТаблицаТекстФичи,МассивТеговФичи,МассивТеговСценариев,МассивТеговВсе,МассивТеговВсеДляПоиска)
	...
	Для каждого СтрокаТаблицы Из ТаблицаТекстФичи Цикл
		Ном = Ном + 1;
		Стр = СокрЛП(СтрокаТаблицы.Стр);//TODO ускорить эту строку и код ниже
...
Функция СледующаяСтрокаСодержитВертикальнуюЧерту(ТаблицаТекстФичи,Знач СчетчикСтрокФичи)
	Пока Истина Цикл
		...
		Стр = СокрЛП(ТаблицаТекстФичи[СчетчикСтрокФичи-1].Стр);//TODO ускорить эту строку и код ниже
		Если Стр = "" Тогда
			Продолжить;
...
Процедура ОбходКаталогов(ПутьКаталога, врДерево, ТаблицаИзвестныхШагов, СтруктураПараметров)
...
Функция ПолучитьStepDefinitionПоСтроке(Знач Стр,ЗначенияПараметров, СтрокаПараметров = "",
	...
	Для Каждого Элем Из МассивПодстрок Цикл
		...
		Если ЗначениеЗаполнено(НастройкиЭлемента) Тогда
			...
			Если (НастройкиЭлемента.Параметры.Количество()-1) < НастройкиЭлемента.Счетчик - 1 Тогда
			...
			СтруктураЗначенияПараметра.Вставить("Значение",
				НастройкиЭлемента.Параметры[НастройкиЭлемента.Счетчик - 1]);
