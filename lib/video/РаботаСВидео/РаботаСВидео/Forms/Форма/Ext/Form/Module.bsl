	...
	Если СокрЛП(КаталогМузыки) <> "" Тогда
		Если ПараметрыВидео.Ванесса.ФайлСуществуетКомандаСистемы(КаталогМузыки) Тогда
			...
			Если СписокФайловКаталога.Количество() > 0 Тогда
				ТекДата = Формат(ТекущаяДатаСеанса(),"DF=dd.MM.yyyy");
...
Процедура УскоритьЗамедлитьФайлВидеоИДобавитьMp3(ПараметрыВидео,ИмяФайлВидео,ИмяФайлMP3,ДлительностьВидео,НоваяДлительностьВидео,ДлительностьMP3)
	...
	ДлинаРасширенияМП4 = 3;
	Если НРег(Прав(ИмяФайлаВидеоВременный, ДлинаРасширенияМП4)) = "mp4" Тогда
...
Функция ПолучитьСписокФайловКаталога(ПараметрыВидео,Каталог)
	...
	Пока Истина Цикл
		...
		Если НРег(Прав(Стр, ДлинаРасширенияМП3)) = "mp3" Тогда
...
Процедура УбратьНачалоСтроки(Текст, НачалоСтроки)
	...
	Если НРег(Лев(Текст, ДлинаСтроки)) = Стр Тогда
		Текст = Сред(Текст, ДлинаСтроки + 1);
	КонецЕсли;
...
Процедура Инициализация(ПараметрыВидео) Экспорт
	...
	ТекДат = ТекущаяДатаСеанса();
	Пока Истина Цикл
		СчетчикПроверок = СчетчикПроверок + 1;

		Если (СчетчикПроверок > МаксСчетчикПроверок) и (ТекущаяДатаСеанса() >= (ТекДат+МаксСчетчикПроверок)) Тогда
...
Процедура ОстановитьЗаписьВидеоОдногоШага(ПараметрыВидео) Экспорт
	...
	Пока Истина Цикл
		...
		Если (КолПопыток >= МаксКолПопыток) и (ТекущаяДатаСеанса() >= (ТекДат + МаксКолПопыток)) Тогда
			Прервать;
		КонецЕсли;
...
Процедура ПроверитьЧтоЗаписьВидеоБудетРаботать(ПараметрыВидео) Экспорт
	...
	Если БылиОшибки Тогда
		Сообщить("Необходимо установить программное обеспечение. Подробности тут: https://github.com/vanessa-opensource/add/blob/develop/README.md");
